# OpenAI-Compatible API Server for ZedClaw
...
SSE streaming. By exposing this endpoint, zedclaw becomes instantly
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.zedclaw/.env`
2. Run `zedclaw gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with zedclaw through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns zedclaw as a model) |
...
Authorization: Bearer zedclaw-api-key-here
...
But zedclaw sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "zedclaw"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured zedclaw model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of zedclaw's core prompt
...
Once implemented, zedclaw works as a drop-in backend for:

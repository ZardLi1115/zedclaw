...
One ``CDPSupervisor`` runs per ZedClaw ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "zedclaw-dialog-bridge.invalid"
...
(() => {
  if (window.__zedclawDialogBridgeInstalled) return;
  window.__zedclawDialogBridgeInstalled = true;
  const ENDPOINT = "http://zedclaw-dialog-bridge.invalid/";

...
This module provides the glue:
    - ``ZedClawTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "ZedClaw"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses ZEDCLAW_HOME so each profile gets its own OAuth tokens.
    Layout: ``ZEDCLAW_HOME/mcp-tokens/``
    """
    try:
        from zedclaw_constants import get_zedclaw_home
        base = Path(get_zedclaw_home())
    except ImportError:
        base = Path(os.environ.get("ZEDCLAW_HOME", str(Path.home() / ".zedclaw")))
...
# ---------------------------------------------------------------------------
# ZedClawTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class ZedClawTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        ZEDCLAW_HOME/mcp-tokens/<server_name>.json         -- tokens
        ZEDCLAW_HOME/mcp-tokens/<server_name>.client.json   -- client info
        ZEDCLAW_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # ZedClaw records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to ZedClaw.</p></body></html>"
            ) if code else (
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "ZedClaw")
    scope = cfg.get("scope")

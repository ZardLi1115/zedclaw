"""
Lazy dependency installer for opt-in ZedClaw backends.

Many ZedClaw features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``zedclaw[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``zedclaw tools`` or the manual pip command.
...
def _allow_lazy_installs() -> bool:
    ...
    """
    if os.environ.get("ZEDCLAW_DISABLE_LAZY_INSTALLS") == "1":
        return False
    try:
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    Mirrors the strategy in ``zedclaw_cli.tools_config._pip_install`` but

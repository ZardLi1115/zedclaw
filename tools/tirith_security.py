...
it is automatically downloaded from GitHub releases to $ZEDCLAW_HOME/bin/tirith.
...
def _get_zedclaw_home() -> str:
    """Return the ZedClaw home directory, respecting ZEDCLAW_HOME env var."""
    return str(get_zedclaw_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_zedclaw_home(), ".tirith-install-failed")
...
def _zedclaw_bin_dir() -> str:
    """Return $ZEDCLAW_HOME/bin, creating it if needed."""
    d = os.path.join(_get_zedclaw_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $ZEDCLAW_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_zedclaw_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $ZEDCLAW_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $ZEDCLAW_HOME/bin/tirith
    ...
    zedclaw_bin = os.path.join(_zedclaw_bin_dir(), "tirith")
    if os.path.isfile(zedclaw_bin) and os.access(zedclaw_bin, os.X_OK):
        _resolved_path = zedclaw_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return zedclaw_bin

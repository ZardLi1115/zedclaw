...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("ZEDCLAW_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return os.getenv("ZEDCLAW_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set ZEDCLAW_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind ZEDCLAW_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds ZEDCLAW_SESSION_PLATFORM via
    ...
    """
    if os.getenv("ZEDCLAW_CRON_SESSION"):
        return False
...
# via shell expansions like $HOME or $ZEDCLAW_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_ZEDCLAW_ENV_PATH = (
    r'(?:~\/\.zedclaw/|'
    r'(?:\$home|\$\{home\})/\.zedclaw/|'
    r'(?:\$zedclaw_home|\$\{zedclaw_home\})/)'
    r'\.env\b'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bzedclaw\s+gateway\s+(stop|restart)\b', "stop/restart zedclaw gateway (kills running agents)"),
    (r'\bzedclaw\s+update\b', "zedclaw update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart zedclaw-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart zedclaw-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(zedclaw|gateway|cli\.py)\b', "kill zedclaw/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill zedclaw` but not
    # `kill -9 $(pgrep -f zedclaw)` because the substitution is opaque
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("ZEDCLAW_YOLO_MODE")) or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = os.getenv("ZEDCLAW_INTERACTIVE")
    ...
    if is_gateway or os.getenv("ZEDCLAW_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("ZEDCLAW_YOLO_MODE")) or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = os.getenv("ZEDCLAW_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = os.getenv("ZEDCLAW_EXEC_ASK")

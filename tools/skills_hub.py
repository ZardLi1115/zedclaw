#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the ZedClaw Skills Hub.
...
Used by zedclaw_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            zedclaw_meta = metadata.get("zedclaw", {})
            if isinstance(zedclaw_meta, dict):
                tags = zedclaw_meta.get("tags", [])
        if not tags:
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            zedclaw_meta = metadata.get("zedclaw", {})
            if isinstance(zedclaw_meta, dict):
                raw_tags = zedclaw_meta.get("tags", [])
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.zedclaw/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                zedclaw_meta = meta_block.get("zedclaw", {})
                if isinstance(zedclaw_meta, dict):
                    tags = zedclaw_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# ZedClaw centralized index source
# ---------------------------------------------------------------------------

ZEDCLAW_INDEX_URL = "https://zedclaw.nousresearch.com/docs/api/skills-index.json"
ZEDCLAW_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "zedclaw-index.json"
ZEDCLAW_INDEX_TTL = 6 * 3600  # 6 hours


def _load_zedclaw_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for ZEDCLAW_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if ZEDCLAW_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - ZEDCLAW_INDEX_CACHE_FILE.stat().st_mtime
            if age < ZEDCLAW_INDEX_TTL:
                return json.loads(ZEDCLAW_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(ZEDCLAW_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("ZedClaw index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("ZedClaw index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        ZEDCLAW_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        ZEDCLAW_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class ZedClawIndexSource(SkillSource):
    """Skill source backed by the centralized ZedClaw Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_zedclaw_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "zedclaw-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "zedclaw-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        ZedClawIndexSource(auth=auth), # Centralized index (search + resolved install paths)
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if source_filter == "all":
        for src in sources:
            if (src.source_id() == "zedclaw-index"

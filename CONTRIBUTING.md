# Contributing to ZedClaw

Thank you for contributing to ZedClaw! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want ZedClaw to work everywhere.
...
Bundled skills (in `skills/`) ship with every ZedClaw install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `zedclaw skills browse` (labeled "official") and install it with `zedclaw skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `zedclaw skills install`.
...
```bash
git clone --recurse-submodules https://github.com/NousResearch/zedclaw.git
cd zedclaw
...
```bash
mkdir -p ~/.zedclaw/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.zedclaw/config.yaml
touch ~/.zedclaw/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.zedclaw/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/zedclaw" ~/.local/bin/zedclaw

# Verify
zedclaw doctor
zedclaw chat -q "Hello"
```
...
├── cli.py                    # ZedClawCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (zedclaw-cli, zedclaw-telegram, etc.)
├── zedclaw_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── zedclaw_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.zedclaw/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── environments/             # RL training environments (Atropos integration)
├── tests/                    # Test suite
├── website/                  # Documentation site (zedclaw.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.zedclaw/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.zedclaw/`)

| Path | Purpose |
|------|---------|
| `~/.zedclaw/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.zedclaw/.env` | API keys and secrets |
| `~/.zedclaw/auth.json` | OAuth credentials (Nous Portal) |
| `~/.zedclaw/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.zedclaw/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.zedclaw/state.db` | SQLite session database |
| `~/.zedclaw/sessions/` | JSON session logs |
| `~/.zedclaw/cron/` | Scheduled job data |
| `~/.zedclaw/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`zedclaw_state.py`) with full-text search and unique session titles. JSON logs go to `~/.zedclaw/sessions/`.
...
(for example `_ZEDCLAW_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.zedclaw`:
...
The user may skip setup and keep loading the skill. ZedClaw only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `zedclaw setup` or update `~/.zedclaw/.env` locally.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing ZedClaw tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `zedclaw --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

ZedClaw uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.zedclaw/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `zedclaw_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `zedclaw_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

ZedClaw runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the zedclaw wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `zedclaw_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `zedclaw_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `zedclaw_cli/gateway_windows.py::
...
ZedClaw has terminal access. Security matters.
...
2. **Test manually**: Run `zedclaw` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/NousResearch/zedclaw/issues)
- Include: OS, Python version, ZedClaw version (`zedclaw version`), full error traceback

"""Centralized logging setup for ZedClaw.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.zedclaw/logs/`` (profile-aware via ``get_zedclaw_home()``).
...
from zedclaw_constants import get_config_path, get_zedclaw_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_zedclaw_session_injector", False):
    ...
    _session_record_factory._zedclaw_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``zedclaw logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway",),
...
def setup_logging(
    *,
    ...
    """Configure the ZedClaw logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    zedclaw_home
        Override for the ZedClaw home directory.  Falls back to
        ``get_zedclaw_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_zedclaw_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._zedclaw_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the zedclaw group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from zedclaw_cli.config import is_managed

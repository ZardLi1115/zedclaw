...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"zedclaw[termux]"' in text
    assert '"zedclaw[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"zedclaw[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]

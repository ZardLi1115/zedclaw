"""Tests for zedclaw_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every ZedClaw entry point
(zedclaw, zedclaw, zedclaw-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every ZedClaw entry point imports zedclaw_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported zedclaw_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("zedclaw_bootstrap", None)
    import zedclaw_bootstrap  # noqa: WPS433
    return zedclaw_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the ZedClaw CLI
...
class TestEntryPointsImportBootstrap:
    """Every ZedClaw entry point must import zedclaw_bootstrap as its
    ...
    # Entry points that invoke ZedClaw as a process.  Each one must
    # import zedclaw_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "zedclaw_cli/main.py",   # zedclaw CLI (console_script)
        "run_agent.py",          # zedclaw (console_script)
        "acp_adapter/entry.py",  # zedclaw-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import zedclaw_bootstrap' and that
        ...
        half-finished ``zedclaw update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``zedclaw_bootstrap`` as a top-level module) leaves zedclaw
        ...
        """
        # Resolve relative to the zedclaw repo root.  Tests live
        # at tests/test_zedclaw_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # zedclaw start even when zedclaw_bootstrap hasn't been
        ...
        assert first_import_name == "zedclaw_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'zedclaw_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import zedclaw_bootstrap' line to be the first import."
        )

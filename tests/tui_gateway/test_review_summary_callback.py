...
def server():
    with patch.dict(
        "sys.modules",
        {
            "zedclaw_constants": MagicMock(
                get_zedclaw_home=MagicMock(return_value="/tmp/zedclaw_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'zedclaw-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'zedclaw-release' patched"
    }

...
Supergroup forum topics route with ``message_thread_id``. ZedClaw-created
...
async def test_send_typing_skips_api_call_for_dm_topic_reply_fallback():
    """ZedClaw-created DM topic lanes have no working Bot API typing route.
...
@pytest.mark.asyncio
async def test_send_uses_reply_fallback_for_zedclaw_dm_topics():
    """ZedClaw-created Telegram DM topics route with thread id plus reply anchor."""
    adapter = _make_adapter()
    call_log = []
...
async def test_send_uses_metadata_reply_fallback_for_streaming_dm_topics():
    """Metadata-only sends still stay in ZedClaw-created Telegram DM topics."""
    adapter = _make_adapter()
    call_log = []

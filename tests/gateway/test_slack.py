...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.zedclaw."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /zedclaw, /btw, /stop,
        ...
        for expected in ("/zedclaw", "/btw", "/stop", "/model", "/help"):
...
class TestSlashCommands:
    ...
    # instead of as /zedclaw subcommands. This is the Discord/Telegram parity
    # fix: the slash name itself becomes the command.
    # ------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /zedclaw btw."""
        command = {
            "command": "/btw",
    ...
    @pytest.mark.asyncio
    async def test_legacy_zedclaw_prefix_still_works(self, adapter):
        """Backward compat: /zedclaw btw foo must still route to /btw foo.

        Old workspace manifests only declared /zedclaw as the single slash.
    ...
    @pytest.mark.asyncio
    async def test_legacy_zedclaw_freeform_question(self, adapter):
        """/zedclaw <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/zedclaw",
...
class TestSlashEphemeralAck:
    ...
    @pytest.mark.asyncio
    async def test_legacy_zedclaw_slash_stashes_context(self, adapter):
        """Legacy /zedclaw <subcommand> also stashes context."""
        command = {
            "command": "/zedclaw",
            "text": "help",
            "user_id": "U_H",
    ...
    @pytest.mark.asyncio
    async def test_freeform_zedclaw_question_does_not_stash_context(self, adapter):
        """Free-form /zedclaw <question> must NOT route agent reply ephemeral."""
        command = {
            "command": "/zedclaw",

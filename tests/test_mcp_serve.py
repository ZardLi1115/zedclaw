"""
Tests for mcp_serve — ZedClaw MCP server.
...
def _isolate_zedclaw_home(tmp_path, monkeypatch):
    """Redirect ZEDCLAW_HOME to a temp directory."""
    monkeypatch.setenv("ZEDCLAW_HOME", str(tmp_path))
    try:
        import zedclaw_constants
        monkeypatch.setattr(zedclaw_constants, "get_zedclaw_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking zedclaw_state schema."""

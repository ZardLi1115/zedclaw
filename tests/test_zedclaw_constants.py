"""Tests for zedclaw_constants module."""
...
from zedclaw_constants import (
    ...
    get_default_zedclaw_root,
    is_container,
    parse_reasoning_effort,
)


class TestGetDefaultZedClawRoot:
    """Tests for get_default_zedclaw_root() — Docker/custom deployment awareness."""

    def test_no_zedclaw_home_returns_native(self, tmp_path, monkeypatch):
        """When ZEDCLAW_HOME is not set, returns ~/.zedclaw."""
        monkeypatch.delenv("ZEDCLAW_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_zedclaw_root() == tmp_path / ".zedclaw"

    def test_zedclaw_home_is_native(self, tmp_path, monkeypatch):
        """When ZEDCLAW_HOME = ~/.zedclaw, returns ~/.zedclaw."""
        native = tmp_path / ".zedclaw"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZEDCLAW_HOME", str(native))
        assert get_default_zedclaw_root() == native

    def test_zedclaw_home_is_profile(self, tmp_path, monkeypatch):
        """When ZEDCLAW_HOME is a profile under ~/.zedclaw, returns ~/.zedclaw."""
        native = tmp_path / ".zedclaw"
    ...
    def test_zedclaw_home_is_docker(self, tmp_path, monkeypatch):
        """When ZEDCLAW_HOME points outside ~/.zedclaw (Docker), returns ZEDCLAW_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZEDCLAW_HOME", str(docker_home))
        assert get_default_zedclaw_root() == docker_home

    def test_zedclaw_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any ZEDCLAW_HOME outside ~/.zedclaw is treated as the root."""
        custom = tmp_path / "my-zedclaw-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZEDCLAW_HOME", str(custom))
        assert get_default_zedclaw_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (ZEDCLAW_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZEDCLAW_HOME", str(profile))
        assert get_default_zedclaw_root() == docker_root
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(zedclaw_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(zedclaw_constants, "_container_detected", True)
        assert is_container() is True

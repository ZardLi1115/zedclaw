...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "zedclaw"
        assert config.workspace_id == "zedclaw"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.zedclaw/honcho.json
        monkeypatch.setenv("ZEDCLAW_HOME", str(tmp_path / "isolated"))
    ...
    def test_context_tokens_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "contextTokens": 1000,
            "hosts": {"zedclaw": {"contextTokens": 2000}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.context_tokens == 2000
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"zedclaw": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"zedclaw": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="zedclaw"
        ):
            result = config.resolve_session_name("/home/user/zedclaw/subdir")
...
class TestResolveConfigPath:
    def test_prefers_zedclaw_home_when_exists(self, tmp_path):
        zedclaw_home = tmp_path / "zedclaw"
        zedclaw_home.mkdir()
        local_cfg = zedclaw_home / "honcho.json"
    ...
    def test_falls_back_to_global_when_no_local(self, tmp_path):
        zedclaw_home = tmp_path / "zedclaw"
        zedclaw_home.mkdir()
        # No honcho.json in ZEDCLAW_HOME — also isolate ~/.zedclaw so
    ...
    def test_falls_back_to_global_without_zedclaw_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()

        with patch.dict(os.environ, {}, clear=False), \
             patch.object(Path, "home", return_value=fake_home):
            os.environ.pop("ZEDCLAW_HOME", None)
            result = resolve_config_path()
        assert result == fake_home / ".honcho" / "config.json"
...
class TestResolveActiveHost:
    def test_default_returns_zedclaw(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("ZEDCLAW_HONCHO_HOST", None)
            os.environ.pop("ZEDCLAW_HOME", None)
            assert resolve_active_host() == "zedclaw"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"ZEDCLAW_HONCHO_HOST": "zedclaw.coder"}):
            assert resolve_active_host() == "zedclaw.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ZEDCLAW_HONCHO_HOST", None)
            with patch("zedclaw_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "zedclaw.coder"

    def test_default_profile_returns_zedclaw(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ZEDCLAW_HONCHO_HOST", None)
            with patch("zedclaw_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "zedclaw"

    def test_custom_profile_returns_zedclaw(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ZEDCLAW_HONCHO_HOST", None)
            with patch("zedclaw_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "zedclaw"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ZEDCLAW_HONCHO_HOST", None)
            # Temporarily remove zedclaw_cli.profiles to simulate import failure
            saved = sys.modules.get("zedclaw_cli.profiles")
            sys.modules["zedclaw_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "zedclaw"
            finally:
                if saved is not None:
                    sys.modules["zedclaw_cli.profiles"] = saved
                else:
                    sys.modules.pop("zedclaw_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="zedclaw.coder")
        assert config.host == "zedclaw.coder"
        assert config.workspace_id == "zedclaw"  # shared workspace
        assert config.ai_peer == "zedclaw.coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="zedclaw")
        assert config.host == "zedclaw"
        assert config.workspace_id == "zedclaw"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "zedclaw": {"aiPeer": "zedclaw", "peerName": "alice"},
                "zedclaw.coder": {
                    "aiPeer": "zedclaw.coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="zedclaw.coder", config_path=config_file,
        )
        assert config.host == "zedclaw.coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "zedclaw.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="zedclaw.dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "zedclaw.dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"zedclaw": {"enabled": True, "aiPeer": "zedclaw"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"zedclaw": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_zedclaw_config_timeout_override_used_when_config_timeout_missing(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="zedclaw",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("zedclaw_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_defaults_to_30s_when_no_timeout_configured(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("zedclaw_cli.config.load_config", return_value={}):
            client = get_honcho_client(cfg)
    ...
    def test_zedclaw_request_timeout_alias_used(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="zedclaw",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("zedclaw_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
...
class TestDialecticDepthParsing:
    ...
    def test_depth_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "***",
            "dialecticDepth": 1,
            "hosts": {"zedclaw": {"dialecticDepth": 3}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)

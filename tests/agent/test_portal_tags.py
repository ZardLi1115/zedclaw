...
def test_zedclaw_client_tag_includes_current_version():
    """The client tag must reflect zedclaw_cli.__version__ verbatim."""
    from zedclaw_cli import __version__
    from agent.portal_tags import zedclaw_client_tag

    assert zedclaw_client_tag() == f"client=zedclaw-client-v{__version__}"


def test_zedclaw_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import zedclaw_client_tag

    tag = zedclaw_client_tag()
    assert tag.startswith("client=zedclaw-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import zedclaw_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=zedclaw" in tags
    assert zedclaw_client_tag() in tags
    assert len(tags) == 2

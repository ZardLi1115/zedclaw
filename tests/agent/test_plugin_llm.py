...
""",
        ...
        from zedclaw_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        zedclaw_home = tmp_path / ".zedclaw"
        zedclaw_home.mkdir()
        (zedclaw_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("ZEDCLAW_HOME", str(zedclaw_home))
        from zedclaw_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from zedclaw_cli.plugins import PluginContext, PluginManifest, PluginManager

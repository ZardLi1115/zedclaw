...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "ZedClaw Tests")
...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_zedclaw_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("ZEDCLAW_HOME", str(tmp_path / ".zedclaw"))

    zedclaw_env = tmp_path / ".zedclaw" / ".env"
    zedclaw_env.parent.mkdir(parents=True)
    zedclaw_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.zedclaw/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,

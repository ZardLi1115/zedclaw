...
See: https://github.com/NousResearch/zedclaw/issues/4426
"""
...
class TestGetSubprocessHome:
    """Unit tests for zedclaw_constants.get_subprocess_home()."""

    def test_returns_none_when_zedclaw_home_unset(self, monkeypatch):
    ...
    def test_returns_path_when_home_dir_exists(self, tmp_path, monkeypatch):
        zedclaw_home = tmp_path / ".zedclaw"
        zedclaw_home.mkdir()
        profile_home = zedclaw_home / "home"
        profile_home.mkdir()
        monkeypatch.setenv("ZEDCLAW_HOME", str(zedclaw_home))
        from zedclaw_constants import get_subprocess_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        base = tmp_path / ".zedclaw" / "profiles"
        ...
        monkeypatch.setenv("ZEDCLAW_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("ZEDCLAW_HOME", str(base / "beta"))
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_injects_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(zedclaw_home / "home")
    ...
    def test_no_injection_when_zedclaw_home_unset(self, monkeypatch):
        monkeypatch.delenv("ZEDCLAW_HOME", raising=False)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from zedclaw_cli.profiles import _PROFILE_DIRS

...
class TestToolsetConsistency:
    ...
    def test_zedclaw_platforms_share_core_tools(self):
        """All zedclaw-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on zedclaw-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["zedclaw-cli", "zedclaw-telegram", "zedclaw-discord", "zedclaw-whatsapp", "zedclaw-slack", "zedclaw-signal", "zedclaw-homeassistant"]

...
- Path resolution (absolute, relative to ZEDCLAW_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp ZEDCLAW_HOME."""
    zedclaw_home = tmp_path / ".zedclaw"
    zedclaw_home.mkdir()
    (zedclaw_home / "cron").mkdir()
    (zedclaw_home / "cron" / "output").mkdir()
    (zedclaw_home / "scripts").mkdir()
    monkeypatch.setenv("ZEDCLAW_HOME", str(zedclaw_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "ZEDCLAW_DIR", zedclaw_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", zedclaw_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", zedclaw_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", zedclaw_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.zedclaw/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("ZEDCLAW_SESSION_PLATFORM") is None
        assert os.environ.get("ZEDCLAW_SESSION_CHAT_ID") is None
        assert os.environ.get("ZEDCLAW_SESSION_CHAT_NAME") is None

"""Shared fixtures for the zedclaw test suite.
...
2. **Isolated ZEDCLAW_HOME.** ZEDCLAW_HOME points to a per-test tempdir so
   code reading ``~/.zedclaw/*`` via ``get_zedclaw_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".zedclaw"`` instead of the canonical
   ``get_zedclaw_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No ZEDCLAW_SESSION_* inheritance** — the agent's current gateway
...
# ZEDCLAW_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_ZEDCLAW_BEHAVIORAL_VARS = frozenset({
    "ZEDCLAW_YOLO_MODE",
    "ZEDCLAW_INTERACTIVE",
    "ZEDCLAW_QUIET",
    ...
    "ZEDCLAW_SESSION_CHAT_ID",
    "ZEDCLAW_SESSION_CHAT_NAME",
    "ZEDCLAW_SESSION_THREAD_ID",
    "ZEDCLAW_SESSION_SOURCE",
    "ZEDCLAW_SESSION_KEY",
    "ZEDCLAW_GATEWAY_SESSION",
    "ZEDCLAW_PLATFORM",
    "ZEDCLAW_MODEL",
    "ZEDCLAW_INFERENCE_MODEL",
    "ZEDCLAW_INFERENCE_PROVIDER",
    "ZEDCLAW_TUI_PROVIDER",
    "ZEDCLAW_MANAGED",
    "ZEDCLAW_DEV",
    "ZEDCLAW_CONTAINER",
    "ZEDCLAW_EPHEMERAL_SYSTEM_PROMPT",
    "ZEDCLAW_TIMEZONE",
    "ZEDCLAW_REDACT_SECRETS",
    "ZEDCLAW_BACKGROUND_NOTIFICATIONS",
    "ZEDCLAW_EXEC_ASK",
    "ZEDCLAW_HOME_MODE",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.zedclaw/kanban.db instead of the per-test ZEDCLAW_HOME.
    "ZEDCLAW_KANBAN_DB",
    "ZEDCLAW_KANBAN_BOARD",
    "ZEDCLAW_KANBAN_WORKSPACES_ROOT",
    "ZEDCLAW_KANBAN_LOGS_ROOT",
    "ZEDCLAW_KANBAN_TASK",
    "ZEDCLAW_KANBAN_WORKSPACE",
    "ZEDCLAW_TENANT",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and ZEDCLAW_HOME to per-test tempdirs so code that
    reads ``~/.zedclaw/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral ZEDCLAW_* vars that could change test semantics.
    for name in _ZEDCLAW_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect ZEDCLAW_HOME to a per-test tempdir. Code that reads
    #    ``~/.zedclaw/*`` via ``get_zedclaw_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.zedclaw/*`` via
    #    ``Path.home() / ".zedclaw"`` instead of ``get_zedclaw_home()``
    #    is a bug to fix at the callsite.
    fake_zedclaw_home = tmp_path / "zedclaw_test"
    fake_zedclaw_home.mkdir()
    (fake_zedclaw_home / "sessions").mkdir()
    (fake_zedclaw_home / "cron").mkdir()
    (fake_zedclaw_home / "memories").mkdir()
    (fake_zedclaw_home / "skills").mkdir()
    monkeypatch.setenv("ZEDCLAW_HOME", str(fake_zedclaw_home))
    ...
    #    ~/.zedclaw/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import zedclaw_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_zedclaw_home(_hermetic_environment):
...
def _reset_module_state():
    ...
    for _logger_name in ("tools", "run_agent", "trajectory_compressor", "cron", "zedclaw_cli"):
...
@pytest.fixture()
def mock_config():
    """Return a minimal zedclaw config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``zedclaw-gateway`` process
...
# correlated with a ``tests/zedclaw_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> zedclaw-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart zedclaw-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting zedclaw/python patterns are also blocked.
    """
    ...
    def _matches_zedclaw_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _ZEDCLAW_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_zedclaw_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch zedclaw-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m zedclaw_cli.main".
                if (
                    "zedclaw" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live zedclaw-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting zedclaw/python could hit the live gateway. "

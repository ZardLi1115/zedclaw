...
def _clean_state():
    ...
    saved = {}
    for k in ("ZEDCLAW_INTERACTIVE", "ZEDCLAW_GATEWAY_SESSION", "ZEDCLAW_EXEC_ASK", "ZEDCLAW_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["ZEDCLAW_INTERACTIVE"] = "1"
        session_key = os.getenv("ZEDCLAW_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No ZEDCLAW_INTERACTIVE or ZEDCLAW_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True

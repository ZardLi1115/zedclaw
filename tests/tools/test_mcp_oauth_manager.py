...
def test_zedclaw_provider_subclass_exists():
    """ZedClawMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _ZEDCLAW_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _ZEDCLAW_PROVIDER_CLS is not None
    assert issubclass(_ZEDCLAW_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_zedclaw_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns ZedClawMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _ZEDCLAW_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("ZEDCLAW_HOME", str(tmp_path))
    ...
    assert _ZEDCLAW_PROVIDER_CLS is not None
    assert isinstance(provider, _ZEDCLAW_PROVIDER_CLS)
    assert provider._zedclaw_server_name == "srv"

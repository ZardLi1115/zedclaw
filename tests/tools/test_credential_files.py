...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        zedclaw_home.mkdir()
        (zedclaw_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"ZEDCLAW_HOME": str(zedclaw_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(zedclaw_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.zedclaw/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        zedclaw_home = tmp_path / ".zedclaw"
        zedclaw_home.mkdir()
        (zedclaw_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        zedclaw_home = tmp_path / ".zedclaw"
        zedclaw_home.mkdir()
        (zedclaw_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        zedclaw_home = tmp_path / ".zedclaw"
        zedclaw_home.mkdir()
        (zedclaw_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        zedclaw_home = tmp_path / ".zedclaw"
        skills_dir = zedclaw_home / "skills"
        ...
        assert mounts[0]["container_path"] == "/root/.zedclaw/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        zedclaw_home = tmp_path / ".zedclaw"
        (zedclaw_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"ZEDCLAW_HOME": str(zedclaw_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.zedclaw")

        assert mounts[0]["container_path"] == "/home/user/.zedclaw/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.zedclaw/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.zedclaw/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape ZEDCLAW_HOME."""
        monkeypatch.setenv("ZEDCLAW_HOME", str(tmp_path / ".zedclaw"))
        (tmp_path / ".zedclaw").mkdir()

        # Create a sensitive file one level above zedclaw_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside zedclaw_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside ZEDCLAW_HOME must still be registered."""
        zedclaw_home = tmp_path / ".zedclaw"
        zedclaw_home.mkdir()
        monkeypatch.setenv("ZEDCLAW_HOME", str(zedclaw_home))
        (zedclaw_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_zedclaw_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of ZEDCLAW_HOME must be allowed."""
        zedclaw_home = tmp_path / ".zedclaw"
        zedclaw_home.mkdir()
        subdir = zedclaw_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside ZEDCLAW_HOME pointing outside must be rejected."""
        zedclaw_home = tmp_path / ".zedclaw"
        zedclaw_home.mkdir()
        monkeypatch.setenv("ZEDCLAW_HOME", str(zedclaw_home))

        # Create a sensitive file outside zedclaw_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside zedclaw_home pointing outside
        symlink = zedclaw_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes ZEDCLAW_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, zedclaw_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape ZEDCLAW_HOME."""
        zedclaw_home = tmp_path / ".zedclaw"
        zedclaw_home.mkdir()
        monkeypatch.setenv("ZEDCLAW_HOME", str(zedclaw_home))
        ...
        sensitive.write_text("{}")
        self._write_config(zedclaw_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(zedclaw_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside ZEDCLAW_HOME via config must still mount."""
        zedclaw_home = tmp_path / ".zedclaw"
        zedclaw_home.mkdir()
        monkeypatch.setenv("ZEDCLAW_HOME", str(zedclaw_home))

        (zedclaw_home / "oauth.json").write_text("{}")
        self._write_config(zedclaw_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        zedclaw_home = tmp_path / ".zedclaw"
        zedclaw_home.mkdir()
        (zedclaw_home / "cache" / "documents").mkdir(parents=True)
        (zedclaw_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("ZEDCLAW_HOME", str(zedclaw_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.zedclaw/cache/documents" in paths
        assert "/root/.zedclaw/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.zedclaw/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        zedclaw_home = tmp_path / ".zedclaw"
        zedclaw_home.mkdir()
        # Use legacy dir name — get_zedclaw_dir prefers old if it exists
        (zedclaw_home / "document_cache").mkdir()
        (zedclaw_home / "image_cache").mkdir()
        monkeypatch.setenv("ZEDCLAW_HOME", str(zedclaw_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(zedclaw_home / "document_cache") in host_paths
        assert str(zedclaw_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.zedclaw/cache/documents" in container_paths
        assert "/root/.zedclaw/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        zedclaw_home = tmp_path / ".zedclaw"
        doc_dir = zedclaw_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        zedclaw_home = tmp_path / ".zedclaw"
        ss_dir = zedclaw_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.zedclaw/cache/screenshots/session_abc/screen1.png"

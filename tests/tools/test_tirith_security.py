...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._zedclaw_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_zedclaw_bin(self):
        """After _INSTALL_FAILED, manual install in ZEDCLAW_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        zedclaw_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(zedclaw_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(zedclaw_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._zedclaw_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == zedclaw_bin
            assert _tirith_mod._resolved_path == zedclaw_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# ZEDCLAW_HOME isolation
# ---------------------------------------------------------------------------

class TestZedClawHomeIsolation:
    def test_zedclaw_bin_dir_respects_zedclaw_home(self):
        """_zedclaw_bin_dir must use ZEDCLAW_HOME, not hardcoded ~/.zedclaw."""
        from tools.tirith_security import _zedclaw_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"ZEDCLAW_HOME": tmpdir}):
            result = _zedclaw_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_zedclaw_home(self):
        """_failure_marker_path must use ZEDCLAW_HOME, not hardcoded ~/.zedclaw."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"ZEDCLAW_HOME": "/custom/zedclaw"}):
            result = _failure_marker_path()
        assert result == "/custom/zedclaw/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets ZEDCLAW_HOME; verify it's active."""
        zedclaw_home = os.getenv("ZEDCLAW_HOME")
        assert zedclaw_home is not None, "ZEDCLAW_HOME should be set by conftest"
        assert "zedclaw_test" in zedclaw_home, "Should point to test temp dir"

    def test_get_zedclaw_home_fallback(self):
        """Without ZEDCLAW_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_zedclaw_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove ZEDCLAW_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".zedclaw")
            result = _get_zedclaw_home()

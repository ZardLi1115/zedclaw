...
- Signal forwarding runs through the init so ``docker stop`` triggers
  zedclaw's own graceful-shutdown path.
"""
...
def test_dockerfile_installs_an_init_for_zombie_reaping(dockerfile_text):
    ...
    Without a PID-1 init that handles SIGCHLD, zedclaw accumulates zombie
    ...
    assert installed, (
        ...
        "orphaned subprocesses, zedclaw accumulates zombies in Docker "
...
def test_dockerfile_entrypoint_routes_through_the_init(dockerfile_text):
    ...
    directly, the shell becomes PID 1 and will ``exec`` into zedclaw,
    ...
    assert routes_through_init, (
        ...
        "If tini is only installed but not wired into ENTRYPOINT, zedclaw "
...
def test_dockerfile_installs_tui_dependencies(dockerfile_text):
    ...
    # resolve dependencies. The bundled ``zedclaw-ink`` workspace package is
    ...
    assert "ui-tui/packages/zedclaw-ink/" in dockerfile_text
    assert any(
...
def test_dockerfile_materializes_local_tui_ink_package(dockerfile_text):
    # ``zedclaw-ink`` is a bundled workspace package referenced from
    # ``ui-tui/package.json`` via ``file:`` — not pulled from the npm
    # registry. The contract this test pins is just that the image
    # actually carries the package source so ``await import('@zedclaw/ink')``
    # can resolve at runtime; the previous, much pickier assertion (manual
    # ``rm -rf`` + ``npm install --omit=dev --prefix node_modules/@zedclaw/ink``)
    ...
    assert "ui-tui/packages/zedclaw-ink/" in dockerfile_text, (
        "Dockerfile must COPY the bundled zedclaw-ink workspace package "
        "so ``await import('@zedclaw/ink')`` resolves at runtime."
    )

...
class TestWriteDenyExactPaths:
    ...
    def test_zedclaw_env(self):
        # ``.env`` under the active ZEDCLAW_HOME (profile-aware, not just
        # ``~/.zedclaw``) must be write-denied. The hermetic test conftest
        # points ZEDCLAW_HOME at a tempdir — resolve via get_zedclaw_home()
        # to match the denylist.
        from zedclaw_constants import get_zedclaw_home
        path = str(get_zedclaw_home() / ".env")
        assert _is_write_denied(path) is True
...
class TestWriteAllowed:
    ...
    def test_zedclaw_config_not_env(self):
        path = os.path.join(str(Path.home()), ".zedclaw", "config.yaml")

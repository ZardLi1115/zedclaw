...
def _restore_tool_modules():
    original_zedclaw_home = os.environ.get("ZEDCLAW_HOME")
    original_modules = {
        name: module
        ...
        or name == "zedclaw_cli"
        or name.startswith("zedclaw_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_zedclaw_home is None:
            os.environ.pop("ZEDCLAW_HOME", None)
        else:
            os.environ["ZEDCLAW_HOME"] = original_zedclaw_home
        _reset_modules(("tools", "zedclaw_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    zedclaw_cli = types.ModuleType("zedclaw_cli")
    zedclaw_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["zedclaw_cli"] = zedclaw_cli
    zedclaw_home = tmp_path / "zedclaw-home"
    os.environ["ZEDCLAW_HOME"] = str(zedclaw_home)
    sys.modules["zedclaw_cli.config"] = types.SimpleNamespace(
        get_zedclaw_home=lambda: zedclaw_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="zedclaw", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": zedclaw_home / "modal_snapshots.json",

"""Tests for the `zedclaw memory reset` CLI command.
...
- Profile-scoped reset (uses ZEDCLAW_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake ZEDCLAW_HOME with memory files."""
    zedclaw_home = tmp_path / ".zedclaw"
    memories = zedclaw_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("ZEDCLAW_HOME", str(zedclaw_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nZedClaw repo is at ~/.zedclaw/zedclaw\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `zedclaw memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        zedclaw_home = tmp_path / ".zedclaw"
        (zedclaw_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("ZEDCLAW_HOME", str(zedclaw_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's ZEDCLAW_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        zedclaw_home = tmp_path / ".zedclaw"
        zedclaw_home.mkdir(parents=True)
        # No memories dir
        monkeypatch.setenv("ZEDCLAW_HOME", str(zedclaw_home))

        # The memories dir won't exist; get_zedclaw_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"

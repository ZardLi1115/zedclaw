"""Tests for the Kanban DB layer (zedclaw_cli.kanban_db)."""
...
from zedclaw_cli import kanban_db as kb


@pytest.fixture
def kanban_home(tmp_path, monkeypatch):
    """Isolated ZEDCLAW_HOME with an empty kanban DB."""
...
def test_stale_claim_reclaimed(kanban_home, monkeypatch):
    import signal
    import zedclaw_cli.kanban_db as _kb
...
def test_dispatch_skips_nonspawnable_into_separate_bucket(kanban_home, monkeypatch):
    ...
    from zedclaw_cli import profiles
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real ZedClaw profile — preserves the
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="zedclaw-task", assignee="daily")
...
def test_scratch_workspace_created_under_zedclaw_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
...
# spawned with `zedclaw -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's ZEDCLAW_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's ZEDCLAW_HOME."""

    def _set_home(self, monkeypatch, tmp_path, zedclaw_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZEDCLAW_HOME", str(zedclaw_home))
        monkeypatch.delenv("ZEDCLAW_KANBAN_HOME", raising=False)

    def test_default_install_anchors_at_home_dot_zedclaw(
        self, tmp_path, monkeypatch
    ):
        # Standard install: ZEDCLAW_HOME == ~/.zedclaw, no profile active.
        default_home = tmp_path / ".zedclaw"
        default_home.mkdir()
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.zedclaw/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.zedclaw/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.zedclaw/kanban.db.
        default_home = tmp_path / ".zedclaw"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # ZEDCLAW_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `zedclaw -p coder`).
        monkeypatch.setenv("ZEDCLAW_HOME", str(profile_home))
    ...
    def test_docker_custom_zedclaw_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: ZEDCLAW_HOME points outside ~/.zedclaw.
        # `get_default_zedclaw_root()` returns env_home directly when it
        # is not a `<root>/profiles/<name>` shape and not under
        # `Path.home() / ".zedclaw"`.
        custom_root = tmp_path / "opt" / "zedclaw"
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: ZEDCLAW_HOME=/opt/zedclaw/profiles/coder;
        # `get_default_zedclaw_root()` walks up to /opt/zedclaw because
    ...
    def test_explicit_override_via_zedclaw_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # Explicit override: ZEDCLAW_KANBAN_HOME beats every other
        # resolution rule.
        default_home = tmp_path / ".zedclaw"
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZEDCLAW_HOME", str(profile_home))
        monkeypatch.setenv("ZEDCLAW_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        default_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZEDCLAW_HOME", str(default_home))
        monkeypatch.setenv("ZEDCLAW_KANBAN_HOME", "   ")
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # ZEDCLAW_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile ZEDCLAW_HOME and reads.
        monkeypatch.setenv("ZEDCLAW_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_zedclaw_kanban_db_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # ZEDCLAW_KANBAN_DB pins the file path directly and beats both
        # ZEDCLAW_KANBAN_HOME and the `get_default_zedclaw_root()` path.
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZEDCLAW_HOME", str(default_home))
        monkeypatch.setenv("ZEDCLAW_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("ZEDCLAW_KANBAN_DB", str(pinned_db))

        assert kb.kanban_db_path() == pinned_db
        # workspaces_root still follows ZEDCLAW_KANBAN_HOME -- the pins
        # are independent.
        assert kb.workspaces_root() == umbrella / "kanban" / "workspaces"

    def test_zedclaw_kanban_workspaces_root_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # ZEDCLAW_KANBAN_WORKSPACES_ROOT pins the workspaces root directly.
        default_home = tmp_path / ".zedclaw"
        default_home.mkdir()
        umbrella = tmp_path / "umbrella"
        umbrella.mkdir()
        ...
        monkeypatch.setenv("ZEDCLAW_HOME", str(default_home))
        monkeypatch.setenv("ZEDCLAW_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("ZEDCLAW_KANBAN_WORKSPACES_ROOT", str(pinned_ws))

        assert kb.workspaces_root() == pinned_ws
        # kanban_db_path still follows ZEDCLAW_KANBAN_HOME.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZEDCLAW_HOME", str(default_home))
        monkeypatch.setenv("ZEDCLAW_KANBAN_DB", "   ")
        monkeypatch.setenv("ZEDCLAW_KANBAN_WORKSPACES_ROOT", "")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # The dispatcher's `_default_spawn` must inject ZEDCLAW_KANBAN_DB
        # and ZEDCLAW_KANBAN_WORKSPACES_ROOT into the worker env so the
        # worker converges on the dispatcher's paths even when the
        # `-p <profile>` flag rewrites ZEDCLAW_HOME.
        default_home = tmp_path / ".zedclaw"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
        ...
        env = captured["env"]
        assert env["ZEDCLAW_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["ZEDCLAW_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["ZEDCLAW_KANBAN_TASK"] == "t_dispatch_env"
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see zedclaw_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(kanban_home, caplog):
    ...
    with _patch("zedclaw_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="zedclaw_state"):
            conn = kb.connect()
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'zedclaw kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_zedclaw_argv()
#
# Workers spawned by the dispatcher must use a `zedclaw` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["zedclaw", ...]`
...
def test_resolve_zedclaw_argv_prefers_path_shim(monkeypatch):
    """When `zedclaw` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
    import zedclaw_cli.kanban_db as kb

    monkeypatch.setattr(shutil, "which", lambda name: "/usr/local/bin/zedclaw")
    argv = kb._resolve_zedclaw_argv()
    assert argv == ["/usr/local/bin/zedclaw"]


def test_resolve_zedclaw_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m zedclaw_cli.main`.

    Pins the correct module name (NOT `zedclaw` — there is no top-level
    `zedclaw` package). Regression for #23198: the original PR shipped
    `python -m zedclaw` which fails with `No module named zedclaw` on every
    invocation.
    """
    import shutil
    import sys
    ...
    argv = kb._resolve_zedclaw_argv()
    assert argv == [sys.executable, "-m", "zedclaw_cli.main"]


def test_resolve_zedclaw_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `zedclaw_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m zedclaw_cli.main --version`
    ...
    assert "ZedClaw" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"

...
def container_env(tmp_path, monkeypatch):
    """Set up a fake ZEDCLAW_HOME with .container-mode file."""
    zedclaw_home = tmp_path / ".zedclaw"
    zedclaw_home.mkdir()
    monkeypatch.setenv("ZEDCLAW_HOME", str(zedclaw_home))
    monkeypatch.delenv("ZEDCLAW_DEV", raising=False)

    container_mode = zedclaw_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=zedclaw\n"
        "exec_user=zedclaw\n"
        "zedclaw_bin=/data/current-package/bin/zedclaw\n"
    )
    return zedclaw_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("zedclaw_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "zedclaw"
    assert info["exec_user"] == "zedclaw"
    assert info["zedclaw_bin"] == "/data/current-package/bin/zedclaw"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("zedclaw_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_zedclaw_dev(container_env, monkeypatch):
    """Returns None when ZEDCLAW_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("ZEDCLAW_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_zedclaw_dev_zero(container_env, monkeypatch):
    """ZEDCLAW_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("ZEDCLAW_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        zedclaw_home = Path(tmpdir) / ".zedclaw"
        zedclaw_home.mkdir()
        (zedclaw_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("zedclaw_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_zedclaw_home": lambda: zedclaw_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ZEDCLAW_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=zedclaw-custom\n"
        "exec_user=myuser\n"
        "zedclaw_bin=/opt/zedclaw/bin/zedclaw\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "zedclaw-custom"
    assert info["exec_user"] == "myuser"
    assert info["zedclaw_bin"] == "/opt/zedclaw/bin/zedclaw"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from zedclaw_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "zedclaw"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "zedclaw" in cmd
    assert "/data/current-package/bin/zedclaw" in cmd
    assert "chat" in cmd

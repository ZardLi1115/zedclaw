...
- Bare `zedclaw setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `zedclaw setup --quick` runs the narrower "fill in missing items" flow.
- `zedclaw setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("zedclaw_cli.setup.ensure_zedclaw_home", {}),
        ("zedclaw_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("zedclaw_cli.config.is_managed", {"return_value": False}),
        ("zedclaw_cli.setup.load_config", {"return_value": {}}),
        ("zedclaw_cli.setup.save_config", {}),
        ("zedclaw_cli.setup.get_env_value", {"return_value": None}),
        ("zedclaw_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("zedclaw_cli.setup._print_setup_summary", {}),
        ("zedclaw_cli.setup._offer_launch_chat", {}),
        ("zedclaw_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("zedclaw_cli.setup.load_config", {"return_value": {}}),
        ("zedclaw_cli.setup.save_config", {}),
        ("zedclaw_cli.auth.get_active_provider", {"return_value": None}),
        ("zedclaw_cli.setup.get_env_value", {"return_value": None}),
        ("zedclaw_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `zedclaw setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="zedclaw_cli.setup.prompt_choice",
                quick="zedclaw_cli.setup._run_quick_setup",
                model="zedclaw_cli.setup.setup_model_provider",
                terminal="zedclaw_cli.setup.setup_terminal_backend",
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("zedclaw_cli.setup.prompt_choice", {"return_value": 0}),
                first="zedclaw_cli.setup._run_first_time_quick_setup",
            )
            from zedclaw_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "zedclaw_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["zedclaw", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_quick_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["zedclaw", "setup", "--quick"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_bare_setup_has_both_flags_false(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["zedclaw", "setup"])
        try:
            main()
        except SystemExit:
            pass

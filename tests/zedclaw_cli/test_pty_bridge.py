"""Unit tests for zedclaw_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf zedclaw-ok"])
        try:
            output = _read_until(bridge, b"zedclaw-ok")
            assert b"zedclaw-ok" in output
        finally:
            bridge.close()
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$ZEDCLAW_PTY_TEST\""],
            env={**os.environ, "ZEDCLAW_PTY_TEST": "pty-env-works"},
        )
        try:

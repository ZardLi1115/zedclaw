"""Regression tests for the OAuth dispatcher in zedclaw_cli.web_server.
...
from zedclaw_cli.web_server import _SESSION_TOKEN, app

client = TestClient(app)
HEADERS = {"X-ZedClaw-Session-Token": _SESSION_TOKEN}
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        "zedclaw_cli.auth._minimax_request_user_code",
        return_value=fake_user_code_resp,
    ), patch(
        "zedclaw_cli.auth._minimax_pkce_pair",
...
def test_minimax_dashboard_poller_accepts_absolute_ms_expired_in():
    ...
    from zedclaw_cli import web_server as ws
    ...
    try:
        with patch(
            "zedclaw_cli.auth._minimax_poll_token",
            return_value={
                "status": "success",
                "access_token": "access",
            ...
            "zedclaw_cli.auth._minimax_save_auth_state",
...
def test_anthropic_pkce_branch_still_works():
    ...
    with patch(
        "zedclaw_cli.web_server._start_anthropic_pkce",
...
def test_unknown_pkce_provider_rejected_cleanly():
    ...
    fake_entry = {
        ...
        "flow": "pkce",
        "cli_command": "zedclaw auth add hypothetical-pkce-provider",

"""Tests for the top-level `./zedclaw` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./zedclaw` should use `zedclaw_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "zedclaw"
    called = []

    fake_main_module = types.ModuleType("zedclaw_cli.main")

    def fake_main():
        called.append("zedclaw_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "zedclaw_cli.main", fake_main_module)
    ...
    assert called == ["zedclaw_cli.main"]

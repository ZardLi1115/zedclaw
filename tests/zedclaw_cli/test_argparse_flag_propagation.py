...
subparser, placing the flag BEFORE the subcommand (e.g. 'zedclaw --yolo chat')
...
def _build_parser():
    """Build the zedclaw argument parser from the real code.
...
class TestYoloEnvVar:
    """Verify --yolo sets ZEDCLAW_YOLO_MODE regardless of flag position.
    ...
    def test_no_yolo_no_env(self):
        ...
        self._simulate_cmd_chat_yolo_check(args)
        assert os.environ.get("ZEDCLAW_YOLO_MODE") is None
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `zedclaw gateway run --accept-hooks` failed
    ...
    def test_accepted_at_every_position(self, argv):
        """Invoking `zedclaw <argv>` must exit 0 (help) rather than

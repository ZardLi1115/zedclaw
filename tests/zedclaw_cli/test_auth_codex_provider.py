"""Tests for Codex auth — tokens stored in ZedClaw auth store (~/.zedclaw/auth.json)."""
...
def _setup_zedclaw_auth(zedclaw_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the ZedClaw auth store."""
    ...
    auth_file = zedclaw_home / "auth.json"
...
def test_read_codex_tokens_success(tmp_path, monkeypatch):
    ...
    _setup_zedclaw_auth(zedclaw_home)
...
def test_read_codex_tokens_missing(tmp_path, monkeypatch):
    ...
    zedclaw_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store
    (zedclaw_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
    monkeypatch.setenv("ZEDCLAW_HOME", str(zedclaw_home))
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    zedclaw_home = tmp_path / "zedclaw"
    _setup_zedclaw_auth(zedclaw_home, access_token="")
    monkeypatch.setenv("ZEDCLAW_HOME", str(zedclaw_home))
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_zedclaw_auth(zedclaw_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("ZEDCLAW_HOME", str(zedclaw_home))
    ...
    monkeypatch.setattr("zedclaw_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    zedclaw_home = tmp_path / "zedclaw"
    _setup_zedclaw_auth(zedclaw_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("ZEDCLAW_HOME", str(zedclaw_home))
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to ZedClaw auth store, not ~/.codex/."""
    zedclaw_home = tmp_path / "zedclaw"
    codex_home = tmp_path / "codex-cli"
    zedclaw_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "zedclaw-at", "refresh_token": "zedclaw-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches ZedClaw store
    assert not (codex_home / "auth.json").exists()

    # ZedClaw auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "zedclaw-at"


def test_resolve_returns_zedclaw_auth_store_source(tmp_path, monkeypatch):
    zedclaw_home = tmp_path / "zedclaw"
    _setup_zedclaw_auth(zedclaw_home)
    monkeypatch.setenv("ZEDCLAW_HOME", str(zedclaw_home))

    creds = resolve_codex_runtime_credentials()
    assert creds["source"] == "zedclaw-auth-store"
...
def _patch_httpx(monkeypatch, response):
    ...
    monkeypatch.setattr("zedclaw_cli.auth.httpx.Client", _factory)
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "zedclaw_cli.auth.resolve_codex_runtime_credentials",
        lambda: {"base_url": DEFAULT_CODEX_BASE_URL},
    )
    monkeypatch.setattr(
        "zedclaw_cli.auth._import_codex_cli_tokens",
    ...
    monkeypatch.setattr("zedclaw_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("zedclaw_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",

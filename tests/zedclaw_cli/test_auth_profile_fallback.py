...
When ``ZEDCLAW_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.zedclaw/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.zedclaw            (has its own auth.json fixture)
    * Profile     -> tmp_path/.zedclaw/profiles/coder   (active, ZEDCLAW_HOME points here)
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (ZEDCLAW_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.zedclaw as the "real home" — which is NOT equal
    # to the ZEDCLAW_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    zedclaw_home = tmp_path / "classic"
    zedclaw_home.mkdir()
    monkeypatch.setenv("ZEDCLAW_HOME", str(zedclaw_home))

    _write(zedclaw_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from zedclaw_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: ZEDCLAW_HOME is set to a custom path that is NOT under
    # ~/.zedclaw/profiles/ — get_default_zedclaw_root() returns ZEDCLAW_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from zedclaw_cli.auth import read_credential_pool, write_credential_pool

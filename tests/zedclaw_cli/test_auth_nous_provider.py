...
from zedclaw_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from zedclaw_cli.auth import _resolve_verify
    ...
    def test_missing_ssl_cert_file_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.delenv("ZEDCLAW_CA_BUNDLE", raising=False)
        result = _resolve_verify(auth_state={"tls": {}})
        assert result is True

    def test_missing_zedclaw_ca_bundle_env_falls_back(self, monkeypatch):
        from zedclaw_cli.auth import _resolve_verify

        monkeypatch.setenv("ZEDCLAW_CA_BUNDLE", "/nonexistent/zedclaw-ca.pem")
...
def _setup_nous_auth(
    ...
    *,
    ...
    (zedclaw_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_get_nous_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    ...
    """
    from zedclaw_cli.auth import get_nous_auth_status
...
def test_get_nous_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    ...
    zedclaw_home = tmp_path / "zedclaw"
    _setup_nous_auth(zedclaw_home, access_token="at-123")
    monkeypatch.setenv("ZEDCLAW_HOME", str(zedclaw_home))
    monkeypatch.setattr(
        "zedclaw_cli.auth.resolve_nous_runtime_credentials",
...
def test_get_nous_auth_status_prefers_runtime_auth_store_over_stale_pool(tmp_path, monkeypatch):
    ...
    zedclaw_home = tmp_path / "zedclaw"
    _setup_nous_auth(zedclaw_home, access_token="at-fresh")
    monkeypatch.setenv("ZEDCLAW_HOME", str(zedclaw_home))
...
def test_get_nous_auth_status_reports_revoked_refresh_session(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("zedclaw_cli.auth.resolve_nous_runtime_credentials", _boom)
...
def test_refresh_token_persisted_when_mint_returns_insufficient_credits(tmp_path, monkeypatch):
    zedclaw_home = tmp_path / "zedclaw"
    _setup_nous_auth(zedclaw_home, refresh_token="refresh-old")
    monkeypatch.setenv("ZEDCLAW_HOME", str(zedclaw_home))
    ...
    monkeypatch.setattr("zedclaw_cli.auth._refresh_access_token", _fake_refresh_access_token)
    monkeypatch.setattr("zedclaw_cli.auth._mint_agent_key", _fake_mint_agent_key)
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `zedclaw model` → Nous Portal → Skip (keep current) after
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        auth_path = zedclaw_home / "auth.json"
        auth_path.write_text(json.dumps({
            "version": 1,
            "active_provider": "openrouter",
            "providers": {"openrouter": {"api_key": "sk-or-fake"}},
        }))
        return zedclaw_home, config_path, auth_path
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        ...
        import zedclaw_cli.auth as auth_mod
        import zedclaw_cli.models as models_mod
        import zedclaw_cli.nous_subscription as ns
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        import argparse
        import yaml
        from zedclaw_cli.auth import PROVIDER_REGISTRY, _login_nous

        zedclaw_home, config_path, auth_path = self._setup_home_with_openrouter(
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `zedclaw auth add nous`
    ...
    """
    from zedclaw_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
...
def test_persist_nous_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    ...
    must succeed (not raise "ZedClaw is not logged into Nous Portal").
    ...
    """
    from zedclaw_cli.auth import persist_nous_credentials, resolve_nous_runtime_credentials
    ...
    # doesn't raise "ZedClaw is not logged into Nous Portal".
    def _fake_refresh_access_token(*, client, portal_base_url, client_id, refresh_token):
        return {
...
def test_persist_nous_credentials_embeds_custom_label(tmp_path, monkeypatch):
    ...
    Previously `zedclaw auth add nous --type oauth --label <name>` silently
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    ``error_description`` containing "reuse detected", ZedClaw must surface an
    ...
    re-authenticate" string led users to report this as a ZedClaw persistence
    bug when the true cause is external RT consumption (monitoring scripts,
    custom self-heal hooks).
    """
    from zedclaw_cli.auth import _refresh_access_token
    ...
    assert "zedclaw auth add nous" in message.lower()
...
def shared_store_env(tmp_path, monkeypatch):
    """Redirect ZEDCLAW_SHARED_AUTH_DIR to a tmp_path.
    ...
    monkeypatch.setenv("ZEDCLAW_SHARED_AUTH_DIR", str(shared_dir))
    return shared_dir


def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without ZEDCLAW_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    writing to the user's real ``~/.zedclaw/shared/`` across CI runs.
    """
    from zedclaw_cli.auth import _nous_shared_store_path

    monkeypatch.delenv("ZEDCLAW_SHARED_AUTH_DIR", raising=False)
...
def test_shared_store_honors_env_override(tmp_path, monkeypatch):
    """ZEDCLAW_SHARED_AUTH_DIR must redirect the path."""
    from zedclaw_cli.auth import _nous_shared_store_path, NOUS_SHARED_STORE_FILENAME

    custom_dir = tmp_path / "custom_shared"
    monkeypatch.setenv("ZEDCLAW_SHARED_AUTH_DIR", str(custom_dir))
...
def test_shared_store_read_missing_returns_none(shared_store_env):
    ...
    from zedclaw_cli.auth import _read_shared_nous_state
...
def test_shared_store_read_malformed_returns_none(shared_store_env):
    ...
    from zedclaw_cli.auth import _nous_shared_store_path, _read_shared_nous_state
...
def test_shared_store_write_skips_when_refresh_token_missing(shared_store_env):
    ...
    from zedclaw_cli.auth import _nous_shared_store_path, _write_shared_nous_state
...
def test_persist_nous_credentials_mirrors_to_shared_store(
    ...
    AND the shared store, so a future profile's `zedclaw auth add nous
    ...
    zedclaw_home = tmp_path / "zedclaw"
    zedclaw_home.mkdir(parents=True, exist_ok=True)
    (zedclaw_home / "auth.json").write_text(
...
def test_try_import_shared_returns_none_when_store_missing(shared_store_env):
    ...
    from zedclaw_cli.auth import _try_import_shared_nous_state
...
def test_try_import_shared_returns_none_on_refresh_failure(
    ...
    """
    from zedclaw_cli import auth as auth_mod
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    assert result["client_id"] == "zedclaw-cli"
...
def test_shared_store_survives_across_profile_switch(
    ...
    (different ZEDCLAW_HOME) sees the same shared state and can rehydrate
    ...
    monkeypatch.setenv("ZEDCLAW_HOME", str(profile_a))
    ...
    # Profile B: fresh ZEDCLAW_HOME, no auth yet, but the shared store
    ...
    monkeypatch.setenv("ZEDCLAW_HOME", str(profile_b))

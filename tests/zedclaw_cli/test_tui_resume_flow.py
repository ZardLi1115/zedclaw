...
def main_mod(monkeypatch):
    import zedclaw_cli.main as mod
...
def test_main_top_level_tui_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(sys, "argv", ["zedclaw", "--tui", "--toolsets", "web,terminal"])
    monkeypatch.setitem(
        sys.modules,
        "zedclaw_cli.plugins",
...
def test_main_top_level_oneshot_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys, "argv", ["zedclaw", "-z", "hello", "--toolsets", "web,terminal"]
    )
    monkeypatch.setitem(
        sys.modules,
        "zedclaw_cli.plugins",
...
def test_oneshot_filters_invalid_toolsets_before_redirect(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    from zedclaw_cli.oneshot import _validate_explicit_toolsets
...
def test_oneshot_rejects_disabled_mcp_toolset(monkeypatch, capsys):
    ...
    assert valid is None
    assert error == "zedclaw -z: --toolsets did not contain any valid toolsets.\n"
    err = capsys.readouterr().err
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """zedclaw -z bypasses ZedClawCLI, but recall still needs SessionDB."""
    from zedclaw_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "zedclaw_state", mod("zedclaw_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "zedclaw_cli.config",
        mod("zedclaw_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "zedclaw_cli.models",
        mod("zedclaw_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
        "zedclaw_cli.runtime_provider",
        mod(
            "zedclaw_cli.runtime_provider",
    ...
    monkeypatch.setitem(
        sys.modules,
        "zedclaw_cli.tools_config",
        mod("zedclaw_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["ZEDCLAW_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="nous/zedclaw-test", provider="nous", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["ZEDCLAW_MODEL"] == "nous/zedclaw-test"
    assert env["ZEDCLAW_INFERENCE_MODEL"] == "nous/zedclaw-test"
    assert env["ZEDCLAW_TUI_PROVIDER"] == "nous"
    assert env["ZEDCLAW_INFERENCE_PROVIDER"] == "nous"
    assert env["ZEDCLAW_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["ZEDCLAW_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("zedclaw-tui-active-session-")
...
def test_print_tui_exit_summary_includes_resume_and_token_totals(monkeypatch, capsys):
    import zedclaw_cli.main as main_mod
    ...
    monkeypatch.setitem(
        sys.modules, "zedclaw_state", types.SimpleNamespace(SessionDB=lambda: _FakeDB())
    )
    ...
    assert "zedclaw --tui --resume 20260409_000001_abc123" in out
    assert 'zedclaw --tui -c "demo title"' in out
...
def test_print_tui_exit_summary_prefers_actual_active_session_file(
    ...
    assert "zedclaw --tui --resume actual_session" in out

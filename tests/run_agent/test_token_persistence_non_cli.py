...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    zedclaw_state = ModuleType("zedclaw_state")
    zedclaw_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "zedclaw_state", zedclaw_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "ZedClaw"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "ZedClaw"

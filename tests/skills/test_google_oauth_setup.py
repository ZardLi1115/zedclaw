...
class TestZedClawConstantsFallback:
    """Tests for _zedclaw_home.py fallback when zedclaw_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_zedclaw_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _zedclaw_home.py with zedclaw_constants blocked."""
        monkeypatch.setitem(sys.modules, "zedclaw_constants", None)
        spec = importlib.util.spec_from_file_location("_zedclaw_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_zedclaw_home_env_var(self, monkeypatch, tmp_path):
        """When zedclaw_constants is missing, ZEDCLAW_HOME comes from env var."""
        monkeypatch.setenv("ZEDCLAW_HOME", str(tmp_path / "custom-zedclaw"))
        module = self._load_helper(monkeypatch)
        assert module.get_zedclaw_home() == tmp_path / "custom-zedclaw"

    def test_fallback_defaults_to_dot_zedclaw(self, monkeypatch):
        """When zedclaw_constants is missing and ZEDCLAW_HOME unset, default to ~/.zedclaw."""
        monkeypatch.delenv("ZEDCLAW_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_zedclaw_home() == Path.home() / ".zedclaw"

    def test_fallback_ignores_empty_zedclaw_home(self, monkeypatch):
        """Empty/whitespace ZEDCLAW_HOME is treated as unset."""
        monkeypatch.setenv("ZEDCLAW_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_zedclaw_home() == Path.home() / ".zedclaw"

    def test_fallback_display_zedclaw_home_shortens_path(self, monkeypatch):
        """Fallback display_zedclaw_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("ZEDCLAW_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_zedclaw_home() == "~/.zedclaw"

    def test_fallback_display_zedclaw_home_profile_path(self, monkeypatch):
        """Fallback display_zedclaw_home() handles profile paths under ~/."""
        monkeypatch.setenv("ZEDCLAW_HOME", str(Path.home() / ".zedclaw/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_zedclaw_home() == "~/.zedclaw/profiles/coder"

    def test_fallback_display_zedclaw_home_custom_path(self, monkeypatch):
        """Fallback display_zedclaw_home() returns full path for non-home locations."""
        monkeypatch.setenv("ZEDCLAW_HOME", "/opt/zedclaw-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_zedclaw_home() == "/opt/zedclaw-custom"

    def test_delegates_to_zedclaw_constants_when_available(self):
        """When zedclaw_constants IS importable, _zedclaw_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_zedclaw_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import zedclaw_constants
        assert module.get_zedclaw_home is zedclaw_constants.get_zedclaw_home
        assert module.display_zedclaw_home is zedclaw_constants.display_zedclaw_home

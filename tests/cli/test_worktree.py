...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("zedclaw/zedclaw-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned zedclaw/* and pr-* branches."""

    def test_prunes_orphaned_zedclaw_branch(self, git_repo):
        """zedclaw/zedclaw-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "zedclaw/zedclaw-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "zedclaw/zedclaw-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "zedclaw/zedclaw-deadbeef" in result.stdout
        ...
        assert "zedclaw/zedclaw-deadbeef" in orphaned
        ...
        assert "zedclaw/zedclaw-deadbeef" not in result.stdout

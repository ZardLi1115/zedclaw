...
def _isolate(tmp_path, monkeypatch):
    """Isolate ZEDCLAW_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal ZedClawCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = ZedClawCLI.__new__(ZedClawCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["zedclaw-core"]
        obj.compact = False

...
def _isolate_env(tmp_path, monkeypatch):
    """Isolate ZEDCLAW_HOME for each test.

    The global hermetic fixture already redirects ZEDCLAW_HOME to a tempdir,
    but we want the plugin to work with a predictable subpath. We reset
    ZEDCLAW_HOME here for clarity.
    """
...
def _load_plugin_init():
    ...
    spec = importlib.util.spec_from_file_location(
        "zedclaw_plugins.disk_cleanup",
        plugin_dir / "__init__.py",
        submodule_search_locations=[str(plugin_dir)],
    )
    # Ensure parent namespace package exists for the relative `. import disk_cleanup`
    import types
    if "zedclaw_plugins" not in sys.modules:
        ns = types.ModuleType("zedclaw_plugins")
        ns.__path__ = []
        sys.modules["zedclaw_plugins"] = ns
    mod = importlib.util.module_from_spec(spec)
    mod.__package__ = "zedclaw_plugins.disk_cleanup"
    mod.__path__ = [str(plugin_dir)]
    sys.modules["zedclaw_plugins.disk_cleanup"] = mod
    spec.loader.exec_module(mod)
    return mod
...
class TestIsSafePath:
    def test_accepts_path_under_zedclaw_home(self, _isolate_env):
        dg = _load_lib()
    ...
    def test_rejects_outside_zedclaw_home(self, _isolate_env):
        dg = _load_lib()
        assert dg.is_safe_path(Path("/etc/passwd")) is False

    def test_accepts_tmp_zedclaw_prefix(self, _isolate_env, tmp_path):
        dg = _load_lib()
        assert dg.is_safe_path(Path("/tmp/zedclaw-abc/x.log")) is True
...
class TestBundledDiscovery:
    def _write_enabled_config(self, zedclaw_home, names):
    ...
    def test_disk_cleanup_discovered_but_not_loaded_by_default(self, _isolate_env):
        ...
        from zedclaw_cli import plugins as pmod

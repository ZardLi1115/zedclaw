...
class TestManifest:
    ...
    def test_manifest_fields(self):
        ...
        # Required env vars are the user-facing ZEDCLAW_ prefixed keys.
        assert "ZEDCLAW_LANGFUSE_PUBLIC_KEY" in data["requires_env"]
        assert "ZEDCLAW_LANGFUSE_SECRET_KEY" in data["requires_env"]
...
class TestDiscovery:
    def test_plugin_is_discovered_as_standalone_opt_in(self, tmp_path, monkeypatch):
        ...
        # Isolated ZEDCLAW_HOME so we don't read the developer's config.yaml.
...
class TestRuntimeGate:
    ...
    def test_get_langfuse_returns_none_without_credentials(self, monkeypatch):
        for k in (
            "ZEDCLAW_LANGFUSE_PUBLIC_KEY", "ZEDCLAW_LANGFUSE_SECRET_KEY",
    ...
    def test_get_langfuse_caches_failure_no_config_load(self, monkeypatch):
        ...
        def tracking_get(key, default=None):
            if key.startswith(("ZEDCLAW_LANGFUSE_", "LANGFUSE_")):
                called["n"] += 1
    ...
    def test_get_langfuse_does_not_import_zedclaw_config(self, monkeypatch):
        ...
        # Drop any cached import of zedclaw_cli.config.
        sys.modules.pop("zedclaw_cli.config", None)

        langfuse_plugin = self._fresh_plugin()
        for _ in range(20):
            langfuse_plugin._get_langfuse()

        assert "zedclaw_cli.config" not in sys.modules, (
            "langfuse plugin imported zedclaw_cli.config — regression toward "

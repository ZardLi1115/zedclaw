...
There are two ways to add a platform to the ZedClaw gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.zedclaw/plugins/` (or under `plugins/platforms/`
...
**zero changes to core ZedClaw code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `zedclaw gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `zedclaw_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the ZedClaw core.
...
And add it to the `zedclaw-gateway` composite:

```python
"zedclaw-gateway": {
    "includes": [..., "zedclaw-your-platform"]
}
```
...
## 12. Status Display (`zedclaw_cli/status.py`)
...
## 13. Gateway Setup Wizard (`zedclaw_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ zedclaw_cli/ toolsets.py \
  --include="*.py" -l | sort -u

...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        tokens_file = get_zedclaw_home() / "slack_tokens.json"
        ...
        try:
            ...
            # /model work directly without /zedclaw prefix). A single regex
            ...
            # manifest (see `zedclaw slack manifest`). In Socket Mode, Slack
            ...
            from zedclaw_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/zedclaw$")

            @self._app.command(_slash_pattern)
            async def handle_zedclaw_command(ack, command):
            ...
            for _action_id in (
                "zedclaw_approve_once",
                "zedclaw_approve_session",
                "zedclaw_approve_always",
                "zedclaw_deny",
            ):
            ...
            for _action_id in (
                "zedclaw_confirm_once",
                "zedclaw_confirm_always",
                "zedclaw_confirm_cancel",
            ):
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = f":thread: ZedClaw handoff — *{(name or 'session').strip()[:80]}*"
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own ZedClaw session — matching the per-thread behavior channels already
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "zedclaw_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "zedclaw_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "zedclaw_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "zedclaw_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "zedclaw_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "zedclaw_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "zedclaw_confirm_once": "once",
            "zedclaw_confirm_always": "always",
            "zedclaw_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "zedclaw_approve_once": "once",
            "zedclaw_approve_session": "session",
            "zedclaw_approve_always": "always",
            "zedclaw_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/zedclaw <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/zedclaw
        ...
        if slash_name in {"zedclaw", ""}:
            # Legacy /zedclaw <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from zedclaw_cli.commands import slack_subcommand_map
        ...
        # "Running /cmd…" ack shown by handle_zedclaw_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/zedclaw <question>" must produce public replies so

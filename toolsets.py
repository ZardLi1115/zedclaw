...
_ZEDCLAW_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (ZEDCLAW_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (ZEDCLAW_KANBAN_TASK env "
    ...
    # ==========================================================================
    # Full ZedClaw toolsets (CLI + messaging platforms)
    #
    ...
    "zedclaw-cli": {
        ...
        "tools": _ZEDCLAW_CORE_TOOLS,
        "includes": []
    },

    "zedclaw-cron": {
        # Mirrors zedclaw-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `zedclaw tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant, rl) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as zedclaw-cli; gated by `zedclaw tools`",
        "tools": _ZEDCLAW_CORE_TOOLS,
        "includes": []
    },
    ...
    "zedclaw-email": {
        "description": "Email bot toolset - interact with ZedClaw via email (IMAP/SMTP)",
        "tools": _ZEDCLAW_CORE_TOOLS,
        "includes": []
    },
    ...
    "zedclaw-sms": {
        "description": "SMS bot toolset - interact with ZedClaw via SMS (Twilio)",
        "tools": _ZEDCLAW_CORE_TOOLS,
        "includes": []
    },
    ...
    "zedclaw-gateway": {
        ...
        "tools": [],
        "includes": ["zedclaw-telegram", "zedclaw-discord", "zedclaw-whatsapp", "zedclaw-slack", "zedclaw-signal", "zedclaw-bluebubbles", "zedclaw-homeassistant", "zedclaw-email", "zedclaw-sms", "zedclaw-mattermost", "zedclaw-matrix", "zedclaw-dingtalk", "zedclaw-feishu", "zedclaw-wecom", "zedclaw-wecom-callback", "zedclaw-weixin", "zedclaw-qqbot", "zedclaw-webhook", "zedclaw-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (zedclaw-<name>).
        # Gives them _ZEDCLAW_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("zedclaw-"):
            platform_name = name[len("zedclaw-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_ZEDCLAW_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(

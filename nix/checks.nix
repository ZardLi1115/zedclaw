...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      zedclaw = self'.packages.default;
      zedclawVenv = zedclaw.zedclawVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "zedclaw-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${zedclawVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "zedclaw-cross-eval" { } (
          if failures != [] then
        ...
        package-contents = pkgs.runCommand "zedclaw-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${zedclaw}/bin/zedclaw || (echo "FAIL: zedclaw binary missing"; exit 1)
          test -x ${zedclaw}/bin/zedclaw || (echo "FAIL: zedclaw binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${zedclaw}/bin/zedclaw version 2>&1 | grep -qi "zedclaw" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "zedclaw-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in zedclaw zedclaw zedclaw-acp; do
            test -x ${zedclaw}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "zedclaw-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking zedclaw --help ==="
          ${zedclaw}/bin/zedclaw --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${zedclaw}/bin/zedclaw --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "zedclaw-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${zedclaw}/share/zedclaw/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${zedclaw}/share/zedclaw/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "ZEDCLAW_BUNDLED_SKILLS" ${zedclaw}/bin/zedclaw || \
            (echo "FAIL: ZEDCLAW_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: ZEDCLAW_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "zedclaw-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${zedclaw}/share/zedclaw/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${zedclaw}/share/zedclaw/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "ZEDCLAW_BUNDLED_PLUGINS" ${zedclaw}/bin/zedclaw || \
            (echo "FAIL: ZEDCLAW_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: ZEDCLAW_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-tui = pkgs.runCommand "zedclaw-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${zedclaw}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${zedclaw}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "ZEDCLAW_TUI_DIR" ${zedclaw}/bin/zedclaw || \
            (echo "FAIL: ZEDCLAW_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: ZEDCLAW_TUI_DIR set in wrapper"
        ...
        # Verify ZEDCLAW_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        zedclaw-node = pkgs.runCommand "zedclaw-node-version" { } ''
          set -e
          echo "=== Checking ZEDCLAW_NODE in wrapper ==="
          grep -q "ZEDCLAW_NODE" ${zedclaw}/bin/zedclaw || \
            (echo "FAIL: ZEDCLAW_NODE not set in wrapper"; exit 1)
          echo "PASS: ZEDCLAW_NODE present in wrapper"

          ZEDCLAW_NODE=$(sed -n "s/^export ZEDCLAW_NODE='\(.*\)'/\1/p" ${zedclaw}/bin/zedclaw)
          test -x "$ZEDCLAW_NODE" || (echo "FAIL: ZEDCLAW_NODE=$ZEDCLAW_NODE not executable"; exit 1)
          echo "PASS: ZEDCLAW_NODE executable at $ZEDCLAW_NODE"

          NODE_MAJOR=$("$ZEDCLAW_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All ZEDCLAW_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify ZEDCLAW_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "zedclaw-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(ZEDCLAW_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking ZEDCLAW_MANAGED guards ==="
          check_blocked "config set" ${zedclaw}/bin/zedclaw config set model foo
          check_blocked "config edit" ${zedclaw}/bin/zedclaw config edit
        ...
        extra-python-packages = let
          ...
          zedclawWithExtra = zedclaw.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "zedclaw-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${zedclawWithExtra}/bin/zedclaw || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${zedclawWithExtra}/bin/zedclaw || \
          ...
          if grep -q "PYTHONPATH" ${zedclaw}/bin/zedclaw; then
        ...
        extra-dependency-groups = let
          zedclawWithGroups = zedclaw.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "zedclaw-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${zedclawWithGroups}"
          echo "venv: ${zedclawWithGroups.zedclawVenv}"
        ...
        in pkgs.runCommand "zedclaw-config-roundtrip" {
          ...
          merge_and_load() {
            local zedclaw_home="$1"
            export ZEDCLAW_HOME="$zedclaw_home"
            ${configMergeScript} ${nixSettings} "$zedclaw_home/config.yaml"
            ${zedclawVenv}/bin/python3 -c '
import json, sys

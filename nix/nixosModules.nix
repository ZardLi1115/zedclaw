# nix/nixosModules.nix — NixOS module for zedclaw
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: zedclaw runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $ZEDCLAW_HOME/.env
# and read by zedclaw at startup — no container recreation needed for env changes.
#
# Tool resolution: the zedclaw wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    ...
    zedclaw = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-zedclaw)
    deepConfigType = lib.types.mkOptionType {
      name = "zedclaw-config-attrs";
      description = "ZedClaw YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "zedclaw-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "zedclaw-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'ZEDCLAW_DOC_EOF'\n${value}\nZEDCLAW_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "zedclaw";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/zedclaw";
    ...
    # zedclaw user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "zedclaw-container-entrypoint" ''
      set -eu

      ZEDCLAW_UID="''${ZEDCLAW_UID:?ZEDCLAW_UID must be set}"
      ZEDCLAW_GID="''${ZEDCLAW_GID:?ZEDCLAW_GID must be set}"

      # ── Group: ensure a group with GID=$ZEDCLAW_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$ZEDCLAW_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="zedclaw"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$ZEDCLAW_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$ZEDCLAW_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$ZEDCLAW_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$ZEDCLAW_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="zedclaw"
        TARGET_HOME="/home/zedclaw"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$ZEDCLAW_UID" -g "$ZEDCLAW_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$ZEDCLAW_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$ZEDCLAW_UID:$ZEDCLAW_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure ZEDCLAW_HOME is owned by the target user.
      ...
      if [ -n "''${ZEDCLAW_HOME:-}" ] && [ -d "$ZEDCLAW_HOME" ]; then
        find "$ZEDCLAW_HOME" \! -user "$ZEDCLAW_UID" -exec chown "$ZEDCLAW_UID:$ZEDCLAW_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/zedclaw-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/zedclaw-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/zedclaw ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/zedclaw
        chmod 0440 /etc/sudoers.d/zedclaw
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$ZEDCLAW_UID" --regid="$ZEDCLAW_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $ZEDCLAW_HOME/.env.
    ...
    # Default: /var/lib/zedclaw/workspace → /data/workspace.
  ...
  in {
    options.services.zedclaw = with lib; {
      enable = mkEnableOption "ZedClaw gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = zedclaw;
        description = "The zedclaw package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/zedclaw";
        description = "State directory. Contains .zedclaw/ subdir (ZEDCLAW_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative ZedClaw config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $ZEDCLAW_HOME/.env at activation time.
          ZedClaw reads this file on every startup via load_zedclaw_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $ZEDCLAW_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $ZEDCLAW_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `zedclaw gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the zedclaw user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the zedclaw plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. ZedClaw discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          zedclaw_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as zedclaw (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in zedclaw's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the zedclaw CLI to environment.systemPackages and export
          ZEDCLAW_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.zedclaw symlink to the service
            stateDir. These users are automatically added to the zedclaw group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.zedclaw.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the zedclaw CLI to system PATH and export ZEDCLAW_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.zedclaw/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.ZEDCLAW_HOME = "${cfg.stateDir}/.zedclaw";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.zedclaw.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the zedclaw user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.zedclaw: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed zedclaw
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure zedclaw is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.zedclaw        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.zedclaw/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.zedclaw/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.zedclaw/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.zedclaw/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.zedclaw/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."zedclaw-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.zedclaw
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.zedclaw ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.zedclaw ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.zedclaw -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.zedclaw/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.zedclaw/$_subdir"
            chmod 2770 "${cfg.stateDir}/.zedclaw/$_subdir"
            find "${cfg.stateDir}/.zedclaw/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.zedclaw/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.zedclaw/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.zedclaw/config.yaml
            chmod 0640 ${cfg.stateDir}/.zedclaw/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.zedclaw/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.zedclaw/.managed
          chmod 0644 ${cfg.stateDir}/.zedclaw/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.zedclaw/.container-mode <<'ZEDCLAW_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    zedclaw_bin=${containerDataDir}/current-package/bin/zedclaw
    ZEDCLAW_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.zedclaw/.container-mode
            chmod 0644 ${cfg.stateDir}/.zedclaw/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.zedclaw/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.zedclaw";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.zedclaw" ]; then
                  rm -f "${symlinkPath}"
                  echo "zedclaw: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.zedclaw -> stateDir/.zedclaw for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.zedclaw";
                target = "${cfg.stateDir}/.zedclaw";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "zedclaw: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.zedclaw/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.zedclaw/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.zedclaw/auth.json
              fi
            ''}
          ''}
          ...
          # ZedClaw reads $ZEDCLAW_HOME/.env at startup via load_zedclaw_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.zedclaw/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'ZEDCLAW_NIX_ENV_EOF'
    ${envFileContent}
    ZEDCLAW_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.zedclaw/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.zedclaw/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.zedclaw/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.zedclaw = {
          ...
          environment = {
            HOME = cfg.stateDir;
            ZEDCLAW_HOME = "${cfg.stateDir}/.zedclaw";
            ZEDCLAW_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $ZEDCLAW_HOME/.env by the activation script. load_zedclaw_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/zedclaw"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the zedclaw group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.zedclaw = {
          description = "ZedClaw Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ZEDCLAW_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              ZEDCLAW_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env ZEDCLAW_UID="$ZEDCLAW_UID" \
                --env ZEDCLAW_GID="$ZEDCLAW_GID" \
                --env ZEDCLAW_HOME=${containerDataDir}/.zedclaw \
                --env ZEDCLAW_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/zedclaw gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}

...
_DEFAULT_CONTAINER_TAG = "zedclaw"
...
def _load_supermemory_config(zedclaw_home: str) -> dict:
    config = _default_config()
    config_path = Path(zedclaw_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, zedclaw_home: str) -> None:
    config_path = Path(zedclaw_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `zedclaw memory setup`.
        # All other options are documented for $ZEDCLAW_HOME/supermemory.json
    ...
    def save_config(self, values, zedclaw_home):
        ...
        _save_supermemory_config(sanitized, zedclaw_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from zedclaw_constants import get_zedclaw_home
        self._zedclaw_home = kwargs.get("zedclaw_home") or str(get_zedclaw_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._zedclaw_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "zedclaw", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "zedclaw_memory", "target": target, "type": "explicit_memory"},

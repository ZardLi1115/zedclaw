...
Config file: `$ZEDCLAW_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `zedclaw` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `zedclaw-{identity}` → `zedclaw-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per ZedClaw profile:

```json
{
  "container_tag": "zedclaw-{identity}"
}
```

For a profile named `coder`, this resolves to `zedclaw-coder`. The default profile resolves to `zedclaw-default`. Without `{identity}`, all profiles share the same container.

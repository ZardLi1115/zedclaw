...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/zedclaw>
...
```bash
zedclaw honcho setup    # full interactive wizard (cloud or local)
zedclaw memory setup    # generic picker, also works
```

Or manually:
```bash
zedclaw config set memory.provider honcho
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$ZEDCLAW_HOME/honcho.json` | Profile-local (isolated ZedClaw instances) |
| 2 | `~/.zedclaw/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active ZedClaw profile: `zedclaw` (default) or `zedclaw.<profile>`.
...
| 4 | `per-session` strategy | ZedClaw session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`zedclaw`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`zedclaw`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-zedclaw`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening zedclaw in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — ZedClaw session ID (timestamp + hex). Every `zedclaw` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple ZedClaw profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`zedclaw`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active ZedClaw profile: `zedclaw` (default) or `zedclaw.<profile>` (e.g. `zedclaw -p coder` → host key `zedclaw.coder`).
...
| `ZEDCLAW_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `zedclaw honcho setup` | Full interactive setup wizard |
| `zedclaw honcho status` | Show resolved config for active profile |
| `zedclaw honcho enable` / `disable` | Toggle Honcho for active profile |
| `zedclaw honcho mode <mode>` | Change recall or observation mode |
| `zedclaw honcho peer --user <name>` | Update user peer name |
| `zedclaw honcho peer --ai <name>` | Update AI peer name |
| `zedclaw honcho tokens --context <N>` | Set context token budget |
| `zedclaw honcho tokens --dialectic <N>` | Set dialectic max chars |
| `zedclaw honcho map <name>` | Map current directory to a session name |
| `zedclaw honcho sync` | Create host blocks for all ZedClaw profiles |

...
# ``model_tools`` import time, so ``zedclaw status``, ``zedclaw chat``, etc.
...
# ``zedclaw_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "ZedClaw is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a zedclaw MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${ZEDCLAW_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _zedclaw_home = _get_zedclaw_home()
        except (ModuleNotFoundError, ImportError):
            _zedclaw_home = _Path.home() / ".zedclaw"
        self._thread_count_store = _ThreadCountStore(
            _zedclaw_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "ZedClaw is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("ZEDCLAW_HOME", str(_Path.home() / ".zedclaw"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${ZEDCLAW_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a zedclaw MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'ZedClaw is thinking…' marker message.
        ...
        "ZedClaw is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "ZedClaw is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "ZedClaw is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "ZedClaw is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "ZedClaw is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``zedclaw setup``.

    The setup wizard at ``zedclaw_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.zedclaw/.env`` so the next
    gateway restart picks them up.
    """
    from zedclaw_cli.cli_output import (
        print_info,
        print_success,
        print_warning,
        prompt,
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. zedclaw-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.zedclaw/.env")
    print_info("Restart the gateway: zedclaw gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``zedclaw cron`` running as a
    separate process from ``zedclaw gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the ZedClaw plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'zedclaw[google_chat]'",
        ...
        # the "ZedClaw is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'ZedClaw is thinking…' marker message "

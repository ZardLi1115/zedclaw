...
- Per-user tokens (keyed by sender email):
    ``${ZEDCLAW_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${ZEDCLAW_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${ZEDCLAW_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${ZEDCLAW_HOME}/google_chat_user_oauth_pending.json``
- Shared OAuth client (one per host):
    ``${ZEDCLAW_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's ZEDCLAW_HOME helper so the token follows the user's
# profile (e.g. tests can override via ZEDCLAW_HOME=/tmp/...).
try:
    from zedclaw_constants import display_zedclaw_home, get_zedclaw_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where zedclaw_constants isn't importable
    # (mirrors the same fallback used by the google-workspace skill's
    # _zedclaw_home.py shim).
...
def _zedclaw_home() -> Path:
    """Resolve ZEDCLAW_HOME at call time (NOT module import).

    Tests and ``ZEDCLAW_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.zedclaw/google_chat_user_tokens/`` trivial.
...
def _legacy_token_path() -> Path:
    return _zedclaw_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _zedclaw_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _zedclaw_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _zedclaw_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _zedclaw_home() / "google_chat_user_client_secret.json"
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Install zedclaw[google_chat]."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("  pip install 'zedclaw[google_chat]'")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into ZEDCLAW_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_zedclaw_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_zedclaw_home()}/google_chat_user_token.json"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for ZedClaw (native attachment delivery)"
    )

"""
Microsoft Teams platform adapter for ZedClaw.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "ZedClaw"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        zedclaw_action = data.get("zedclaw_action", "")
        session_key = data.get("session_key", "")

        if not zedclaw_action or not session_key:
        ...
        choice = choice_map.get(zedclaw_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
    ...
    async def send_exec_approval(
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="zedclaw_approve",
                    data={**btn_data_base, "zedclaw_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="zedclaw_approve",
                    data={**btn_data_base, "zedclaw_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="zedclaw_approve",
                    data={**btn_data_base, "zedclaw_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="zedclaw_approve",
                    data={**btn_data_base, "zedclaw_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"ZedClaw\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.zedclaw/.env")

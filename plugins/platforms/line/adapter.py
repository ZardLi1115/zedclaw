"""
LINE Messaging API platform adapter for ZedClaw.
...
to ZedClaw. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``ZEDCLAW_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            zedclaw_home = Path.home().joinpath(".zedclaw").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``zedclaw status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``zedclaw status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``zedclaw setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.zedclaw/.env`` via ``zedclaw_cli.config``.
    """
    print()
    ...
    try:
        from zedclaw_cli.config import get_env_var, set_env_var
    except ImportError:
        print("zedclaw_cli.config not available; set LINE_* vars manually in ~/.zedclaw/.env")
        return

/**
 * ZedClaw Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.zedclaw/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__ZEDCLAW_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__ZEDCLAW_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "zedclaw-kanban-dot-triage",
    todo: "zedclaw-kanban-dot-todo",
    ready: "zedclaw-kanban-dot-ready",
    running: "zedclaw-kanban-dot-running",
    blocked: "zedclaw-kanban-dot-blocked",
    done: "zedclaw-kanban-dot-done",
    archived: "zedclaw-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-zedclaw-task";
  ...
  const DOCS_URL = "https://zedclaw.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://zedclaw.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "zedclaw.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="zedclaw-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "zedclaw-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "zedclaw-kanban-md",
  ...
  // standard `drop` event and our `zedclaw-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("zedclaw-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (col !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("zedclaw-kanban-column--drop");
          if (col) col.classList.add("zedclaw-kanban-column--drop");
          lastTarget = col;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("zedclaw-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          lastTarget.dispatchEvent(new CustomEvent("zedclaw-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        const token = window.__ZEDCLAW_SESSION_TOKEN__ || "";
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "zedclaw-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "zedclaw-kanban-attention",
        "zedclaw-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "zedclaw-kanban-attention-bar" },
        h("span", { className: "zedclaw-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "zedclaw-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "zedclaw-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "zedclaw-kanban-attention-row",
                  "zedclaw-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "zedclaw-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "zedclaw-kanban-attention-row-id" }, task.id),
                h("span", { className: "zedclaw-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "zedclaw-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "zedclaw-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "zedclaw-kanban-diag-action-btn",
      action.suggested ? "zedclaw-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " zedclaw-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".zedclaw-kanban-drawer-comment-row input, .zedclaw-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "zedclaw-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("zedclaw-kanban-diag", sevClass) },
      h("div", { className: "zedclaw-kanban-diag-header" },
        h("span", { className: "zedclaw-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "zedclaw-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "zedclaw-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "zedclaw-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "zedclaw-kanban-diag-data-row" },
                  h("span", { className: "zedclaw-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "zedclaw-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "zedclaw-kanban-diag-data-row" },
                h("span", { className: "zedclaw-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "zedclaw-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "zedclaw-kanban-diag-reassign-row" },
            h("span", { className: "zedclaw-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "zedclaw-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "zedclaw-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "zedclaw-kanban-diag-msg",
              msg.ok ? "zedclaw-kanban-diag-msg--ok" : "zedclaw-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "zedclaw-kanban-section" },
      h("div", { className: "zedclaw-kanban-section-head-row" },
        h("span", { className: "zedclaw-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "zedclaw-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "zedclaw-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "zedclaw-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "zedclaw-kanban-docs-link",
      title: "Open ZedClaw Kanban docs in a new tab",
      "aria-label": "ZedClaw Kanban documentation",
    }, "?");
  }
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "zedclaw-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "zedclaw-kanban-boardswitcher" },
      h("div", { className: "zedclaw-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "zedclaw-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "zedclaw-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "zedclaw-kanban-dialog-title" },
        ...
        h("div", { className: "zedclaw-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned ZedClaw profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "zedclaw-kanban-bulk" },
      h("span", { className: "zedclaw-kanban-bulk-count" },
      ...
      h("div", { className: "zedclaw-kanban-bulk-priority",
      ...
      h("div", { className: "zedclaw-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different ZedClaw profile. Pick a profile (or unassign) and click Apply." },
        h(Select, {
          value: assignee,
      ...
      h("label", { className: "zedclaw-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h("input", {
          type: "checkbox",
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".zedclaw-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "zedclaw-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("zedclaw-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("zedclaw-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "zedclaw-kanban-column",
        dragOver ? "zedclaw-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "zedclaw-kanban-column-header",
                 title: colHelp || "" },
        h("input", {
          type: "checkbox",
          className: "zedclaw-kanban-col-check",
        ...
        h("span", { className: cn("zedclaw-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "zedclaw-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "zedclaw-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "zedclaw-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "zedclaw-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "zedclaw-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "zedclaw-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "zedclaw-kanban-lane" },
                  h("div", { className: "zedclaw-kanban-lane-head" },
                    h("span", { className: "zedclaw-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "zedclaw-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "zedclaw-kanban-card--stale-red";
    if (age >= tier.amber) return "zedclaw-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".zedclaw-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "zedclaw-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "zedclaw-kanban-card",
        props.selected ? "zedclaw-kanban-card--selected" : "",
        props.failed ? "zedclaw-kanban-card--failed" : "",
        props.draggingSource ? "zedclaw-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "zedclaw-kanban-card-content" },
          h("div", { className: "zedclaw-kanban-card-row" },
            h("label", {
              className: "zedclaw-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h("input", {
                type: "checkbox",
                className: "zedclaw-kanban-card-check",
            ...
            h("span", { className: "zedclaw-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or zedclaw kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "zedclaw-kanban-warning-badge",
                    "zedclaw-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
              ...
              : null,
            t.priority > 0
              ? h(Badge, { className: "zedclaw-kanban-priority",
                           title: `Priority ${t.priority}. Higher-priority tasks are claimed first by the dispatcher.` }, `P${t.priority}`)
              : null,
            t.tenant
              ? h(Badge, { variant: "outline", className: "zedclaw-kanban-tag",
                           title: `Tenant: ${t.tenant}. Free-form tag for grouping tasks (customer, project, team).` }, t.tenant)
              : null,
            progress
              ? h("span", {
                  className: cn(
                    "zedclaw-kanban-progress",
                    progress.done === progress.total ? "zedclaw-kanban-progress--full" : "",
                  ),
                  title: `${progress.done} of ${progress.total} child tasks done`,
                }, `${progress.done}/${progress.total}`)
              : null,
          ),
          h("div", { className: "zedclaw-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "zedclaw-kanban-card-row zedclaw-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "zedclaw-kanban-assignee",
                            title: `Assigned to ZedClaw profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "zedclaw-kanban-unassigned",
            ...
            t.comment_count > 0
              ? h("span", { className: "zedclaw-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "zedclaw-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "zedclaw-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "ZedClaw profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "ZedClaw profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "zedclaw-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "zedclaw-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "zedclaw-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "zedclaw-kanban-drawer-close",
        ...
        data ? h("div", { className: "zedclaw-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "zedclaw-kanban-drawer-body" },
      h("div", { className: "zedclaw-kanban-drawer-title" },
        h("span", { className: cn("zedclaw-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "zedclaw-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "zedclaw-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "zedclaw-kanban-section" },
        h("div", { className: "zedclaw-kanban-section-head" }, tx(i18n, "result", "Result")),
        h(MarkdownBlock, { source: t.result, enabled: props.renderMarkdown }),
      ) : null,
      h("div", { className: "zedclaw-kanban-section" },
        h("div", { className: "zedclaw-kanban-section-head" },
          `${tx(i18n, "comments", "Comments")} (${comments.length})`),
        comments.length === 0
          ? h("div", { className: "text-xs text-muted-foreground" },
              tx(i18n, "noComments", "— no comments —"))
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "zedclaw-kanban-comment" },
                h("div", { className: "zedclaw-kanban-comment-head" },
                  h("span", { className: "zedclaw-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "zedclaw-kanban-comment-ago" },
      ...
      h("div", { className: "zedclaw-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            key: e.id,
            className: cn(
              "zedclaw-kanban-event",
              isDiag ? "zedclaw-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "zedclaw-kanban-event-header" },
                  h("span", { className: "zedclaw-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "zedclaw-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "zedclaw-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "zedclaw-kanban-event-header-plain" },
                  h("span", { className: "zedclaw-kanban-event-kind" }, e.kind),
                  h("span", { className: "zedclaw-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "zedclaw-kanban-event-phantom-row" },
                  h("span", { className: "zedclaw-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "zedclaw-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "zedclaw-kanban-event-payload" },
    ...
    return h("div", { className: "zedclaw-kanban-section" },
      h("div", { className: "zedclaw-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              ...
              className: "zedclaw-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `zedclaw-kanban-run--${r.outcome || r.status || "ended"}`
          : "zedclaw-kanban-run--active";
        return h("div", { key: r.id, className: cn("zedclaw-kanban-run", outcomeClass) },
          h("div", { className: "zedclaw-kanban-run-head" },
            h("span", { className: "zedclaw-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "zedclaw-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "zedclaw-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "zedclaw-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "zedclaw-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "zedclaw-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "zedclaw-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "zedclaw-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "zedclaw-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "zedclaw-kanban-pre zedclaw-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "zedclaw-kanban-meta-row" },
      h("span", { className: "zedclaw-kanban-meta-label" }, props.label),
      h("span", { className: "zedclaw-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "zedclaw-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "zedclaw-kanban-meta-row" },
        h("span", { className: "zedclaw-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "zedclaw-kanban-meta-value zedclaw-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "zedclaw-kanban-meta-row" },
        h("span", { className: "zedclaw-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "zedclaw-kanban-meta-value zedclaw-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "zedclaw-kanban-section" },
      h("div", { className: "zedclaw-kanban-section-head-row" },
        h("span", { className: "zedclaw-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "zedclaw-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "zedclaw-kanban-section" },
      h("div", { className: "zedclaw-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "zedclaw-kanban-deps-row" },
        h("span", { className: "zedclaw-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "zedclaw-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "zedclaw-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "zedclaw-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "zedclaw-kanban-dep-chip-x",
      ...
      h("div", { className: "zedclaw-kanban-deps-row" },
        h("span", { className: "zedclaw-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "zedclaw-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "zedclaw-kanban-actions" },
        specifyButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "zedclaw-kanban-msg-ok"
          : "zedclaw-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "zedclaw-kanban-section" },
      ...
      h("div", { className: "zedclaw-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "zedclaw-kanban-home-sub zedclaw-kanban-home-sub--on"
              : "zedclaw-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__ZEDCLAW_PLUGINS__ && typeof window.__ZEDCLAW_PLUGINS__.register === "function") {
    window.__ZEDCLAW_PLUGINS__.register("kanban", KanbanPage);
  }
})();

...
``zedclaw_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
own pages inject it via ``window.__ZEDCLAW_SESSION_TOKEN__`` so logged-in
...
the in-browser PTY bridge in ``zedclaw_cli/web_server.py``.

This means ``zedclaw dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
from zedclaw_cli import kanban_db
...
def _check_ws_token(provided: Optional[str]) -> bool:
    ...
    if not provided:
        return False
    try:
        from zedclaw_cli import web_server as _ws
    except Exception:
...
def _compute_task_diagnostics(
    ...
    Uses ``zedclaw_cli.kanban_diagnostics`` — see that module for the
    rule definitions.
    """
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from zedclaw_cli.kanban_diagnostics import SEVERITY_ORDER
...
def get_board(
    ...
    through to the active board (``ZEDCLAW_KANBAN_BOARD`` env → on-disk
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from zedclaw_cli.kanban import _check_dispatcher_presence
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``zedclaw kanban
...
# spawn failures, stuck-blocked). See zedclaw_cli.kanban_diagnostics for
# the rule engine.
# ---------------------------------------------------------------------------
...
def list_diagnostics(
    ...
    AND for ``zedclaw kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``zedclaw kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``zedclaw kanban specify <task_id>``.
    ...
    prev_env = os.environ.get("ZEDCLAW_KANBAN_BOARD")
    try:
        os.environ["ZEDCLAW_KANBAN_BOARD"] = board or kanban_db.DEFAULT_BOARD
        # Import lazily so a missing auxiliary client at import time
        # doesn't break plugin load.
        from zedclaw_cli import kanban_specify  # noqa: WPS433 (intentional)
        ...
        if prev_env is None:
            ...
            os.environ["ZEDCLAW_KANBAN_BOARD"] = prev_env
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``zedclaw kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.zedclaw/config.yaml.
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.zedclaw/profiles/*`` on disk and every
...
async def stream_events(ws: WebSocket):
    ...
    # authenticates in zedclaw_cli/web_server.py.

"""disk_cleanup — ephemeral file cleanup for ZedClaw.
...
Rules:
  ...
  - empty dirs    → always delete (under ZEDCLAW_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly ZEDCLAW_HOME and /tmp/zedclaw-*
Never touches: ~/.zedclaw/logs/ or any system directory.
"""
...
try:
    ...
    def get_zedclaw_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("ZEDCLAW_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".zedclaw").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$ZEDCLAW_HOME/logs/``."""
    return get_zedclaw_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$ZEDCLAW_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under ZEDCLAW_HOME or ``/tmp/zedclaw-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    zedclaw_home = get_zedclaw_home()
    try:
        path.resolve().relative_to(zedclaw_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/zedclaw-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("zedclaw-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside ZEDCLAW_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under ZEDCLAW_HOME (but leave ZEDCLAW_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "zedclaw", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(zedclaw_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == zedclaw_home:
                continue
            try:
                rel_parts = dirpath.relative_to(zedclaw_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    zedclaw_home = get_zedclaw_home()
    try:
        rel = path.resolve().relative_to(zedclaw_home)
        ...
        # Path isn't under ZEDCLAW_HOME (e.g. /tmp/zedclaw-*) — fall through.
        pass

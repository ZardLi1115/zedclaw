...
Auto-tracks and cleans up ephemeral files created during ZedClaw
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$ZEDCLAW_HOME` and `/tmp/zedclaw-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ZEDCLAW_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `ZEDCLAW_HOME` or `/tmp/zedclaw-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$ZEDCLAW_HOME/disk-cleanup/` is itself excluded
- `$ZEDCLAW_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,

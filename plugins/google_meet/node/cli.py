"""`zedclaw meet node ...` subcommand tree.

Wired into the existing ``zedclaw meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``zedclaw meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="zedclaw-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``zedclaw meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             zedclaw meet node approve <name> ws://<host>:{args.port} {token}")
        try:

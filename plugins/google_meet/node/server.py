...
Launched by ``zedclaw meet node run``.

Token handling
--------------
On first boot we mint 32 hex chars of entropy and persist them at
``$ZEDCLAW_HOME/workspace/meetings/node_token.json``. Subsequent boots
...
via ``zedclaw meet node approve <name> <url> <token>``.
...
def _default_token_path() -> Path:
    return Path(get_zedclaw_home()) / "workspace" / "meetings" / "node_token.json"
...
class NodeServer:
    ...
    def __init__(
        ...
        display_name: str = "zedclaw-meet-node",
    ) -> None:
        self.host = host
        self.port = port

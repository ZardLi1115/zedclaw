...
session-scoped state file under ``$ZEDCLAW_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $ZEDCLAW_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_zedclaw_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this zedclaw install, leave it first —
    ...
    env = os.environ.copy()
    env["ZEDCLAW_MEET_URL"] = url
    env["ZEDCLAW_MEET_OUT_DIR"] = str(out)
    env["ZEDCLAW_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["ZEDCLAW_MEET_HEADED"] = "1"
    if auth_state:
        env["ZEDCLAW_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["ZEDCLAW_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if ZEDCLAW_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["ZEDCLAW_MEET_MODE"] = mode
    if realtime_model:
        env["ZEDCLAW_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["ZEDCLAW_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["ZEDCLAW_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["ZEDCLAW_MEET_REALTIME_KEY"] = realtime_api_key

...
Let the zedclaw agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where zedclaw runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `zedclaw meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `zedclaw meet` CLI |
...
| `cli.py` | `zedclaw meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$ZEDCLAW_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `zedclaw meet node {run,list,approve,remove,status,ping}` |
...
zedclaw meet install                                      # pip + Chromium
zedclaw meet setup                                        # preflight
zedclaw meet auth                                         # optional
zedclaw meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
zedclaw meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.zedclaw/.env
zedclaw meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
zedclaw meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
zedclaw meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.zedclaw/.env
zedclaw meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, zedclaw will **not** switch your system audio input automatically — the
...
zedclaw plugins enable google_meet
zedclaw meet node run --display-name my-mac --host 0.0.0.0 --port 18789

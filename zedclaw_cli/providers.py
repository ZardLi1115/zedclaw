"""
Single source of truth for provider identity in ZedClaw.
...
2. **ZedClaw overlays** — transport type, auth patterns, aggregator flags,
...
# -- ZedClaw overlay ----------------------------------------------------------
# ZedClaw-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class ZedClawOverlay:
    """ZedClaw-specific provider metadata layered on top of models.dev."""
...
ZEDCLAW_OVERLAYS: Dict[str, ZedClawOverlay] = {
    "openrouter": ZedClawOverlay(
    ...
    "openai-codex": ZedClawOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "qwen-oauth": ZedClawOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="ZEDCLAW_QWEN_BASE_URL",
    ),
    "google-gemini-cli": ZedClawOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "lmstudio": ZedClawOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": ZedClawOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": ZedClawOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": ZedClawOverlay(
    ...
    "kimi-for-coding": ZedClawOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": ZedClawOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": ZedClawOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": ZedClawOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": ZedClawOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": ZedClawOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": ZedClawOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": ZedClawOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "vercel": ZedClawOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ),
    "opencode": ZedClawOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": ZedClawOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nvidia": ZedClawOverlay(
        transport="openai_chat",
        base_url_override="https://integrate.api.nvidia.com/v1",
        base_url_env_var="NVIDIA_BASE_URL",
    ),
    "xiaomi": ZedClawOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "tencent-tokenhub": ZedClawOverlay(
        transport="openai_chat",
        base_url_env_var="TOKENHUB_BASE_URL",
    ),
    "arcee": ZedClawOverlay(
        transport="openai_chat",
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "zedclaw", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. ZedClaw overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + ZedClaw overlay
    ...
    overlay = ZEDCLAW_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + zedclaw extra
    ...
    if overlay is not None:
        # ZedClaw-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in ZEDCLAW_OVERLAYS
    if provider == "bedrock":

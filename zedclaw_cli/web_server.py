"""
ZedClaw — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m zedclaw_cli.main web          # Start on http://127.0.0.1:9119
    python -m zedclaw_cli.main web --port 8080
"""
...
try:
    ...
    # running `zedclaw dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["ZEDCLAW_WEB_DIST"]) if "ZEDCLAW_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="ZedClaw", version=__version__)
...
_SESSION_HEADER_NAME = "X-ZedClaw-Session-Token"

# In-browser Chat tab (/chat, /api/pty, …).  Off unless ``zedclaw dashboard --tui``
# or ZEDCLAW_DASHBOARD_TUI=1.  Set from :func:`start_server`.
...
# streamed to a per-action log file under ``~/.zedclaw/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_zedclaw_home() / "logs"

# Short ``name`` (from the URL) → absolute log file path.
_ACTION_LOG_FILES: Dict[str, str] = {
    "gateway-restart": "gateway-restart.log",
    "zedclaw-update": "zedclaw-update.log",
}
...
def _spawn_zedclaw_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``zedclaw <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``zedclaw_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "zedclaw_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "ZEDCLAW_NONINTERACTIVE": "1"},
    }
...
async def restart_gateway():
    """Kick off a ``zedclaw gateway restart`` in the background."""
    try:
        proc = _spawn_zedclaw_action(["gateway", "restart"], "gateway-restart")
...
@app.post("/api/zedclaw/update")
async def update_zedclaw():
    """Kick off ``zedclaw update`` in the background."""
    try:
        proc = _spawn_zedclaw_action(["update"], "zedclaw-update")
    except Exception as exc:
        _log.exception("Failed to spawn zedclaw update")
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ZedClaw supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in zedclaw_cli/config.py — listed here for deterministic ordering in the UI.
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.zedclaw/config.yaml`` — applies to **new** sessions only.
...
# the canonical ``zedclaw auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    ZedClaw resolves Anthropic creds in this order at runtime:
    1. ``~/.zedclaw/.anthropic_oauth.json`` — ZedClaw-managed PKCE flow
    ...
    try:
        from agent.anthropic_adapter import (
            read_zedclaw_oauth_credentials,
            read_claude_code_credentials,
            _ZEDCLAW_OAUTH_FILE,
        )
    except ImportError:
        read_claude_code_credentials = None  # type: ignore
        read_zedclaw_oauth_credentials = None  # type: ignore
        _ZEDCLAW_OAUTH_FILE = None  # type: ignore

    zedclaw_creds = None
    if read_zedclaw_oauth_credentials:
        try:
            zedclaw_creds = read_zedclaw_oauth_credentials()
        except Exception:
            zedclaw_creds = None
    if zedclaw_creds and zedclaw_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "zedclaw_pkce",
            "source_label": f"ZedClaw PKCE ({_ZEDCLAW_OAUTH_FILE})",
            "token_preview": _truncate_token(zedclaw_creds.get("accessToken")),
            "expires_at": zedclaw_creds.get("expiresAt"),
            "has_refresh_token": bool(zedclaw_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into ZedClaw even
    when they also have a separate ZedClaw-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "zedclaw auth add anthropic",
    ...
    },
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "zedclaw auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "zedclaw auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "zedclaw auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "zedclaw auth add minimax-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from zedclaw_cli import auth as hauth
        if provider_id == "nous":
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("zedclaw_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same ZedClaw-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from agent.anthropic_adapter import _ZEDCLAW_OAUTH_FILE
            if _ZEDCLAW_OAUTH_FILE.exists():
                _ZEDCLAW_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from zedclaw_cli.auth import clear_provider_auth
...
#          → persists to ~/.zedclaw/.anthropic_oauth.json AND credential pool
...
# Guarded so zedclaw web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both ZedClaw file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``zedclaw auth add anthropic``.
    """
    from agent.anthropic_adapter import _ZEDCLAW_OAUTH_FILE
    payload = {
    ...
    _ZEDCLAW_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    _ZEDCLAW_OAUTH_FILE.write_text(json.dumps(payload, indent=2), encoding="utf-8")
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "zedclaw-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    """
    if provider_id == "nous":
        from zedclaw_cli.auth import _request_device_code, PROVIDER_REGISTRY
        import httpx
        pconfig = PROVIDER_REGISTRY["nous"]
        portal_base_url = (
            os.getenv("ZEDCLAW_PORTAL_BASE_URL")
...
def _nous_poller(session_id: str) -> None:
    ...
    from zedclaw_cli.auth import _poll_for_token, refresh_nous_oauth_from_state
    ...
    try:
        ...
        from zedclaw_cli.auth import persist_nous_credentials
...
def _minimax_poller(session_id: str) -> None:
    ...
    ``zedclaw auth add minimax-oauth``.
    """
    from zedclaw_cli.auth import (
        _minimax_poll_token,
...
async def get_logs(
    ...
    from zedclaw_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_zedclaw_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from zedclaw_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_zedclaw_home()
...
def _resolve_profile_dir(name: str) -> Path:
    ...
    from zedclaw_cli import profiles as profiles_mod
    try:
...
def _profile_setup_command(name: str) -> str:
    ...
    return "zedclaw setup" if name == "default" else f"{name} setup"
...
async def get_skills():
    ...
    from zedclaw_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from zedclaw_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def get_toolsets():
    from zedclaw_cli.tools_config import (
...
#
# The endpoint spawns the same ``zedclaw --tui`` binary the CLI uses, behind
...
try:
    from zedclaw_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _resolve_chat_argv(
    ...
    Default: whatever ``zedclaw --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``ZEDCLAW_TUI_RESUME`` env var —
    matching what ``zedclaw_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    `sidecar_url` (when set) is forwarded as ``ZEDCLAW_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from zedclaw_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("ZEDCLAW_TUI_DISABLE_MOUSE", "1")

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["ZEDCLAW_TUI_RESUME"] = resume

    if sidecar_url:
        env["ZEDCLAW_TUI_SIDECAR_URL"] = sidecar_url
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall ZedClaw inside WSL2 to use the dashboard's /chat "
...
# ZEDCLAW_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/zedclaw"`` (no trailing slash) or ``""`` when
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/zedclaw/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /zedclaw`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__ZEDCLAW_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/zedclaw``)
        or empty string when served at root.
        """
        html = _index_path.read_text()
        chat_js = "true" if _DASHBOARD_EMBEDDED_CHAT_ENABLED else "false"
        token_script = (
            f'<script>window.__ZEDCLAW_SESSION_TOKEN__="{_SESSION_TOKEN}";'
            f"window.__ZEDCLAW_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
            f'window.__ZEDCLAW_BASE_PATH__="{prefix}";</script>'
        )
        if prefix:
    ...
    # under ``/zedclaw`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the ZedClaw backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "ZedClaw Teal",         "description": "Classic dark teal — the canonical ZedClaw look"},
    {"name": "default-large", "label": "ZedClaw Teal (Large)", "description": "ZedClaw Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.zedclaw/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.zedclaw/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_zedclaw_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.zedclaw/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as zedclaw_cli.plugins):
    1. User plugins:    ~/.zedclaw/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.zedclaw/plugins/  (only if ZEDCLAW_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from zedclaw_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_zedclaw_home() / "plugins", "user"),
        (bundled_root / "memory", "bundled"),
        (bundled_root, "bundled"),
    ]
    if os.environ.get("ZEDCLAW_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".zedclaw" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from zedclaw_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_zedclaw_home() / "plugins").resolve()
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"zedclaw auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from zedclaw_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from zedclaw_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from zedclaw_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from zedclaw_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"zedclaw_dashboard_plugin_{plugin['name']}"
...
def start_server(
    ...
    *,
    ...
    print(f"  ZedClaw Web UI → http://{host}:{port}")

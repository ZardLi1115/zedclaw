"""Shell completion script generation for zedclaw CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_zedclaw_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# ZedClaw bash completion
# Add to ~/.bashrc:
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef zedclaw
# ZedClaw zsh completion
# Add to ~/.zshrc:
...
_zedclaw() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_zedclaw_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'zedclaw command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# ZedClaw fish completion",
        "# Add to your config:",
        "#   zedclaw completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __zedclaw_profiles",
        "    echo default",
        "    if test -d $HOME/.zedclaw/profiles",
        "        ls $HOME/.zedclaw/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        "# Disable file completion by default",
        "complete -c zedclaw -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c zedclaw -f -s p -l profile"
        " -d 'Profile name' -xa '(__zedclaw_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__zedclaw_profiles)' -d 'Profile name'"
                )

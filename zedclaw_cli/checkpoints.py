"""`zedclaw checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.zedclaw/checkpoints/``.  Actions:

    zedclaw checkpoints               # same as `status`
    zedclaw checkpoints status        # total size, project count, breakdown
    zedclaw checkpoints list          # per-project checkpoint counts + workdir
    zedclaw checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    zedclaw checkpoints clear [-f]    # nuke the entire base (asks first)
    zedclaw checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    zedclaw checkpoints
    zedclaw checkpoints prune --retention-days 3 --max-size-mb 200
    zedclaw checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: zedclaw checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``zedclaw checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `zedclaw checkpoints` → status

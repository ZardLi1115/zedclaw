...
on Japanese locales, etc.).  ZedClaw's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``zedclaw_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. ZedClaw is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``ZEDCLAW_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("ZEDCLAW_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the ZedClaw-managed Git install directories so
    ...
    # already-running shells don't see the change — which means zedclaw
    ...
    # install without ZedClaw-managed PortableGit).
    ...
    # stdin is re-configured for completeness; ZedClaw's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and ZedClaw's
    ``zedclaw config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching ZedClaw (User env var in Windows Settings, or
    export in a PowerShell profile) and ZedClaw picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known ZedClaw-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\zedclaw\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``zedclaw`` from right after install) retain their old PATH.

    Any subprocess ZedClaw spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known ZedClaw-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "zedclaw", "git", "cmd"),
        os.path.join(local_appdata, "zedclaw", "git", "bin"),
        os.path.join(local_appdata, "zedclaw", "git", "usr", "bin"),
        # ZedClaw venv Scripts directory — host of the zedclaw.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes zedclaw, but harmless to include.
        os.path.join(local_appdata, "zedclaw", "zedclaw", "venv", "Scripts"),

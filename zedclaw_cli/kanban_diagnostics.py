...
class DiagnosticAction:
    ...
    * ``cli_hint`` — print/copy a shell command (e.g.
      ``zedclaw -p <profile> auth``). No HTTP side effect.
...
def _rule_repeated_failures(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    if most_recent_outcome == "spawn_failed" and assignee and assignee != "default":
        ...
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label=f"Verify profile: zedclaw -p {assignee} doctor",
            payload={"command": f"zedclaw -p {assignee} doctor"},
            suggested=True,
        ))
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label=f"Fix profile auth: zedclaw -p {assignee} auth",
            payload={"command": f"zedclaw -p {assignee} auth"},
        ))
        ...
        if task_id:
            actions.append(DiagnosticAction(
                kind="cli_hint",
                label=f"Check logs: zedclaw kanban log {task_id}",
                payload={"command": f"zedclaw kanban log {task_id}"},
                suggested=True,
            ))
...
def _rule_stranded_in_ready(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    * Dispatcher is misconfigured (wrong board, wrong ZEDCLAW_HOME).
    ...
    works for ZedClaw profiles, registered lanes, external workers, and
    ...
    actions = [
        ...
        DiagnosticAction(
            ...
            payload={"command": "zedclaw kanban diagnostics"},
        ),
    ]

"""Profile distributions — shareable, packaged ZedClaw profiles via git.

A distribution is a ZedClaw profile published as a git repository (or
...
* ``zedclaw profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``zedclaw skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``zedclaw profile``, not a parallel tree):

    zedclaw profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "zedclaw_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".zedclaw_history",
    # User data
    ...
    "backups", "cache",
    # Infrastructure
    "zedclaw", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            zedclaw_requires=str(data.get("zedclaw_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.zedclaw_requires:
            out["zedclaw_requires"] = self.zedclaw_requires
        if self.author:
...
def check_zedclaw_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires ZedClaw {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``zedclaw profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a ZedClaw profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a ZedClaw distribution."
        )

    # Version check up-front so we fail fast
    check_zedclaw_requires(manifest.zedclaw_requires, zedclaw_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.zedclaw).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="zedclaw_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `zedclaw profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`zedclaw profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="zedclaw_dist_update_") as tmp:
        plan = plan_install(

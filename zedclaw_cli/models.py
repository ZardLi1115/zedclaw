...
Add, remove, or reorder entries here — both `zedclaw setup` and
`zedclaw` provider-selection will pick up the change automatically.
"""
...
_ZEDCLAW_USER_AGENT = f"zedclaw-cli/{_ZEDCLAW_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `zedclaw model`
    flow without maintaining a separate static list.
    """
    from zedclaw_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $ZEDCLAW_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $ZEDCLAW_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. ZedClaw refreshes the cache from
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older ZedClaw that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older ZedClaw that doesn't ship
...
def check_nous_free_tier() -> bool:
    ...
    try:
        from zedclaw_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
# this list:  zedclaw model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `zedclaw model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `zedclaw model` TUI
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``zedclaw model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``zedclaw auth add openai-codex`` without
    ``zedclaw model``).
    """
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    zedclaw is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from zedclaw_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — zedclaw
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from zedclaw_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from zedclaw_constants import AI_GATEWAY_BASE_URL
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return zedclaw-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; zedclaw's picker expects
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``zedclaw model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from zedclaw_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from zedclaw_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to ZedClaw' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``zedclaw_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether ZedClaw should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``zedclaw auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.zedclaw/.env``.
    ...
    """
    try:
        from zedclaw_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        from zedclaw_cli.auth import read_credential_pool
        from zedclaw_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a ZedClaw release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a ZedClaw release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        try:
            from zedclaw_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _ZEDCLAW_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: ZedClaw' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _ZEDCLAW_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    return get_zedclaw_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"ZedClaw will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so ZedClaw cannot verify the model name."

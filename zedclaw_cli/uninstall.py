...
- Keep data: Remove code but keep ~/.zedclaw/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove ZedClaw PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing zedclaw or zedclaw PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# ZedClaw" comment and following line
                if '# ZedClaw' in line or '# zedclaw' in line:
                    skip_next = True
                    continue
                if skip_next and ('zedclaw' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing zedclaw
                if 'zedclaw' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the zedclaw wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "zedclaw",
        Path("/usr/local/bin/zedclaw"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains zedclaw_cli reference)
                content = wrapper.read_text()
                if 'zedclaw_cli' in content or 'zedclaw' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``zedclaw gateway run`` processes
    ...
    try:
        from zedclaw_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from zedclaw_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``ZEDCLAW_HOME`` and ``ZEDCLAW_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\zedclaw\git\cmd``, ``%LOCALAPPDATA%\zedclaw\git\bin``,
#      ``%LOCALAPPDATA%\zedclaw\git\usr\bin``, ``%LOCALAPPDATA%\zedclaw\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\zedclaw\git\`` and Node to
#      ``%LOCALAPPDATA%\zedclaw\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``zedclaw dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\zedclaw\gateway-service\`` and sometimes
...
def _zedclaw_path_markers(zedclaw_home: Path) -> list[str]:
    """Path-entry substrings that identify ZedClaw-owned User-PATH entries."""
    root = str(zedclaw_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare zedclaw install dir.
    markers = [root + "\\zedclaw", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if ZEDCLAW_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "zedclaw".  We don't want to catch
    # unrelated entries like "czedclaw-foo" or "ephermeral", so we look for
    # backslash-zedclaw as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(zedclaw_home: Path) -> list[str]:
    """Strip ZedClaw-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _zedclaw_path_markers(zedclaw_home)
            kept: list[str] = []
...
def remove_zedclaw_env_vars_windows() -> list[str]:
    """Delete ZEDCLAW_HOME and ZEDCLAW_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("ZEDCLAW_HOME", "ZEDCLAW_GIT_BASH_PATH"):
                try:
...
def _is_default_zedclaw_home(zedclaw_home: Path) -> bool:
    """Return True when ``zedclaw_home`` points at the default (non-profile) root."""
    try:
        from zedclaw_constants import get_default_zedclaw_root
        return zedclaw_home.resolve() == get_default_zedclaw_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its ZEDCLAW_HOME directory.

    We shell out to ``zedclaw -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current ZEDCLAW_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m zedclaw_cli.main` so we don't depend on a `zedclaw`
    #    wrapper that may be half-removed mid-uninstall.
    zedclaw_invocation = [_sys.executable, "-m", "zedclaw_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                zedclaw_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's ZEDCLAW_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.zedclaw/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.zedclaw/ for future reinstall
    """
    project_root = get_project_root()
    zedclaw_home = get_zedclaw_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie ZEDCLAW_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_zedclaw_home(zedclaw_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ ZedClaw Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {zedclaw_home / 'config.yaml'}")
    print(f"  Secrets: {zedclaw_home / '.env'}")
    print(f"  Data:    {zedclaw_home / 'cron/'}, {zedclaw_home / 'sessions/'}, {zedclaw_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their ZEDCLAW_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL ZedClaw data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the ZedClaw code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in zedclaw_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\zedclaw\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(zedclaw_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No ZedClaw-owned PATH entries in User environment")

        log_info("Removing ZEDCLAW_HOME / ZEDCLAW_GIT_BASH_PATH User env vars...")
        removed_env = remove_zedclaw_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No ZedClaw-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing zedclaw command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.zedclaw/, just remove the zedclaw subdir
            if zedclaw_home in project_root.parents or project_root.parent == zedclaw_home:
    ...
    #     under ZEDCLAW_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(zedclaw_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(zedclaw_home)
    ...
    # 5. Optionally remove ~/.zedclaw/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile ZEDCLAW_HOME dirs live under
        ...
        try:
            if zedclaw_home.exists():
                shutil.rmtree(zedclaw_home)
                log_success(f"Removed {zedclaw_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {zedclaw_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {zedclaw_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {zedclaw_home}/")
        print()
        print("To reinstall later with your existing settings:")
        if _is_windows():
            print(color("  irm https://raw.githubusercontent.com/ZedClaw/zedclaw/main/scripts/install.ps1 | iex", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/ZedClaw/zedclaw/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using ZedClaw! ⚕")
    print()

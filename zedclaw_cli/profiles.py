"""
Profile management for multiple isolated ZedClaw instances.

Each profile is a fully independent ZEDCLAW_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.zedclaw/profiles/<name>/`` by default.

The "default" profile is ``~/.zedclaw`` itself — backward compatible,
zero migration needed.

Usage::

    zedclaw profile create coder          # fresh profile + bundled skills
    zedclaw profile create coder --clone  # also copy config, .env, SOUL.md, skills
    zedclaw profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    zedclaw -p coder chat                 # or via flag
    zedclaw profile use coder             # set as sticky default
    zedclaw profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See zedclaw_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# Marker file written by `zedclaw profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `zedclaw
...
# `zedclaw skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         ZedClaw infrastructure directories that only the default profile
         (``~/.zedclaw``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_zedclaw_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.zedclaw) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "zedclaw",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "zedclaw", "default", "test", "tmp", "root", "sudo",
})

# ZedClaw subcommands that cannot be used as profile names/aliases
_ZEDCLAW_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the zedclaw root, NOT to the current ZEDCLAW_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where ZEDCLAW_HOME points outside
    ``~/.zedclaw``, profiles live under ``ZEDCLAW_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_zedclaw_home() / "profiles"


def _get_default_zedclaw_home() -> Path:
    """Return the default (pre-profile) ZEDCLAW_HOME path.

    In standard deployments this is ``~/.zedclaw``.
    In Docker/custom deployments where ZEDCLAW_HOME is outside ``~/.zedclaw``
    (e.g. ``/opt/data``), returns ZEDCLAW_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_zedclaw_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the ZedClaw installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its ZEDCLAW_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_zedclaw_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, zedclaw subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _ZEDCLAW_SUBCOMMANDS:
        return f"'{canon}' conflicts with a zedclaw subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``zedclaw profile list`` for an unrelated profile.
    """
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_zedclaw_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``zedclaw update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.zedclaw)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.zedclaw/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `zedclaw update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`zedclaw profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `zedclaw update`.\n",
                encoding="utf-8",
            )
        except OSError:
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches ZEDCLAW_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``zedclaw profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ZEDCLAW_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.zedclaw).\n"
            "To remove everything, use: zedclaw uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set ZEDCLAW_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("ZEDCLAW_HOME")
    try:
        os.environ["ZEDCLAW_HOME"] = str(profile_dir)
        from zedclaw_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["ZEDCLAW_HOME"] = old_home
        elif "ZEDCLAW_HOME" in os.environ:
            del os.environ["ZEDCLAW_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.zedclaw/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: zedclaw profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from ZEDCLAW_HOME.

    Returns ``"default"`` if ZEDCLAW_HOME is not set or points to ``~/.zedclaw``.
    Returns the profile name if ZEDCLAW_HOME points into ``~/.zedclaw/profiles/<name>``.
    Returns ``"custom"`` if ZEDCLAW_HOME is set to an unrecognized path.
    """
    from zedclaw_constants import get_zedclaw_home
    zedclaw_home = get_zedclaw_home()
    resolved = zedclaw_home.resolve()

    default_resolved = _get_default_zedclaw_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.zedclaw itself — its parent is ~/ and its
        # directory name is ".zedclaw", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: zedclaw profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.zedclaw itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.zedclaw). "
            "Specify a different name: zedclaw profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="zedclaw_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"zedclaw.{old_name}"
    new_host = f"zedclaw.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_zedclaw_home() / "honcho.json",
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for zedclaw profile names."""
    return '''# ZedClaw profile completion
# Add to ~/.bashrc: eval "$(zedclaw completion bash)"

_zedclaw_profiles() {
    local profiles_dir="$HOME/.zedclaw/profiles"
...
_zedclaw_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_zedclaw_profiles)" -- "$cur"))
        return
    fi
...
complete -F _zedclaw_completion zedclaw
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for zedclaw profile names."""
    return '''#compdef zedclaw
# ZedClaw profile completion
# Add to ~/.zshrc: eval "$(zedclaw completion zsh)"

_zedclaw() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.zedclaw/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.zedclaw/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a ZEDCLAW_HOME path string.

    Called early in the CLI entry point, before any zedclaw modules
    are imported, to set the ZEDCLAW_HOME environment variable.
    """

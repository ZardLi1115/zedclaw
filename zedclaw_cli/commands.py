"""Slash command definitions and autocomplete for the ZedClaw CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of ZedClaw config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal ZedClaw works on across turns until achieved", "Session",
               args_hint="[text | pause | resume | clear | status]"),
    CommandDef("status", "Show session info", "Session"),
    CommandDef("osspr", "Show autonomous OSS PR Agent runtime status", "Info",
               gateway_only=True),
    CommandDef("humanreview", "Show real OSS PR Agent human review items", "Info",
               gateway_only=True, aliases=("hr",)),
    CommandDef("language", "Set OSS PR Agent status language", "Info",
               gateway_only=True, args_hint="[zh|en]"),
    CommandDef("method", "Set OSS PR Agent issue search theme", "Info",
               gateway_only=True, args_hint="<theme|all>"),
    CommandDef("prefer", "Analyze a preferred GitHub repo for one-shot OSS PR plans", "Info",
               gateway_only=True, args_hint="<github-url|approve <id>|reject <id>|list>"),
    CommandDef("oss-tool", "Show reusable tools created by the OSS PR Agent", "Info",
               gateway_only=True, aliases=("osstool",)),
    ...
    CommandDef("busy", "Control what Enter does while ZedClaw is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.zedclaw/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update ZedClaw to the latest version", "Info",
               gateway_only=True),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`zedclaw_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/zedclaw`` subcommand mapping, and
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``zedclaw skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``zedclaw skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``zedclaw skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/zedclaw <verb>`` subcommand.
    ...
    can still reach them via ``/zedclaw <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/zedclaw`` is always reserved as the first entry so the
    legacy ``/zedclaw <subcommand>`` form keeps working for anything that
    ...
    # Reserve /zedclaw as the catch-all top-level command.
    entries.append(("zedclaw", "Talk to ZedClaw or run a subcommand", "[subcommand] [args]"))
    seen.add("zedclaw")
...
def slack_app_manifest(request_url: str = "https://zedclaw.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /zedclaw handler.

    Maps both canonical names and aliases so /zedclaw bg do stuff works
    the same as /zedclaw background do stuff.

    Plugin-registered slash commands are included so ``/zedclaw <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from zedclaw_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]

"""zedclaw hooks — inspect and manage shell-script hooks.

Usage::

    zedclaw hooks list
    zedclaw hooks test <event> [--for-tool X] [--payload-file F]
    zedclaw hooks revoke <command>
    zedclaw hooks doctor

Consent records live under ``~/.zedclaw/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.zedclaw/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``zedclaw hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: zedclaw hooks {list|test|revoke|doctor}")
        print("Run 'zedclaw hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.zedclaw/config.yaml.")
        print("See `zedclaw hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `zedclaw hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `zedclaw hooks test` and `zedclaw hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (ZedClaw wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `zedclaw hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `zedclaw hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `zedclaw hooks doctor`.")

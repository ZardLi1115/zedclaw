"""
zedclaw fallback — manage the fallback provider chain.

Fallback providers are tried in order when the primary model fails with
rate-limit, overload, or connection errors. See:
https://zedclaw.nousresearch.com/docs/user-guide/features/fallback-providers

Subcommands:
  zedclaw fallback [list]   Show the current fallback chain (default when no subcommand)
  zedclaw fallback add      Pick provider + model via the same picker as `zedclaw model`,
                           then append the selection to the chain
  zedclaw fallback remove   Pick an entry to delete from the chain
  zedclaw fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.zedclaw/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from zedclaw_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `zedclaw model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  zedclaw fallback add")
        print()
        return
    ...
    print("  Docs: https://zedclaw.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `zedclaw model`, then append the selection to the chain."""
    from zedclaw_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `zedclaw model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `zedclaw fallback list` to view, or `zedclaw fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``zedclaw fallback [subcommand]``."""

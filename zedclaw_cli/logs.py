"""``zedclaw logs`` — view and filter ZedClaw log files.
...
Usage examples::
    ...
    zedclaw logs                    # last 50 lines of agent.log
    zedclaw logs -f                 # follow agent.log in real time
    zedclaw logs errors             # last 50 lines of errors.log
    zedclaw logs gateway -n 100    # last 100 lines of gateway.log
    zedclaw logs --level WARNING    # only WARNING+ lines
    zedclaw logs --session abc123   # filter by session ID substring
    zedclaw logs --component tools  # only tool-related lines
    zedclaw logs --since 1h         # lines from the last hour
    zedclaw logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_zedclaw_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when ZedClaw runs — try 'zedclaw chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_zedclaw_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_zedclaw_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_zedclaw_home()}/logs/")
        return

    print(f"Log files in {display_zedclaw_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'zedclaw chat' to generate logs)")

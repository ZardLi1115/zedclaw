...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m zedclaw_cli.main gateway run --replace`` invocation) and
...
_TASK_NAME_DEFAULT = "ZedClaw_Gateway"
_TASK_DESCRIPTION = "ZedClaw Gateway - Messaging Platform Integration"
...
def get_task_name() -> str:
    ...
    Default profile: ``ZedClaw_Gateway``
    Named profile X: ``ZedClaw_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during zedclaw_cli boot.
    from zedclaw_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\zedclaw\\gateway-service\\<task_name>.cmd``
    (or ``<ZEDCLAW_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    ZedClaw installs stay self-contained).
    """
    _assert_windows()
    from zedclaw_cli.config import get_zedclaw_home

    script_dir = Path(get_zedclaw_home()) / "gateway-service"
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports ZEDCLAW_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``python -m zedclaw_cli.main [--profile X] gateway run --replace``
    ...
    lines.append(f'set "ZEDCLAW_HOME={zedclaw_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "ZEDCLAW_GATEWAY_DETACHED=1"')
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports zedclaw_constants-based logic during startup.
    venv_dir = str(Path(python_path).resolve().parent.parent)
    lines.append(f'set "VIRTUAL_ENV={venv_dir}"')

    prog_args = [python_path, "-m", "zedclaw_cli.main"]
    if profile_arg:
...
def _write_task_script() -> Path:
    ...
    zedclaw_home = str(Path(get_zedclaw_home()).resolve())
    profile_arg = _profile_arg(zedclaw_home)

    content = _build_gateway_cmd_script(python_path, working_dir, zedclaw_home, profile_arg)
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "zedclaw_cli.main"]
    if profile_arg:
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m zedclaw_cli.main gateway run --replace``
    ...
    log_dir = Path(get_zedclaw_home()) / "logs"
...
def _wait_for_gateway_ready(timeout_s: float = 6.0, interval_s: float = 0.4) -> list[int]:
    ...
    """
    from zedclaw_cli.gateway import find_gateway_pids
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from zedclaw_cli.config import get_zedclaw_home
        print(f"    type {Path(get_zedclaw_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_zedclaw_home()).resolve()}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from zedclaw_cli.config import get_zedclaw_home

    zedclaw_home = Path(get_zedclaw_home()).resolve()
    print()
    print("Next steps:")
    print("  zedclaw gateway status                      # Check status")
    print(f"  type {zedclaw_home}\\logs\\gateway.log       # View logs")
...
def stop() -> None:
    ...
    _assert_windows()
    from zedclaw_cli.gateway import kill_gateway_processes

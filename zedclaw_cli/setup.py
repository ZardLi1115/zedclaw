"""
Interactive setup wizard for ZedClaw.
...
from zedclaw_cli.nous_subscription import get_nous_subscription_features
...
_DOCS_BASE = "https://zedclaw.nousresearch.com/docs"
...
# display_zedclaw_home imported lazily at call sites (stale-module safety during zedclaw update)
...
from zedclaw_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print_info("Configure ZedClaw using environment variables or config commands:")
    print_info("  zedclaw config set model.provider custom")
    print_info("  zedclaw config set model.base_url http://localhost:8080/v1")
    print_info("  zedclaw config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'zedclaw setup' in an interactive terminal to use the full wizard.")
    print()
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from zedclaw_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from zedclaw_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'zedclaw setup')")


def _print_setup_summary(config: dict, zedclaw_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'zedclaw setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from zedclaw_cli.plugins import _ensure_plugins_discovered
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'zedclaw setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'zedclaw setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'zedclaw setup terminal'"))
    ...
    # Spotify (OAuth via zedclaw auth spotify — check auth.json, not env vars)
    try:
        from zedclaw_cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'zedclaw setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {zedclaw_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('zedclaw setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('zedclaw setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('zedclaw setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('zedclaw setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('zedclaw setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('zedclaw config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('zedclaw config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('zedclaw config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('zedclaw', Colors.GREEN)}              Start chatting")
    print(f"   {color('zedclaw gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('zedclaw doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `zedclaw tools` and `zedclaw setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``zedclaw model``)
    ...
    provider added to ``zedclaw model`` is automatically available here.
    ...
    # Delegate to the shared zedclaw model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from zedclaw_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: zedclaw model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from zedclaw_cli.auth_commands import auth_add_command
            ...
            print_info(
                "ZedClaw can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one ZedClaw can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'zedclaw setup' or configure AUXILIARY_VISION_* settings")
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.zedclaw/.env."
            )
        ...
        if not existing:
            ...
            if api_key:
                ...
                print_warning(
                    ...
                    f"zedclaw setup model or {_dhh()}/.env to use xAI TTS. "
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'zedclaw setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where ZedClaw runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("Requires the optional SDK: pip install 'zedclaw[vercel]'")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'zedclaw[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into ZEDCLAW_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("ZEDCLAW_MAX_ITERATIONS")
    ...
    print_info("  Run `zedclaw setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # ZEDCLAW_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where ZedClaw delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `zedclaw update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://zedclaw.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under ZEDCLAW_HOME, and print
    ...
    """
    try:
        from zedclaw_cli.slack_cli import _build_full_manifest
        from zedclaw_constants import get_zedclaw_home

        manifest = _build_full_manifest(
            bot_name="ZedClaw",
            bot_description="Your ZedClaw on Slack",
        )
        target = Path(get_zedclaw_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `zedclaw slack manifest --write` anytime to refresh after "
            "ZedClaw adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where ZedClaw delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where ZedClaw delivers cron job results and notifications.")
    print_info("   To get a channel ID: click channel name → View Info → copy the ID")
    print_info("   You can also set this later by typing /set-home in a Mattermost channel.")
    home_channel = prompt("Home channel ID (leave empty to set later with /set-home)")
    if home_channel:
        save_env_value("MATTERMOST_HOME_CHANNEL", home_channel)
    print_info("   Open config in your editor:  zedclaw config edit")
...
def _setup_bluebubbles():
    ...
    print_info("Connects ZedClaw to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from zedclaw_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://zedclaw.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://zedclaw.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  zedclaw config edit")
    print_info("   Open config in your editor:  zedclaw config edit")
...
def setup_gateway(config: dict):
    ...
    from zedclaw_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with ZedClaw from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'zedclaw setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     zedclaw config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_zedclaw_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  zedclaw gateway")
        else:
            from zedclaw_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   zedclaw gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   zedclaw gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `zedclaw setup tools` and `zedclaw tools` use the same flow:
...
# =============================================================================
# Section 6: OSS PR Agent Configuration
# =============================================================================


def _split_osspr_focus_terms(value: str) -> list[str]:
    """Split user-entered OSS PR focus text into compact search terms."""
    terms = []
...
def _osspr_queries_from_terms(terms: list[str]) -> tuple[list[str], list[str]]:
    queries: list[str] = []
    fallback: list[str] = []
    for term in terms:
        queries.extend([
...
def _configure_osspr_gmail_intake() -> None:
    ...
    current_host = get_env_value("EMAIL_IMAP_HOST") or "imap.gmail.com"
...
def setup_oss_pr_agent(config: dict):
    """Configure the autonomous OSS PR Agent runtime."""
    print_header("OSS PR Agent")
    print_info("Configure the gateway-hosted runtime that searches GitHub issues and calls Codex CLI to open PRs.")
    print_info("Feishu notifications use the existing ZedClaw gateway configuration.")
    print()

    agent_cfg = config.setdefault("oss_pr_agent", {})

    agent_cfg["enabled"] = prompt_yes_no(
        "Enable OSS PR Agent runtime?", bool(agent_cfg.get("enabled", True))
    )

    current_lang = str(agent_cfg.get("language") or "en").strip().lower()
    lang_default = 0 if current_lang in {"zh", "cn", "chinese", "中文"} else 1
    lang_idx = prompt_choice(
        "OSS PR Agent language:",
    ...
    if focus_idx == 0:
        ...
        if terms:
            queries, fallback_queries = _osspr_queries_from_terms(terms)
            agent_cfg["focus"] = ", ".join(terms)
    ...
    if codex_model:
        agent_cfg["codex_model"] = codex_model
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``zedclaw_cli.auth`` — lists every supported
    ...
    """
    try:
        from zedclaw_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from zedclaw_cli.config
    ...
    if section_key == "model":
        ...
        from zedclaw_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_zedclaw.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_zedclaw migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_zedclaw", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and ZedClaw.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure ZedClaw to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point ZedClaw at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point ZedClaw at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point ZedClaw at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point ZedClaw at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to ZedClaw equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing ZedClaw config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in ZedClaw.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ ZedClaw's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(zedclaw_home: Path) -> bool:
    ...
    print_info("ZedClaw can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: zedclaw claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=zedclaw_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: zedclaw claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing ZedClaw configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing ZedClaw config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in ZedClaw (use zedclaw claw migrate --overwrite to force).")
    if skipped:
...
SETUP_SECTIONS = [
    ...
    ("osspr", "OSS PR Agent", setup_oss_pr_agent),
]
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      zedclaw setup           — full or quick (auto-detected)
      zedclaw setup model     — just model/provider
      zedclaw setup tts       — just text-to-speech
      zedclaw setup terminal  — just terminal backend
      zedclaw setup gateway   — just messaging platforms
      zedclaw setup tools     — just tool configuration
      zedclaw setup agent     — just agent settings
      zedclaw setup osspr     — just OSS PR Agent settings
    """
    ...
    print(
        color(
            "│  Let's configure your ZedClaw installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, zedclaw_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have ZedClaw configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'zedclaw setup model|terminal|")
        print_info("     gateway|tools|agent|osspr', or fill only missing items with --quick.")
        ...
        migration_ran = _offer_openclaw_migration(zedclaw_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice("How would you like to set up ZedClaw?", [
            "Quick setup — provider, model & messaging (recommended)",
            "Full setup — configure everything",
        ], 0)

        if setup_mode == 0:
            _run_first_time_quick_setup(config, zedclaw_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {zedclaw_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'zedclaw config edit'")
    ...
    # Section 6: OSS PR Agent
    if not (migration_ran and _skip_configured_section(config, "oss_pr_agent", "OSS PR Agent")):
        setup_oss_pr_agent(config)
    ...
    _print_setup_summary(config, zedclaw_home)
...
def _offer_launch_chat():
    ...
    print()
    if not prompt_yes_no("Launch zedclaw chat now?", True):
        return
...
def _run_first_time_quick_setup(config: dict, zedclaw_home, is_existing: bool):
    ...
    the user can customize later via ``zedclaw setup <section>``.
    """
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'zedclaw setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    zedclaw setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  zedclaw setup gateway")
    print()
...
def _run_quick_setup(config: dict, zedclaw_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'zedclaw setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect ZedClaw to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'zedclaw setup gateway'.")

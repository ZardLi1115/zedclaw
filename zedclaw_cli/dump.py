"""
Dump command for zedclaw CLI.

Outputs a compact, plain-text summary of the user's ZedClaw setup
...
from zedclaw_cli.config import get_zedclaw_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``zedclaw dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from zedclaw_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(zedclaw_home: Path) -> int:
...
def _cron_summary(zedclaw_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = zedclaw_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"zedclaw_home:      {display_zedclaw_home()}")
    ...
    toolsets = config.get("toolsets", ["zedclaw-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(zedclaw_home)}")
    lines.append(f"  skills:             {_count_skills(zedclaw_home)}")

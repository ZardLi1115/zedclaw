"""
Backup and import commands for zedclaw CLI.

`zedclaw backup` creates a zip archive of the entire ~/.zedclaw/ directory
(excluding the zedclaw repo and transient files).

`zedclaw import` restores from a backup zip, overlaying onto the current
ZEDCLAW_HOME root.
"""
...
from zedclaw_constants import get_default_zedclaw_root, get_zedclaw_home, display_zedclaw_home
...
_EXCLUDED_DIRS = {
    "zedclaw",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to zedclaw root) should be skipped."""
...
def run_backup(args) -> None:
    """Create a zip backup of the ZedClaw home directory."""
    zedclaw_root = get_default_zedclaw_root()

    if not zedclaw_root.is_dir():
        print(f"Error: ZedClaw home directory not found at {zedclaw_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"zedclaw-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"zedclaw-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_zedclaw_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(zedclaw_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(zedclaw_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(zedclaw_root)

            if _should_exclude(rel):
                continue

            # Skip the output zip itself if it happens to be inside zedclaw root
            try:
    ...
    print(f"\nRestore with: zedclaw import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a ZedClaw backup.
    ...
    # Look for telltale files that a zedclaw home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a ZedClaw backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.zedclaw/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a zedclaw dir name
        if prefix in {".zedclaw", "zedclaw"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_zedclaw_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (zedclaw_root / "config.yaml").exists()
        has_env = (zedclaw_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has ZedClaw configuration.")
        ...
        for member in members:
            ...
            target = zedclaw_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(zedclaw_root.resolve())
            except ValueError:
        ...
        profiles_dir = zedclaw_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # zedclaw_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: zedclaw profile list  (after installing zedclaw)")

        # Guidance
        print()
        if not (zedclaw_root / "zedclaw").is_dir():
            print("Note: The zedclaw codebase was not included in the backup.")
            print("  If this is a fresh install, run: zedclaw update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  zedclaw -p {pname} gateway install")

        print("Done. Your ZedClaw configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and zedclaw backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to ZEDCLAW_HOME).
...
# — `zedclaw update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(zedclaw_home: Optional[Path] = None) -> Path:
    home = zedclaw_home or get_zedclaw_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    zedclaw_home: Optional[Path] = None,
) -> Optional[str]:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(zedclaw_home)
    if not root.exists():
        return []
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(zedclaw_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for zedclaw backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_zedclaw_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, zedclaw_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``zedclaw_root`` to ``out_path``.
...
def _pre_update_backup_dir(zedclaw_home: Optional[Path] = None) -> Path:
    home = zedclaw_home or get_zedclaw_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of ZEDCLAW_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<ZEDCLAW_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``zedclaw update``) should continue even if the backup fails.
    """
    zedclaw_root = zedclaw_home or get_default_zedclaw_root()
    if not zedclaw_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(zedclaw_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, zedclaw_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `zedclaw claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of ZEDCLAW_HOME under ``backups/`` before a
    ``zedclaw claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``zedclaw import <archive>``.  Writes to
    ``<ZEDCLAW_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `zedclaw import` and the

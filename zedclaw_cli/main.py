#!/usr/bin/env python3
"""
ZedClaw CLI - Main entry point.

Usage:
    zedclaw                     # Interactive chat (default)
    zedclaw chat                # Interactive chat
    zedclaw gateway             # Run gateway in foreground
    zedclaw gateway start       # Start gateway as service
    zedclaw gateway stop        # Stop gateway service
    zedclaw gateway status      # Show gateway status
    zedclaw gateway install     # Install gateway service
    zedclaw gateway uninstall   # Uninstall gateway service
    zedclaw setup               # Interactive setup wizard
    zedclaw logout              # Clear stored authentication
    zedclaw status              # Show status of all components
    zedclaw cron                # Manage cron jobs
    zedclaw cron list           # List cron jobs
    zedclaw cron status         # Check if cron scheduler is running
    zedclaw doctor              # Check configuration and dependencies
    zedclaw honcho setup                    # Configure Honcho AI memory integration
    zedclaw honcho status                   # Show Honcho config and connection status
    zedclaw honcho sessions                 # List directory → session name mappings
    zedclaw honcho map <name>               # Map current directory to a session name
    zedclaw honcho peer                     # Show peer names and dialectic settings
    zedclaw honcho peer --user NAME         # Set user peer name
    zedclaw honcho peer --ai NAME           # Set AI peer name
    zedclaw honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    zedclaw honcho mode                     # Show current memory mode
    zedclaw honcho mode [hybrid|honcho|local]  # Set memory mode
    zedclaw honcho tokens                   # Show token budget settings
    zedclaw honcho tokens --context N       # Set session.context() token cap
    zedclaw honcho tokens --dialectic N     # Set dialectic result char cap
    zedclaw honcho identity                 # Show AI peer identity representation
    zedclaw honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    zedclaw honcho migrate                  # Step-by-step migration guide: OpenClaw native → ZedClaw + Honcho
    zedclaw version             Show version
    zedclaw update              Update to latest version
    zedclaw uninstall           Uninstall ZedClaw
    zedclaw acp                 Run as an ACP server for editor integration
    zedclaw sessions browse     Interactive session picker with search

    zedclaw claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: zedclaw_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``zedclaw_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``zedclaw update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``zedclaw_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, zedclaw crashes on import and the user can't run
# ``zedclaw update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import zedclaw_bootstrap  # noqa: F401
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to ZEDCLAW_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (zedclaw tools, zedclaw setup, zedclaw model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'zedclaw {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any zedclaw module import.
#
# Many modules cache ZEDCLAW_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("ZEDCLAW_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.zedclaw/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set ZEDCLAW_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors zedclaw_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If ZEDCLAW_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.zedclaw/profiles/coder or
    # /opt/data/profiles/coder).  If ZEDCLAW_HOME points to the zedclaw root
    # instead (e.g. systemd hardcodes ZEDCLAW_HOME=/root/.zedclaw), we must
    # still read active_profile — the user may have switched profiles via
    # `zedclaw profile use` and the gateway should honour that choice.
    # See issue #22502.
    zedclaw_home_env = os.environ.get("ZEDCLAW_HOME", "")
    if profile_name is None and zedclaw_home_env:
        if Path(zedclaw_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the zedclaw root
    if profile_name is None:
        try:
            from zedclaw_constants import get_default_zedclaw_root

            active_path = get_default_zedclaw_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set ZEDCLAW_HOME
    if profile_name is not None:
        try:
            from zedclaw_cli.profiles import resolve_profile_env

            zedclaw_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent zedclaw from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["ZEDCLAW_HOME"] = zedclaw_home
...
# Load .env from ~/.zedclaw/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from zedclaw_cli.config import get_zedclaw_home
from zedclaw_cli.env_loader import load_zedclaw_dotenv

load_zedclaw_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → ZEDCLAW_REDACT_SECRETS env
# var BEFORE zedclaw_logging imports agent.redact (which snapshots the flag at
# module-import time). Without this, config.yaml's toggle is ignored because
# the setup_logging() call below imports agent.redact, which reads the env var
# exactly once. Env var in .env still wins — this is config.yaml fallback only.
try:
    if "ZEDCLAW_REDACT_SECRETS" not in os.environ:
        import yaml as _yaml_early

        _cfg_path = get_zedclaw_home() / "config.yaml"
        if _cfg_path.exists():
            ...
            if isinstance(_early_sec_cfg, dict):
                ...
                if _early_redact is not None:
                    os.environ["ZEDCLAW_REDACT_SECRETS"] = str(_early_redact).lower()
            del _early_sec_cfg
        del _cfg_path
except Exception:
    pass  # best-effort — redaction stays at default (enabled) on config errors

# Initialize centralized file logging early — all `zedclaw` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from zedclaw_logging import setup_logging as _setup_logging
...
try:
    from zedclaw_cli.config import load_config as _load_config_early
    from zedclaw_constants import apply_ipv4_preference as _apply_ipv4
...
from zedclaw_cli import __version__, __release_date__
from zedclaw_constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from zedclaw_cli.config import get_env_path, get_zedclaw_home, load_config
    from zedclaw_cli.auth import get_auth_status

    # Determine whether ZedClaw itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from zedclaw_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_zedclaw_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from zedclaw_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_zedclaw_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if ZedClaw has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants ZedClaw to use their tokens.
    if _has_zedclaw_config:
        try:
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    db = None
    try:
        from zedclaw_state import SessionDB
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, zedclaw_bin
        cli_args: the original CLI arguments (everything after 'zedclaw')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    zedclaw_bin = container_info["zedclaw_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo zedclaw {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo zedclaw {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  zedclaw --tui --resume {target}")
    if title:
        print(f'  zedclaw --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @zedclaw/ink is missing or node_modules is behind package-lock.json.
    ...
    ink = root / "node_modules" / "@zedclaw" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
def _ensure_tui_node() -> None:
    ...
    ``ZEDCLAW_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("ZEDCLAW_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    zedclaw_home = os.environ.get("ZEDCLAW_HOME") or str(Path.home() / ".zedclaw")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "ZEDCLAW_HOME": zedclaw_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(zedclaw_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (ZEDCLAW_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("ZEDCLAW_NODE")
    ...
    ext_dir = os.environ.get("ZEDCLAW_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with ZEDCLAW_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset ZEDCLAW_TUI_DIR (e.g. `unset ZEDCLAW_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if _tui_need_npm_install(tui_dir):
        npm = _node_bin("npm")
        if not os.environ.get("ZEDCLAW_QUIET"):
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from zedclaw_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="zedclaw-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["ZEDCLAW_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["ZEDCLAW_PYTHON_SRC_ROOT"] = os.environ.get(
        "ZEDCLAW_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("ZEDCLAW_PYTHON", sys.executable)
    env.setdefault("ZEDCLAW_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["ZEDCLAW_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["ZEDCLAW_MODEL"] = model
        env["ZEDCLAW_INFERENCE_MODEL"] = model
    if provider:
        env["ZEDCLAW_TUI_PROVIDER"] = provider
        env["ZEDCLAW_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["ZEDCLAW_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["ZEDCLAW_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["ZEDCLAW_TUI_SKILLS"] = value
    if query:
        env["ZEDCLAW_TUI_QUERY"] = query
    if image:
        env["ZEDCLAW_TUI_IMAGE"] = image
    if checkpoints:
        env["ZEDCLAW_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["ZEDCLAW_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["ZEDCLAW_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["ZEDCLAW_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["ZEDCLAW_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["ZEDCLAW_ACCEPT_HOOKS"] = "1"
    ...
    if resume_session_id:
        env["ZEDCLAW_TUI_RESUME"] = resume_session_id
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``ZEDCLAW_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``zedclaw kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``zedclaw kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("ZEDCLAW_KANBAN_BOARD"):
        return
    try:
        from zedclaw_cli.kanban_db import get_current_board

        os.environ["ZEDCLAW_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("ZEDCLAW_TUI") == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'zedclaw sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like ZedClaw isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  zedclaw setup")
        print()

        from zedclaw_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'zedclaw setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from zedclaw_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["ZEDCLAW_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.zedclaw/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["ZEDCLAW_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["ZEDCLAW_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["ZEDCLAW_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from zedclaw_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    from zedclaw_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with ZedClaw?")
        print()
    ...
    session_dir = get_zedclaw_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: zedclaw gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  zedclaw gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ ZedClaw'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  zedclaw gateway")
        ...
        print()
        print("  Or install as a service: zedclaw gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'zedclaw whatsapp' to try again.")
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``zedclaw model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from zedclaw_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    ...
    from zedclaw_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("ZEDCLAW_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'zedclaw model' for "
                "available providers, or run 'zedclaw doctor' to diagnose config "
                "issues."
            )
    ...
    from zedclaw_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from zedclaw_cli.config import read_raw_config
    ...
    # a leftover OPENAI_BASE_URL in ~/.zedclaw/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.zedclaw/.env if the active provider is not 'custom'.
    ...
    """
    from zedclaw_cli.config import get_env_value, save_env_value, load_config
...
#
# ZedClaw uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `zedclaw model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `zedclaw model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  ZedClaw only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``zedclaw model`` flow, then route aux tasks to them here.
    """
    from zedclaw_cli.config import load_config
    from zedclaw_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from zedclaw_cli.auth import _prompt_model_selection
    from zedclaw_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from zedclaw_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from zedclaw_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from zedclaw_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from zedclaw_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    from zedclaw_cli.models import (
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from zedclaw_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from zedclaw_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `zedclaw auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from zedclaw_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from zedclaw_cli.models import fetch_api_models
...
def _model_flow_minimax_oauth(config, current_model="", args=None):
    ...
    from zedclaw_cli.models import _PROVIDER_MODELS
...
def _model_flow_custom(config):
    ...
    """
    from zedclaw_cli.auth import _save_model_choice, deactivate_provider
    from zedclaw_cli.config import get_env_value, load_config, save_config
    ...
    from zedclaw_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `zedclaw model` to set a model.")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from zedclaw_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from zedclaw_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from zedclaw_cli import azure_detect
    import getpass
    ...
    print("Anthropic-style API endpoints.  ZedClaw will probe your")
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from zedclaw_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from zedclaw_cli.config import save_env_value, load_config, save_config
    from zedclaw_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from zedclaw_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from zedclaw_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates ZedClaw turns to `copilot --acp`.")
    print("  ZedClaw currently starts its own ACP subprocess for each request.")
    print("  ZedClaw uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set ZEDCLAW_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``zedclaw setup`` / ``zedclaw model``.
    ...
    recover from a malformed paste without editing ``~/.zedclaw/.env`` by hand.
    ...
    from zedclaw_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from zedclaw_cli.config import save_env_value
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `zedclaw setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so zedclaw knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   ZedClaw typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with ZedClaw, enable billing on your "
    ...
    if provider_id == "lmstudio":
        from zedclaw_cli.auth import AuthError
        from zedclaw_cli.models import fetch_lmstudio_models
        ...
        from zedclaw_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from zedclaw_constants import display_zedclaw_home as _dhh_fn

            print(
                f"    ZedClaw will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from zedclaw_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'zedclaw model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate ZedClaw CLI with a provider."""
    from zedclaw_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from zedclaw_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from zedclaw_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from zedclaw_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from zedclaw_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from zedclaw_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``zedclaw slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: zedclaw slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `zedclaw slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from zedclaw_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from zedclaw_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from zedclaw_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from zedclaw_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from zedclaw_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from zedclaw_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from zedclaw_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up ZedClaw home directory to a zip file."""
    if getattr(args, "quick", False):
        from zedclaw_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from zedclaw_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a ZedClaw backup from a zip file."""
    from zedclaw_cli.backup import run_import
...
def cmd_version(args):
    """Show version."""
    print(f"ZedClaw v{__version__} ({__release_date__})")
    ...
    try:
        from zedclaw_cli.banner import check_for_updates
        from zedclaw_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    ...
    from zedclaw_cli.uninstall import run_uninstall
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``zedclaw update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``zedclaw_cli/web_dist/`` (per vite.config.ts
    outDir: "../zedclaw_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "zedclaw_cli" / "web_dist"
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``zedclaw update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``zedclaw web``).
    ...
    if r1.returncode != 0:
        print(
            ...
            + ("" if fatal else " (zedclaw web will not be available)")
        )
        if fatal:
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``zedclaw dashboard`` processes other than ourselves.

    ``zedclaw dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``zedclaw update`` replaces files on disk, the running
    ...
    patterns = [
        "zedclaw dashboard",
        "zedclaw_cli.main dashboard",
        "zedclaw_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "zedclaw.*dashboard"`) keeps us consistent
            # with `zedclaw_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `zedclaw update`.
    ...
    print("  Preview now:  zedclaw curator run --dry-run")
    print("  Pause it:     zedclaw curator pause")
    print(
        "  Docs:         https://zedclaw.zedclaw.local/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``zedclaw update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``zedclaw update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: zedclaw curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``zedclaw dashboard`` processes.

    Called at the end of ``zedclaw update`` (default ``reason``) and also
    from ``zedclaw dashboard --stop`` (which overrides ``reason``).  The
    ...
    if killed:
        ...
        print("    zedclaw dashboard --port <port>")
...
def _update_via_zip(args):
    """Update ZedClaw by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/ZedClaw/zedclaw/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="zedclaw-update-")
        zip_path = os.path.join(tmp_dir, f"zedclaw-{branch}.zip")
        ...
        # GitHub ZIPs extract to zedclaw-<branch>/
        extracted = os.path.join(tmp_dir, f"zedclaw-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "zedclaw-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if ZedClaw behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes zedclaw completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but ZedClaw couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but ZedClaw couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if ZedClaw behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `zedclaw update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/ZedClaw/zedclaw.git",
    "git@github.com:ZedClaw/zedclaw.git",
    "https://github.com/ZedClaw/zedclaw",
    "git@github.com:ZedClaw/zedclaw",
}
OFFICIAL_REPO_URL = "https://github.com/ZedClaw/zedclaw.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_zedclaw_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_zedclaw_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official ZedClaw repository.")
        print("  This means you may miss updates from ZedClaw/zedclaw.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/ZedClaw/zedclaw.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/ZedClaw/zedclaw.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``zedclaw update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from zedclaw_constants import get_default_zedclaw_root

    default_home = get_default_zedclaw_root()
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``zedclaw update`` is still progressing even if pip/uv itself is silent.
    """
...
def _zedclaw_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "zedclaw.exe",
        scripts_dir / "zedclaw-gateway.exe",
    ]


def _quarantine_running_zedclaw_exe(scripts_dir: Path) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``zedclaw.exe``.
    ...
    when ``zedclaw update`` runs, ``zedclaw.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``zedclaw.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next zedclaw invocation by ``_cleanup_quarantined_exes``.
    ...
    for shim in _zedclaw_exe_shims(scripts_dir):
        if not shim.exists():
            continue
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_zedclaw_exe`` if uv didn't write replacements."""
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``zedclaw.exe.old.*`` left by prior updates.

    Called early on every zedclaw invocation. The .old files are unlocked once
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``zedclaw.exe`` / ``zedclaw-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_zedclaw_exe`` for the rationale.
    """
    ...
    def _install(args: list[str]) -> None:
        ...
        if scripts_dir is not None:
            moved = _quarantine_running_zedclaw_exe(scripts_dir)
        try:
...
class _UpdateOutputStream:
    """Stream wrapper used during ``zedclaw update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.zedclaw/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``zedclaw update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``zedclaw update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.zedclaw/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``zedclaw update --gateway``) the update is already
    ...
    try:
        ...
        # zedclaw_cli.config.get_zedclaw_home to simulate setup failure.
        from zedclaw_cli.config import get_zedclaw_home as _get_zedclaw_home

        logs_dir = _get_zedclaw_home() / "logs"
...
def _cmd_update_check():
    """Implement ``zedclaw update --check``: fetch and report without installing."""
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``zedclaw update`` without requiring a reinstall.  The
    ...
    ``zedclaw`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/zedclaw.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v zedclaw'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/zedclaw, code at /usr/local/lib/zedclaw).
    fhs_link = Path("/usr/local/bin/zedclaw")
    ...
    path_comment = (
        "# ZedClaw — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of ZEDCLAW_HOME before running the update.
    ...
    ZEDCLAW_HOME directories.  The ``--backup`` flag on ``zedclaw update``
    ...
    try:
        from zedclaw_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_zedclaw_home so the user sees ~/.zedclaw/...
    try:
        from zedclaw_constants import get_zedclaw_home, display_zedclaw_home

        home = get_zedclaw_home()
        try:
            display_path = f"{display_zedclaw_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  zedclaw import {out_path}")
...
def cmd_update(args):
    """Update ZedClaw to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from zedclaw_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update ZedClaw")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://raw.githubusercontent.com/ZedClaw/zedclaw/main/scripts/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        # the old bytecode (e.g. get_zedclaw_home added to zedclaw_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload zedclaw_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_zedclaw_home() added since the last release.
        try:
            import importlib
            import zedclaw_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit ZEDCLAW_HOME so
        # it is not affected by sync_skills()'s module-level ZEDCLAW_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's ZEDCLAW_HOME env var points at the default or a named profile.
        try:
            from zedclaw_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        if needs_migration:
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: zedclaw config migrate")
            else:
                print()
                print("Skipped. Run 'zedclaw config migrate' later to configure.")
        else:
        ...
        # check `zedclaw curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``zedclaw update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from zedclaw_cli.tools_config import install_cua_driver
        ...
        # When running as ``zedclaw update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_zedclaw_home() / ".update_exit_code"
            try:
        ...
        try:
            from zedclaw_cli.gateway import (
                is_macos,
            ...
            # Discover all zedclaw-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. zedclaw-gateway.service or zedclaw-gateway-coder.service
                            ...
                            # path in `zedclaw gateway restart`
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: zedclaw gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: zedclaw -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy ZedClaw gateway unit files are still installed.
        # When both zedclaw.service (from a pre-rename install) and the
        # current zedclaw-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `zedclaw update` surfaces the issue until the user migrates.
        try:
            from zedclaw_cli.gateway import (
                has_legacy_zedclaw_units,
                _find_legacy_zedclaw_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_zedclaw_units():
                print()
                print("⚠ Legacy ZedClaw gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_zedclaw_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (zedclaw.service) fight the current")
                print("  zedclaw-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    zedclaw gateway migrate-legacy")
                print()
        ...
        print("  zedclaw model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``zedclaw -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from zedclaw_constants import display_zedclaw_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `zedclaw profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_zedclaw_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → zedclaw -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.zedclaw)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  zedclaw profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     zedclaw -p {name} chat")
                else:
        ...
        if dist_name:
            ...
            print(f"  (run `zedclaw profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        from zedclaw_cli.profiles import profile_exists
        ...
        from zedclaw_cli.profiles import rename_profile
        ...
        from zedclaw_cli.profiles import export_profile
        ...
        from zedclaw_cli.profiles import import_profile
        ...
        import tempfile
        from zedclaw_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="zedclaw_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  zedclaw -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      zedclaw -p {plan.manifest.name} chat")
        ...
        from zedclaw_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `zedclaw profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"zedclaw -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from zedclaw_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("zedclaw_requires"):
            print(f"Requires:     ZedClaw {data['zedclaw_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from zedclaw_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.zedclaw_requires:
        print(f"  Requires: ZedClaw {mf.zedclaw_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``zedclaw dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``zedclaw dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No zedclaw dashboard processes running.")
        return 0

    print(f"{len(pids)} zedclaw dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No zedclaw dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `zedclaw update`.
    ...
    if "ZEDCLAW_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["ZEDCLAW_WEB_DIST"])
            if "ZEDCLAW_WEB_DIST" in os.environ
            else PROJECT_ROOT / "zedclaw_cli" / "web_dist"
        )
    ...
    from zedclaw_cli.web_server import start_server

    embedded_chat = args.tui or os.environ.get("ZEDCLAW_DASHBOARD_TUI") == "1"
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from zedclaw_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter ZedClaw log files."""
    from zedclaw_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from zedclaw_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``zedclaw -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``zedclaw_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``zedclaw -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``zedclaw`` or only flags → defaults to ``chat``.
        return False
...
def main():
    """Main entry point for zedclaw CLI."""
    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from zedclaw_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``zedclaw.exe.old.*`` quarantine files left by previous
    # ``zedclaw update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_zedclaw_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    from zedclaw_cli._parser import build_top_level_parser
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: zedclaw-cli)",
    )
    model_parser.add_argument(
    ...
    # =========================================================================
    from zedclaw_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://zedclaw.zedclaw.local/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `zedclaw model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy zedclaw.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy ZedClaw gateway unit files "
            "(e.g. zedclaw.service) left over from older installs. Profile "
            "units (zedclaw-gateway-<profile>.service) and unrelated "
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure ZedClaw with an interactive wizard. "
        "Run a specific section: zedclaw setup model|tts|terminal|gateway|tools|agent|osspr",
    )
    setup_parser.add_argument(
        "section",
        nargs="?",
        choices=["model", "tts", "terminal", "gateway", "tools", "agent", "osspr"],
        default=None,
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'zedclaw setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for ZedClaw.",
    )
    ...
    slack_manifest.add_argument(
        ...
        "writes to $ZEDCLAW_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "ZedClaw")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for ZedClaw CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: zedclaw-cli)"
    )
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate ZedClaw with Spotify via PKCE"
    )
    ...
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set ZEDCLAW_SPOTIFY_CLIENT_ID)"
    )
    auth_spotify.add_argument(
        "--redirect-uri",
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of ZedClaw components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.zedclaw/scripts/. Default mode: "
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.zedclaw/scripts/. Pass empty string to clear. "
    ...
    # =========================================================================
    from zedclaw_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.zedclaw/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.zedclaw/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with ZedClaw setup",
    )
    ...
    doctor_parser.add_argument(
        ...
        help=(
            ...
            "advisory will no longer trigger startup banners. Run `zedclaw "
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your ZedClaw setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for ZedClaw. Use 'zedclaw debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    zedclaw debug share              Upload debug report and print URL
    zedclaw debug share --lines 500  Include more log lines
    zedclaw debug share --expire 30  Keep paste for 30 days
    zedclaw debug share --local      Print report locally (no upload)
    zedclaw debug share --no-redact  Disable upload-time secret redaction
    zedclaw debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'zedclaw debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up ZedClaw home directory to a zip file",
        description="Create a zip archive of your entire ZedClaw configuration, "
        "skills, sessions, and data (excludes the zedclaw codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/zedclaw-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.zedclaw/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo zedclaw uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from zedclaw_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a ZedClaw backup from a zip file",
        description="Extract a previously created ZedClaw backup into your "
        "ZedClaw home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage ZedClaw configuration",
    )
    ...
    def cmd_pairing(args):
        from zedclaw_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.zedclaw/skills/.bundled_manifest) "
            "so future 'zedclaw update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from zedclaw_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from zedclaw_cli.skills_hub import skills_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/zedclaw-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `zedclaw plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from zedclaw_cli.plugins_cmd import plugins_command
    ...
    # subcommand — ``zedclaw --help``, ``zedclaw version``, ``zedclaw logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from zedclaw_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from zedclaw_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            mem_dir = get_zedclaw_home() / "memories"
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_zedclaw_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_zedclaw_home()}/memories/\n")
        else:
            from zedclaw_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'zedclaw tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # zedclaw tools list [--platform cli]
    ...
    # zedclaw tools disable <name...> [--platform cli]
    ...
    # zedclaw tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from zedclaw_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from zedclaw_cli.tools_config import tools_command
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `zedclaw computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `zedclaw tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: zedclaw computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: zedclaw computer-use install")
            return
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run ZedClaw as an MCP server",
        description=(
            "Manage MCP server connections and run ZedClaw as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'zedclaw mcp add' to connect to a new server, or\n"
            "'zedclaw mcp serve' to expose ZedClaw conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run ZedClaw as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `zedclaw mcp add ...` to fall through to
    # interactive chat.
    ...
    def cmd_mcp(args):
        from zedclaw_cli.mcp_config import mcp_command
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_zedclaw_home() / "sessions"
            ...
            # Launch zedclaw --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            from zedclaw_cli.relaunch import relaunch
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to ZedClaw",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to ZedClaw",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.zedclaw/ (by default a "
        "single restore-point archive is written to ~/.zedclaw/backups/ "
        "before apply; restorable with 'zedclaw import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from zedclaw_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update ZedClaw to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'zedclaw config migrate' separately for those.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall ZedClaw",
        description="Remove ZedClaw from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run ZedClaw as an ACP (Agent Client Protocol) server",
        description="Start ZedClaw in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    _add_accept_hooks_flag(acp_parser)

    def cmd_acp(args):
        """Launch ZedClaw as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated ZedClaw instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `zedclaw update` skill sync)",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a ZedClaw profile distribution. SOURCE can be a git URL "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the ZedClaw web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `zedclaw --tui` via PTY/WebSocket). "
            "Alternatively set ZEDCLAW_DASHBOARD_TUI=1."
        ),
    )
    dashboard_parser.add_argument(
        "--skip-build",
    ...
    # for `zedclaw dashboard` cmdlines and SIGTERM them directly — the same
    # path `zedclaw update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running zedclaw dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running zedclaw dashboard processes and exit",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter ZedClaw log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    zedclaw logs                    Show last 50 lines of agent.log
    zedclaw logs -f                 Follow agent.log in real time
    zedclaw logs errors             Show last 50 lines of errors.log
    zedclaw logs gateway -n 100     Show last 100 lines of gateway.log
    zedclaw logs --level WARNING    Only show WARNING and above
    zedclaw logs --session abc123   Filter by session ID
    zedclaw logs --component tools  Only show tool-related lines
    zedclaw logs --since 1h         Lines from the last hour
    zedclaw logs --since 30m -f     Follow, starting from 30 min ago
    zedclaw logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``zedclaw -c Pokemon Agent Dev`` → ``zedclaw -c 'Pokemon Agent Dev'``
    ...
    from zedclaw_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'zedclaw -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (zedclaw hooks list, cron
    ...
    if args.command in _AGENT_COMMANDS or (
        ...
        try:
            from zedclaw_cli.plugins import discover_plugins
    ...
    if getattr(args, "oneshot", None):
        from zedclaw_cli.oneshot import run_oneshot

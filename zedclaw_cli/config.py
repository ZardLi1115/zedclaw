"""
Configuration management for ZedClaw.

Config files are stored in ~/.zedclaw/ for easy access:
- ~/.zedclaw/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.zedclaw/.env         - API keys and secrets

This module provides:
- zedclaw config          - Show current configuration
- zedclaw config edit     - Open config in editor
- zedclaw config set      - Set a specific value
- zedclaw config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.zedclaw/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``zedclaw logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  zedclaw config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `zedclaw plugins enable
    # observability/langfuse` or `zedclaw tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "ZEDCLAW_LANGFUSE_ENV",
    "ZEDCLAW_LANGFUSE_RELEASE",
    "ZEDCLAW_LANGFUSE_SAMPLE_RATE",
    "ZEDCLAW_LANGFUSE_MAX_CHARS",
    "ZEDCLAW_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from zedclaw_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("ZEDCLAW_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_zedclaw_home() / ".managed"
...
def is_managed() -> bool:
    """Check if ZedClaw is running in package-manager-managed mode.

    Two signals: the ZEDCLAW_MANAGED env var (set by the systemd service),
    or a .managed marker file in ZEDCLAW_HOME (set by the NixOS activation
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "zedclaw update"


def format_managed_message(action: str = "modify this ZedClaw installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("ZEDCLAW_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this ZedClaw installation is managed by NixOS "
            f"(ZEDCLAW_MANAGED={env_hint}).\n"
            "Edit services.zedclaw.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this ZedClaw installation is managed by Homebrew "
            f"(ZEDCLAW_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade zedclaw"
        )

    return (
        f"Cannot {action}: this ZedClaw installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall ZedClaw."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ZEDCLAW_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, zedclaw_bin
    or None if container mode is not active, we're already inside the
    container, or ZEDCLAW_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("ZEDCLAW_DEV") == "1":
        return None
    ...
    container_mode_file = get_zedclaw_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "zedclaw")
    exec_user = info.get("exec_user", "zedclaw")
    zedclaw_bin = info.get("zedclaw_bin", "/data/current-package/bin/zedclaw")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "zedclaw_bin": zedclaw_bin,
    }
...
# Re-export from zedclaw_constants — canonical definition lives there.
from zedclaw_constants import get_zedclaw_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_zedclaw_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_zedclaw_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the zedclaw group can
    share state with the gateway service.

    The mode can be overridden via the ZEDCLAW_HOME_MODE environment variable
    (e.g. ZEDCLAW_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse ZEDCLAW_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("ZEDCLAW_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When ZedClaw runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("ZEDCLAW_CONTAINER") or os.environ.get("ZEDCLAW_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set ZEDCLAW_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into ZEDCLAW_HOME if the user doesn't have one yet."""
...
def ensure_zedclaw_home():
    """Ensure ~/.zedclaw directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_zedclaw_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_zedclaw_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"ZEDCLAW_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the ZedClaw-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, ZedClaw auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), ZedClaw sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when ZedClaw
        ...
        # bundled ZedClaw image, which drops to the `zedclaw` user via gosu).
    ...
    "browser": {
        ...
        "camofox": {
            # When true, ZedClaw sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and ZedClaw should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``zedclaw chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.zedclaw/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: zedclaw sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, ZedClaw truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``zedclaw kanban specify`` (single id or --all). Set a
        ...
        # model; override via `zedclaw model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `zedclaw --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `zedclaw -c` muscle memory.  Default off so existing
        # users aren't surprised.  ZEDCLAW_TUI_RESUME=<id> always wins.
        ...
        # e.g. `model · 68% · ~/projects/zedclaw`. Per-platform overrides go under
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.zedclaw/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.zedclaw/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, ZedClaw feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before ZedClaw auto-pauses the goal and
    ...
    # always goes to ~/.zedclaw/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${ZEDCLAW_SKILL_DIR} and ${ZEDCLAW_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `zedclaw curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot ~/.zedclaw/skills/ into
        # ~/.zedclaw/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `zedclaw curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for zedclaw-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *ZedClaw*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (ZEDCLAW_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.zedclaw/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or ZEDCLAW_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `zedclaw doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``zedclaw_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow ZedClaw to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via ZEDCLAW_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `zedclaw -p <assignee> chat -q ...` for
    ...
    # Autonomous OSS PR agent — gateway-hosted runtime that searches GitHub
    # issues, calls Codex CLI to create PRs, watches PR/check state, and gives
    # Codex up to max_fix_attempts to repair failing PRs. It self-schedules via
    # ~/.zedclaw/oss_pr_agent/state.db next_wake_at rather than fixed cron jobs.
    "oss_pr_agent": {
        "enabled": True,
        "language": "en",
        "focus": "all",
        ...
        "min_wake_delay_minutes": 5,
        "max_wake_delay_hours": 24,
        "email_check_limit": 10,
        "email_classifier_model": "gpt-5.4-mini",
        ...
        "max_fix_attempts": 5,
        "max_active_prs": 3,
        "repo_close_cooldown_enabled": True,
        "repo_close_cooldown_days": 7,
        "small_pr_until_merged_count": 3,
        "planner_memory_max_chars": 5000,
        "codex_memory_max_chars": 5000,
        "reusable_tools_enabled": True,
        "max_reusable_tools": 15,
        "min_repo_stars": 100,
        "max_repo_inactive_days": 30,
        "fallback_max_repo_inactive_days": 60,
        "github_search_limit": 12,
        "workspaces_dir": "",
        "budget_url": "",
        "cpa_budget_enabled": True,
        "cpa_config_path": "/opt/cliproxyapi/config.yaml",
        "cpa_management_base_url": "",
        "cpa_management_key": "",
        "cpa_auth_dir": "",
        "cpa_codex_auth_email": "",
        "cpa_codex_auth_file": "",
    ...
    "code_execution": {
        ...
        #     zedclaw's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.zedclaw/logs/.
    ...
    # update model picker lists without shipping a zedclaw release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://zedclaw.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `zedclaw model` invocation; network failures
    ...
    # Session storage — controls automatic cleanup of ~/.zedclaw/state.db.
    ...
    "sessions": {
        ...
        # default of ``zedclaw sessions prune``.
        "retention_days": 90,
    ...
    # ``zedclaw update`` behaviour.
    "updates": {
        # Run a full ``zedclaw backup``-style zip of ZEDCLAW_HOME before every
        # ``zedclaw update``.  Backups land in ``<ZEDCLAW_HOME>/backups/`` and
        # can be restored with ``zedclaw import <path>``.  Off by default —
        # on large ZEDCLAW_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<ZEDCLAW_HOME>/lsp/bin/`` on first use.
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'zedclaw model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the ZedClaw auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @zedclaw:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. ZEDCLAW_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #zedclaw)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: zedclaw-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'zedclaw' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote ZedClaw API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote ZedClaw API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote ZedClaw API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # ZEDCLAW_TOOL_PROGRESS and ZEDCLAW_TOOL_PROGRESS_MODE are deprecated —
    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).
    # Gateway falls back to these env vars for backward compatibility.
    "ZEDCLAW_TOOL_PROGRESS": {
    ...
    "ZEDCLAW_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.zedclaw.config`` entries, then checks
    ...
    try:
        ...
        # should never break `zedclaw update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older ZedClaw versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``zedclaw_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'zedclaw setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — ZedClaw won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'zedclaw doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("ZEDCLAW_HOME", "~/.zedclaw")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("ZEDCLAW_TOOL_PROGRESS")
            old_mode = get_env_value("ZEDCLAW_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from ZEDCLAW_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from ZEDCLAW_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("ZEDCLAW_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from ZEDCLAW_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$ZEDCLAW_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_zedclaw_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `zedclaw plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `zedclaw curator status` has no
    ...
    #   3. Creates `~/.zedclaw/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_zedclaw_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_zedclaw_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: zedclaw config set <key> <value>")
    ...
    # metadata.zedclaw.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _zedclaw_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.zedclaw/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.zedclaw/config.yaml.
    ...
    (which change ``ZEDCLAW_HOME`` and therefore ``get_config_path()``)
    don't collide.
    """
    with _CONFIG_LOCK:
        ensure_zedclaw_home()
...
#   openai-codex (OAuth — zedclaw auth) — OpenAI Codex
#   nous         (OAuth — zedclaw auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.zedclaw/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.zedclaw/.env.
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real ZedClaw env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.zedclaw/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.zedclaw/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.zedclaw/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.zedclaw/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to ZedClaw — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known ZedClaw vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.zedclaw/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ ZedClaw Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  zedclaw config edit     # Edit config file", Colors.DIM))
    print(color("  zedclaw config set <key> <value>", Colors.DIM))
    print(color("  zedclaw setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: zedclaw config set <key> <value>")
            print()
            print("Examples:")
            print("  zedclaw config set model anthropic/claude-sonnet-4")
            print("  zedclaw config set terminal.backend docker")
            print("  zedclaw config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'zedclaw config migrate' to add them")
        ...
        print("  zedclaw config           Show current configuration")
        print("  zedclaw config edit      Open config in editor")
        print("  zedclaw config set <key> <value>   Set a config value")
        print("  zedclaw config check     Check for missing/outdated config")
        print("  zedclaw config migrate   Update config with new options")
        print("  zedclaw config path      Show config file path")
        print("  zedclaw config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``zedclaw config`` UI so users

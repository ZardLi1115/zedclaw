...
from zedclaw_constants import OPENROUTER_BASE_URL, get_zedclaw_home
...
if tinker_atropos_dir.exists():
    ...
    os.environ['ZEDCLAW_QUIET'] = '1'  # Disable temp subdirectory creation
    print(f"📂 Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to zedclaw directory if submodule not found
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.zedclaw/config.yaml if not provided)
    ...
    """
    # Load config from ~/.zedclaw/config.yaml
    config = load_zedclaw_config()

#!/usr/bin/env python3
"""Google Workspace API CLI for ZedClaw.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing ZedClaw-facing JSON contract and falls back to the Python client
...
from _zedclaw_home import get_zedclaw_home
...
def _gws_binary() -> str | None:
    override = os.getenv("ZEDCLAW_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for ZedClaw")

...
## 5. ZedClaw Configuration

### Config location

`$ZEDCLAW_HOME/config.yaml` (defaults to `~/.zedclaw/config.yaml` when `ZEDCLAW_HOME` is unset)
...
Restart the ZedClaw session for changes to take effect. The MCP connection is

...
ZEDCLAW_HOME_DIR="${ZEDCLAW_HOME:-$HOME/.zedclaw}"
ZEDCLAW_CFG="${ZEDCLAW_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure ZedClaw config has twozero_td MCP entry ──
if [[ ! -f "$ZEDCLAW_CFG" ]]; then
    echo -e " ${FAIL} ZedClaw config not found at ${ZEDCLAW_CFG}"
    manual_steps+=("Create ${ZEDCLAW_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$ZEDCLAW_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in ZedClaw config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to ZedClaw config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$ZEDCLAW_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${ZEDCLAW_CFG} manually"); }
    manual_steps+=("Restart ZedClaw session to pick up config change")
fi

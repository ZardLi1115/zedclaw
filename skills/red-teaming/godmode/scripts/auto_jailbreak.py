...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("ZEDCLAW_HOME", os.path.expanduser("~/.zedclaw")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("ZEDCLAW_HOME", Path.home() / ".zedclaw")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# ZedClaw config paths
# ═══════════════════════════════════════════════════════════════════

ZEDCLAW_HOME = Path(os.getenv("ZEDCLAW_HOME", Path.home() / ".zedclaw"))
CONFIG_PATH = ZEDCLAW_HOME / "config.yaml"
PREFILL_PATH = ZEDCLAW_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/ZedClaw models — already uncensored, just needs clean prompt
    "zedclaw": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "zedclaw" in model_lower or "nous" in model_lower:
        return "zedclaw"
...
def _get_current_model() -> tuple:
    """Read current model and provider from ZedClaw config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.zedclaw/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart ZedClaw for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart ZedClaw for changes to take effect.")

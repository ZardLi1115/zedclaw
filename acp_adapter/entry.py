"""CLI entry point for the zedclaw ACP adapter.

Loads environment variables from ``~/.zedclaw/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from ZEDCLAW_HOME (default ``~/.zedclaw``)."""
    from zedclaw_cli.env_loader import load_zedclaw_dotenv

    zedclaw_home = get_zedclaw_home()
    loaded = load_zedclaw_dotenv(zedclaw_home=zedclaw_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", zedclaw_home / ".env"
        )
...
def main() -> None:
    ...
    logger.info("Starting zedclaw ACP adapter")
    ...
    import acp
    from .server import ZedClawACPAgent

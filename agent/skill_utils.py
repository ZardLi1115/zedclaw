...
from zedclaw_constants import get_config_path, get_skills_dir
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``ZEDCLAW_PLATFORM`` or
            ``ZEDCLAW_SESSION_PLATFORM`` env vars.  Falls back to the
...
# which becomes the dominant cost of ``zedclaw`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.zedclaw/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``zedclaw`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against ZEDCLAW_HOME, not cwd
        if not p.is_absolute():
            p = (zedclaw_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.zedclaw/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    zedclaw = metadata.get("zedclaw") or {}
    if not isinstance(zedclaw, dict):
        zedclaw = {}
    return {
        "fallback_for_toolsets": zedclaw.get("fallback_for_toolsets", []),
        "requires_toolsets": zedclaw.get("requires_toolsets", []),
        "fallback_for_tools": zedclaw.get("fallback_for_tools", []),
        "requires_tools": zedclaw.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    zedclaw = metadata.get("zedclaw")
    if not isinstance(zedclaw, dict):
        return []
    raw = zedclaw.get("config")
    if not raw:
        return []

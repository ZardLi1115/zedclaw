...
as ``kind: backend``) or ``~/.zedclaw/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``zedclaw tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``zedclaw tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``zedclaw tools`` picker.
...
def _images_cache_dir() -> Path:
    """Return ``$ZEDCLAW_HOME/cache/images/``, creating parents as needed."""
    from zedclaw_constants import get_zedclaw_home

    path = get_zedclaw_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$ZEDCLAW_HOME/cache/images/``.

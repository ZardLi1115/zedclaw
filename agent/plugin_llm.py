...
Plugins built on ZedClaw often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing ZedClaw
...
:class:`~zedclaw_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors ZedClaw' main config
...
handles every provider, fallback chain, and per-task override ZedClaw
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`zedclaw_cli.plugins.PluginContext`

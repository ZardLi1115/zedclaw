...
Every ZedClaw request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to ZedClaw and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=zedclaw",
        "client=zedclaw-client-v<__version__>",
    ]

The version is sourced live from ``zedclaw_cli.__version__`` so it auto-aligns
...
``zedclaw_cli.__version__`` is the canonical source of truth.
"""
...
def _zedclaw_version() -> str:
    """Return the current ZedClaw release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``zedclaw_cli`` cannot be imported (should
...
def zedclaw_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=zedclaw-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=zedclaw-client-v{_zedclaw_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=zedclaw", zedclaw_client_tag()]

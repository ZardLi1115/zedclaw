...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time ZedClaw starts and finds ``~/.openclaw/``.

    Points users at ``zedclaw claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``zedclaw claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to ZedClaw, run "
        "`zedclaw claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `zedclaw claw cleanup` (renames it to ~/.openclaw.pre-migration — "

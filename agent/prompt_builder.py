...
from zedclaw_constants import get_zedclaw_home, get_skills_dir, is_wsl
...
_ZEDCLAW_MD_NAMES = (".zedclaw.md", "ZEDCLAW.md")


def _find_zedclaw_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.zedclaw.md`` or ``ZEDCLAW.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are ZedClaw, an intelligent AI assistant created by Nous Research. "
...
ZEDCLAW_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using ZedClaw "
    "itself, load the `zedclaw` skill with skill_view(name='zedclaw') "
    "before answering. Docs: https://zedclaw.nousresearch.com/docs"
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.zedclaw/kanban.db`. Your task id is in "
    "`$ZEDCLAW_KANBAN_TASK`; your workspace is `$ZEDCLAW_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $ZEDCLAW_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `zedclaw kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the ZedClaw WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where ZedClaw itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host ZedClaw runs on.
    """
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where ZedClaw itself is running. The host OS, home, and cwd "
                f"of the ZedClaw process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where ZedClaw "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_zedclaw_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.zedclaw/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("ZEDCLAW_PLATFORM")
        or get_session_env("ZEDCLAW_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot ZedClaw itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `zedclaw` skill "
            "first. It has the actual commands (e.g. `zedclaw config set …`, `zedclaw tools`, "
            "`zedclaw setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: zedclaw setup, zedclaw setup tools, zedclaw setup terminal, zedclaw status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from ZEDCLAW_HOME and return its content, or None.
    ...
    """
    try:
        from zedclaw_cli.config import ensure_zedclaw_home
        ensure_zedclaw_home()
    except Exception as e:
        logger.debug("Could not ensure ZEDCLAW_HOME before loading SOUL.md: %s", e)

    soul_path = get_zedclaw_home() / "SOUL.md"
...
def _load_zedclaw_md(cwd_path: Path) -> str:
    """.zedclaw.md / ZEDCLAW.md — walk to git root."""
    zedclaw_md_path = _find_zedclaw_md(cwd_path)
    if not zedclaw_md_path:
        return ""
    try:
        content = zedclaw_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = zedclaw_md_path.name
        try:
            rel = str(zedclaw_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".zedclaw.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", zedclaw_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .zedclaw.md / ZEDCLAW.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from ZEDCLAW_HOME is independent and always included when present.
    ...
    # SOUL.md from ZEDCLAW_HOME only — skip when already loaded as identity
    if not skip_soul:

...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``ZEDCLAW_PLATFORM`` env var and
    ``ZEDCLAW_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = os.getenv("ZEDCLAW_PLATFORM")
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.zedclaw.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_zedclaw_home()}/config.yaml):"]
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.zedclaw/skills/ and return a mapping of /command -> skill info.
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.zedclaw/skills/`` and any ``skills.external_dirs`` so the

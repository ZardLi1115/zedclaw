...
class MemoryManager:
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``zedclaw_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_zedclaw_home()`` themselves.
        """
        if "zedclaw_home" not in kwargs:
            from zedclaw_constants import get_zedclaw_home
            kwargs["zedclaw_home"] = str(get_zedclaw_home())
        for provider in self._providers:
            try:

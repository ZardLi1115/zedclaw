"""Windows UTF-8 bootstrap for ZedClaw entry points.
...
should be imported at the very top of every ZedClaw entry point
(``zedclaw``, ``zedclaw``, ``zedclaw-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import zedclaw_bootstrap``
# (or ``from zedclaw_bootstrap import apply_windows_utf8_bootstrap``) at

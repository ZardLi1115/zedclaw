...
React + Ink terminal UI for ZedClaw. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `ZEDCLAW_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.zedclaw/.zedclaw_history` or under `ZEDCLAW_HOME`.
...
```text
ui-tui/
  packages/zedclaw-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      zedclaw-ink.d.ts    type declarations for @zedclaw/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent ZedClawCLI subprocess for slash commands
```

...
await build({
  ...
  // Skip the prebuilt @zedclaw/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@zedclaw/ink': resolve(root, 'packages/zedclaw-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The zedclaw_cli launcher

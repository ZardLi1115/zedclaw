import type { RunExternalProcess } from '@zedclaw/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`zedclaw ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching zedclaw: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`zedclaw ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })

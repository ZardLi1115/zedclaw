"""
ZedClaw MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (ZedClaw-specific extra)

Usage:
    zedclaw mcp serve
    zedclaw mcp serve --verbose
...
logger = logging.getLogger("zedclaw.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using ZEDCLAW_HOME."""
    try:
        from zedclaw_constants import get_zedclaw_home
        return get_zedclaw_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("ZEDCLAW_HOME", Path.home() / ".zedclaw")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from zedclaw_constants import get_zedclaw_home
        directory_file = get_zedclaw_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("ZEDCLAW_HOME", Path.home() / ".zedclaw")
...
class EventBridge:
    ...
    This is the ZedClaw equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from zedclaw_constants import get_zedclaw_home
            db_file = get_zedclaw_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("ZEDCLAW_HOME", Path.home() / ".zedclaw")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the ZedClaw MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "zedclaw",
        instructions=(
            "ZedClaw messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the ZedClaw MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(

...
zedclaw kanban watch --tenant <project-slug>

# Snapshot of the board
zedclaw kanban list --tenant <project-slug>
zedclaw kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
zedclaw kanban stats --tenant <project-slug>

# Visual dashboard (browser)
zedclaw dashboard

# Inspect a specific task (includes comments + events)
zedclaw kanban show <task-id>

# Follow a single task's event stream
zedclaw kanban tail <task-id>
```

Verify available subcommands with `zedclaw kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `zedclaw kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `zedclaw kanban block <id>` then `zedclaw kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `zedclaw kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `zedclaw kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `zedclaw profile list` (profile exists?), `zedclaw status` (gateway/dispatcher up?), and verify tenant. |
...
zedclaw kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
zedclaw kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(zedclaw kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
zedclaw kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
zedclaw kanban block <task-id>
zedclaw kanban archive <task-id>

# Diagnose what happened
zedclaw kanban show <task-id>      # task body, comments, recent events
zedclaw kanban tail <task-id>      # follow the live event stream
zedclaw kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `zedclaw kanban create` for the director
...
while true; do
    clear
    zedclaw kanban list --tenant <slug>
    echo "---"
    zedclaw kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `zedclaw kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `zedclaw profile list` and start

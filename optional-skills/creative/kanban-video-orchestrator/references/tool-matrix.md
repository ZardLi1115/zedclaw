...
Maps each role archetype to the ZedClaw skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public zedclaw
...
## ZedClaw skills relevant to video production

### Visual / rendering skills (`zedclaw/skills/creative/`)
...
### Design / pre-production skills (`zedclaw/skills/creative/`)
...
### Audio / media skills (`zedclaw/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`zedclaw/skills/devops/`)
...
These are **not** ZedClaw skills but external CLIs / APIs that profiles invoke.
...
## Built-in ZedClaw tools for media review

These are native ZedClaw tools — not invoked via terminal but through their own
...
key is present in `~/.zedclaw/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every ZedClaw profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.zedclaw/.env` → environment variable.
...
When a new ZedClaw-public video skill ships:

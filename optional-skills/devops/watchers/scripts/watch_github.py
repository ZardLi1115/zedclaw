...
Usage (via cron with --no-agent):
    ...
    zedclaw cron create zedclaw-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$ZEDCLAW_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name zedclaw-issues --repo NousResearch/zedclaw --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.zedclaw/.env to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "ZedClaw-Watcher/1.0",
    }

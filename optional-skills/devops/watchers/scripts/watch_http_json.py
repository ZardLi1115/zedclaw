...
Usage (via cron with --no-agent):
    ...
    zedclaw cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$ZEDCLAW_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "ZedClaw-Watcher/1.0"})

"""Persistent slash-command worker — one ZedClawCLI per TUI session.
...
def _run(cli: ZedClawCLI, command: str) -> str:
...
def main():
    ...
    os.environ["ZEDCLAW_SESSION_KEY"] = args.session_key
    os.environ["ZEDCLAW_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = ZedClawCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)

...
# 5. Symlinks the 'zedclaw' CLI command into a user-facing bin dir
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/zedclaw-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink zedclaw into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up zedclaw command..."

ZEDCLAW_BIN="$SCRIPT_DIR/venv/bin/zedclaw"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$ZEDCLAW_BIN" "$COMMAND_LINK_DIR/zedclaw"
echo -e "${GREEN}✓${NC} Symlinked zedclaw → $COMMAND_LINK_DISPLAY_DIR/zedclaw"
...
# ============================================================================
# Seed bundled skills into ~/.zedclaw/skills/
# ============================================================================

ZEDCLAW_SKILLS_DIR="${ZEDCLAW_HOME:-$HOME/.zedclaw}/skills"
mkdir -p "$ZEDCLAW_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.zedclaw/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$ZEDCLAW_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  zedclaw status        # Check configuration"
if is_termux; then
    echo "  zedclaw gateway       # Run gateway in foreground"
else
    echo "  zedclaw gateway install # Install gateway service (messaging + cron)"
fi
echo "  zedclaw cron list     # View scheduled jobs"
echo "  zedclaw doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m zedclaw_cli.main setup
fi

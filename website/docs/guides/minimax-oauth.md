...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in ZedClaw — no API key required"
---

# MiniMax OAuth

ZedClaw supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and ZedClaw automatically refreshes your session.
...
# → ZedClaw opens your browser to the MiniMax authorization page
...
After the first login, credentials are stored under `~/.zedclaw/auth.json` and are refreshed automatically before each session.
...
```bash
zedclaw auth add minimax-cn --type oauth   # if OAuth is supported on your CN account
# or simpler:
echo 'MINIMAX_CN_API_KEY=your-key' >> ~/.zedclaw/.env
```
...
```bash
zedclaw auth add minimax-oauth --no-browser
```

ZedClaw will print the verification URL and user code — open the URL on any device and enter the code when prompted.

## The OAuth Flow

ZedClaw implements a PKCE device-code flow against the MiniMax OAuth endpoints:

1. ZedClaw generates a PKCE verifier / challenge pair and a random state value.
...
4. ZedClaw polls `{base_url}/oauth/token` until the token arrives (or the deadline passes).
5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/.zedclaw/auth.json` under the `minimax-oauth` key.
...
```bash
zedclaw config set model MiniMax-M2.7
zedclaw config set provider minimax-oauth
```

## Configuration Reference

After login, `~/.zedclaw/config.yaml` will contain entries similar to:
...
```bash
zedclaw --provider minimax-oauth    # canonical
zedclaw --provider minimax-portal   # alias
zedclaw --provider minimax-global   # alias
zedclaw --provider minimax_oauth    # alias (underscore form)
```
...
```bash
ZEDCLAW_INFERENCE_PROVIDER=minimax-oauth zedclaw
```
...
ZedClaw refreshes the token on every session start if it is within 60 seconds of expiry. If the access token is already expired (for example, after a long offline period), the refresh happens automatically on the next request. If refresh fails with `refresh_token_reused` or `invalid_grant`, ZedClaw marks the session as requiring re-login.

**Fix:** run `zedclaw auth add minimax-oauth` again to start a fresh login.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, ZedClaw raises a timeout error.

**Fix:** re-run `zedclaw auth add minimax-oauth` (or `zedclaw model`). The flow starts fresh.

### State mismatch (possible CSRF)

ZedClaw detected that the `state` value returned by the authorization server does not match what it sent.
...
If `zedclaw` cannot open a browser window, use `--no-browser`:

```bash
zedclaw auth add minimax-oauth --no-browser
```

ZedClaw prints the URL and code. Open the URL on any device and complete the flow there.
...
**Fix:** run `zedclaw model` and select MiniMax (OAuth), or run `zedclaw auth add minimax-oauth`.
...
```bash
zedclaw auth remove minimax-oauth
```
...
- [zedclaw doctor](../reference/cli-commands.md)

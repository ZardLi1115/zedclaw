...
This tutorial walks you through setting up a Telegram bot powered by ZedClaw that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **ZedClaw installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.zedclaw/.env`

:::tip
A $5/month VPS is plenty for running the gateway. ZedClaw itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team ZedClaw Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_zedclaw_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by ZedClaw. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.zedclaw/.env`:
...
```
[Gateway] Starting ZedClaw Gateway...
...
# View live logs
journalctl --user -u zedclaw-gateway -f
...
zedclaw gateway start
zedclaw gateway stop
tail -f ~/.zedclaw/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `zedclaw gateway install` to update the plist.
:::
...
```bash
zedclaw gateway stop && zedclaw gateway start
```
...
zedclaw pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.zedclaw/.env`:
...
Control how much detail the bot shows when using tools. In `~/.zedclaw/config.yaml`:
...
Customize how the bot communicates by editing `~/.zedclaw/SOUL.md`:

For a full guide, see [Use SOUL.md with ZedClaw](/docs/guides/use-soul-with-zedclaw).
...
```bash
# From the CLI
zedclaw cron list          # View all scheduled jobs
zedclaw cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u zedclaw-gateway` (Linux) or `~/.zedclaw/logs/gateway.log` (macOS) |
| Cron job output | `~/.zedclaw/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.zedclaw/cron/jobs.json` |
| Pairing data | `~/.zedclaw/pairing/` |
| Session history | `~/.zedclaw/sessions/` |

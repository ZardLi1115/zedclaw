...
```bash
zedclaw teams-pipeline validate
```
...
```bash
zedclaw teams-pipeline token-health
zedclaw teams-pipeline token-health --force-refresh
```
...
```bash
zedclaw teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
zedclaw teams-pipeline maintain-subscriptions
zedclaw teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: ZedClaw cron (recommended if you already run the ZedClaw gateway)

ZedClaw ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.zedclaw/scripts/`. First create the script:

```bash
mkdir -p ~/.zedclaw/scripts
cat > ~/.zedclaw/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec zedclaw teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.zedclaw/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
zedclaw cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/zedclaw-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=ZedClaw Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=zedclaw
EnvironmentFile=/etc/zedclaw/env
ExecStart=/usr/local/bin/zedclaw teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/zedclaw-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run ZedClaw Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now zedclaw-teams-pipeline-maintain.timer
systemctl list-timers zedclaw-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/zedclaw teams-pipeline maintain-subscriptions >> /var/log/zedclaw/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.zedclaw/.env` at the top of a wrapper script that crontab calls.
...
```bash
zedclaw teams-pipeline subscriptions   # should show expirationDateTime advanced
zedclaw teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
zedclaw teams-pipeline list
zedclaw teams-pipeline list --status failed
zedclaw teams-pipeline show <job-id>
```

### Replay a stored job

```bash
zedclaw teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
zedclaw teams-pipeline fetch --meeting-id <meeting-id>
zedclaw teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `zedclaw teams-pipeline maintain-subscriptions --dry-run`
- inspect `zedclaw teams-pipeline list --status failed`
...
- run `zedclaw teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `zedclaw teams-pipeline run`
...
- [ ] `zedclaw teams-pipeline validate` returns an OK snapshot
- [ ] `zedclaw teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (ZedClaw cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.

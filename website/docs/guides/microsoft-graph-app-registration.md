...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.zedclaw/.env` at the end.
...
4. Fill in:
   - **Name:** `ZedClaw Teams Meeting Pipeline` (or any name you'll recognize).
...
3. **Description:** `zedclaw-graph-secret`. **Expires:** pick a value that matches your rotation policy (6-24 months is typical).
4. Click **Add**.
...
```powershell
# Create a policy scoped to the ZedClaw app
New-CsApplicationAccessPolicy `
  -Identity "ZedClaw-Meeting-Pipeline-Policy" `
  -AppIds "<MSGRAPH_CLIENT_ID>" `
  -Description "Restrict ZedClaw meeting pipeline to allow-listed users"

# Grant the policy to specific users whose meetings the pipeline may read
Grant-CsApplicationAccessPolicy `
  -PolicyName "ZedClaw-Meeting-Pipeline-Policy" `
...
Put the three values you collected into `~/.zedclaw/.env`:
...
ZedClaw ships a Graph auth smoke-test. From your ZedClaw install:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.zedclaw/.env` with the new value.
3. Restart the gateway so the new secret is picked up: `zedclaw gateway restart`.

...
title: "Build a ZedClaw Plugin"
description: "Step-by-step guide to building a complete ZedClaw plugin with tools, hooks, data files, and skills"
---

# Build a ZedClaw Plugin

This guide walks through building a complete ZedClaw plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
ZedClaw has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](/docs/user-guide/features/tts#voice-message-transcription-stt) — set `ZEDCLAW_LOCAL_STT_COMMAND` to a shell template |
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/docs/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.zedclaw/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/docs/user-guide/features/skills) — `zedclaw skills tap add <repo>` · [Publishing a tap](/docs/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
```bash
mkdir -p ~/.zedclaw/plugins/calculator
cd ~/.zedclaw/plugins/calculator
```
...
This tells ZedClaw: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
4. **Accept `**kwargs`:** ZedClaw may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `zedclaw my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but ZedClaw continues fine
...
If your plugin doesn't show up — or shows up but isn't loading — set `ZEDCLAW_PLUGINS_DEBUG=1` to get verbose discovery logs on stderr:

```bash
ZEDCLAW_PLUGINS_DEBUG=1 zedclaw plugins list
```
...
The same logs are always written to `~/.zedclaw/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
zedclaw logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `zedclaw plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout** — must be `~/.zedclaw/plugins/<plugin-name>/plugin.yaml` (flat) or `~/.zedclaw/plugins/<category>/<plugin-name>/plugin.yaml` (one level of category nesting, max). Anything deeper is ignored.
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.zedclaw/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.zedclaw/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `zedclaw plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), ZedClaw injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.

#### Return format

```python
# Dict with context key
return {"context": "Recalled memories:\n- User prefers dark mode\n- Last project: zedclaw"}
...
- **The system prompt is ZedClaw's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
Plugins can add their own `zedclaw <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for zedclaw my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: zedclaw my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for zedclaw my-plugin."""
...
After registration, users can run `zedclaw my-plugin status`, `zedclaw my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `zedclaw name` in a terminal |
...
ZedClaw has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.zedclaw/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `zedclaw gateway status` without SDK instantiation.
...
ZedClaw also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/docs/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into ZedClaw without any Python plugin. Declare them in `~/.zedclaw/config.yaml`:
...
ZedClaw connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/docs/user-guide/features/mcp).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.zedclaw/hooks/<name>/`:

```yaml
# ~/.zedclaw/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.zedclaw/hooks/long-task-alert/handler.py
...
zedclaw skills search my-workflow --source myorg/skills-repo
zedclaw skills install myorg/skills-repo/my-workflow
```
...
For STT, point `ZEDCLAW_LOCAL_STT_COMMAND` at a shell template. Supported placeholders: `{input_path}`, `{output_path}`, `{format}`, `{voice}`, `{model}`, `{speed}` (TTS); `{input_path}`, `{output_dir}`, `{language}`, `{model}` (STT). Any path-interacting CLI is automatically a plugin.
...
```toml
# pyproject.toml
[project.entry-points."zedclaw_agent.plugins"]
my-plugin = "my_plugin_package"
```

```bash
pip install zedclaw-plugin-calculator
# Plugin auto-discovered on next zedclaw startup
```
...
```python
# Wrong — will break if ZedClaw passes extra context
def handler(args):
    ...

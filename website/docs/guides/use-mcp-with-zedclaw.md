---
sidebar_position: 6
title: "Use MCP with ZedClaw"
description: "A practical guide to connecting MCP servers to ZedClaw, filtering their tools, and using them safely in real workflows"
---

# Use MCP with ZedClaw

This guide shows how to actually use MCP with ZedClaw in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native ZedClaw tool
- you want ZedClaw to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect ZedClaw to internal APIs, databases, or company systems without modifying ZedClaw core

Do not use MCP when:
- a built-in ZedClaw tool already solves the job well
...
- ZedClaw remains the agent
- MCP servers contribute tools
- ZedClaw discovers those tools at startup or reload time
...
If you installed ZedClaw with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- ZedClaw banner/status should show MCP integration when configured
- ask ZedClaw what tools it has available
...
## WSL2: bridge ZedClaw in WSL to Windows Chrome

This is the practical setup when:

- ZedClaw runs inside WSL2
...
In this setup, ZedClaw does **not** connect to Chrome directly. Instead:

- ZedClaw runs in WSL
- ZedClaw starts a local stdio MCP server
...
```text
ZedClaw (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- ZedClaw stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on ZedClaw core browser transport
...
```bash
zedclaw mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
zedclaw mcp test chrome-devtools-win
```

Then start a fresh ZedClaw session or run:
...
Once loaded, ZedClaw can use the MCP-prefixed browser tools directly. For example:
...
If ZedClaw runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start ZedClaw from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start ZedClaw from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in ZedClaw:
...
2. ZedClaw-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So ZedClaw will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want ZedClaw to reason over a bounded workspace.
...
Now ZedClaw can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing ZedClaw core.
...
Because ZedClaw now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.

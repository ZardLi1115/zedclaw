---
sidebar_position: 8
title: "Use Voice Mode with ZedClaw"
description: "A practical guide to setting up and using ZedClaw voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with ZedClaw
...
- you want ZedClaw sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in ZedClaw.
...
## Step 1: make sure normal ZedClaw works first
...
ZedClaw supports both local and cloud speech stacks.
...
### If you use `zedclaw setup`

If you choose NeuTTS in the setup wizard, ZedClaw checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. ZedClaw transcribes and responds
...
- asking ZedClaw to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full ZedClaw loop.

## Tuning CLI behavior

### Silence threshold

If ZedClaw starts/stops too aggressively, tune:
...
ZedClaw stays a normal chat bot, but can speak replies.
...
- you want ZedClaw to function like a portable research or ops assistant
...
ZedClaw joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- ZedClaw detects speech boundaries
- transcripts are posted in the associated text channel
- ZedClaw responds in text and audio
...
1. get text ZedClaw working
2. install `zedclaw[voice]`

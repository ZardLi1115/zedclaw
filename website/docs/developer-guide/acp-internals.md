...
The ACP adapter wraps ZedClaw' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
zedclaw acp / zedclaw-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> load ~/.zedclaw/.env
  -> configure stderr logging
  -> construct ZedClawACPAgent
...
- `allow_once` -> ZedClaw `once`
- `allow_always` -> ZedClaw `always`
- reject options -> ZedClaw `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps ZedClaw tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["zedclaw-acp"])
...
Instead it reuses ZedClaw' runtime resolver:

- `acp_adapter/auth.py`
- `zedclaw_cli/runtime_provider.py`

So ACP advertises and uses the currently configured ZedClaw provider/credentials.
...
- ACP sessions are persisted to the shared `~/.zedclaw/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `zedclaw-acp` toolset definition
- `zedclaw_cli/main.py` — `zedclaw acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `zedclaw-acp` script

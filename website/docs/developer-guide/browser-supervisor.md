...
[PR #12550](https://github.com/NousResearch/zedclaw/pull/12550) proposed a
...
(`zedclaw-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
One `asyncio.Task` running in a background daemon thread per ZedClaw `task_id`.
...
- `toolsets.py` — register `browser_dialog` in `browser`, `zedclaw-acp`, `zedclaw-api-server`, core toolsets (gated on CDP reachability)
...
- `zedclaw_cli/config.py` — add `browser.dialog_policy` and `browser.dialog_timeout_s` to `DEFAULT_CONFIG`

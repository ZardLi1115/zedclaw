...
description: "How ZedClaw stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `zedclaw_cli/cron.py` | CLI `zedclaw cron` subcommands |
...
Jobs are stored in `~/.zedclaw/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `zedclaw cron` commands are run or during active CLI sessions.
...
```python
# ~/.zedclaw/scripts/check_competitors.py
...
2. **Environment variable** — `ZEDCLAW_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.zedclaw/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `zedclaw cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `zedclaw cron` CLI provides direct job management:

```bash
zedclaw cron list                    # Show all jobs
zedclaw cron create                  # Interactive job creation (alias: add)
zedclaw cron edit <job_id>           # Edit job configuration
zedclaw cron pause <job_id>          # Pause a running job
zedclaw cron resume <job_id>         # Resume a paused job
zedclaw cron run <job_id>            # Trigger immediate execution
zedclaw cron remove <job_id>         # Delete a job
```

...
description: "Building RL training environments, running evaluation benchmarks, and generating SFT data with the ZedClaw-Agent Atropos integration"
---

# Environments, Benchmarks & Data Generation

ZedClaw includes a full environment framework that connects its tool-calling capabilities to the [Atropos](https://github.com/NousResearch/atropos) RL training framework. This enables three workflows:
...
The Python environment framework documented here lives under the repo's `environments/` directory and is the implementation-level API for ZedClaw/Atropos integration. This is separate from the user-facing `rl_*` tools, which operate as an orchestration surface for remote RL training workflows.
:::
...
classDiagram
    ...
    BaseEnv <|-- ZedClawAgentBaseEnv
    ZedClawAgentBaseEnv <|-- TerminalTestEnv
    ZedClawAgentBaseEnv <|-- ZedClawSweEnv
    ZedClawAgentBaseEnv <|-- TerminalBench2EvalEnv
...
The zedclaw layer (`environments/zedclaw_base_env.py`). Adds:
- **Terminal backend configuration** — sets `TERMINAL_ENV` for sandboxed execution (local, Docker, Modal, Daytona, SSH, Singularity)
- **Tool resolution** — `_resolve_tools_for_group()` calls zedclaw's `get_tool_definitions()` to get the right tool schemas based on enabled/disabled toolsets
- **Agent loop integration** — `collect_trajectory()` runs `ZedClawAgentLoop` and scores the result
...
Your environment inherits from `ZedClawAgentBaseEnv` and implements five methods:
...
`ZedClawAgentLoop` (`environments/agent_loop.py`) is the reusable multi-turn agent engine. It runs the same tool-calling pattern as zedclaw's main loop:
...
| **Generic** | `call_tool(name, args)` — escape hatch for any zedclaw tool |
...
parser = get_parser("zedclaw")  # or "mistral", "llama3_json", "qwen", "deepseek_v3", etc.
content, tool_calls = parser.parse(raw_model_output)
```

Available parsers: `zedclaw`, `mistral`, `llama3_json`, `llama4_json`, `qwen`, `qwen3_coder`, `deepseek_v3`, `deepseek_v3_1` (alias `deepseek_v31`), `kimi_k2`, `longcat`, `glm45`, `glm47`.
...
```bash
python environments/zedclaw_swe_env/zedclaw_swe_env.py serve \
...
- Set `tool_call_parser` in config to match your model's format (e.g., `"zedclaw"`, `"qwen"`, `"mistral"`)
...
```python
from environments.zedclaw_base_env import ZedClawAgentBaseEnv, ZedClawAgentEnvConfig
from atroposlib.envs.server_handling.server_manager import APIServerConfig

class MyEnvConfig(ZedClawAgentEnvConfig):
    my_custom_field: str = "default_value"

class MyEnv(ZedClawAgentBaseEnv):
    name = "my-env"
...
### ZedClawAgentEnvConfig Fields

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled_toolsets` | `List[str]` | `None` (all) | Which zedclaw toolsets to enable |
...
| `tool_call_parser` | `str` | `"zedclaw"` | Parser for Phase 2 raw output |
...
env:
  ...
  tokenizer_name: "NousResearch/ZedClaw-3-Llama-3.1-8B"
  use_wandb: true
...
- [Modal](https://modal.com) account and CLI: `pip install "zedclaw[modal]"`
- `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET` environment variables

### For YC-Bench

- `pip install "zedclaw[yc-bench]"` (installs the yc-bench CLI + SQLAlchemy)
...
```
environments/
├── zedclaw_base_env.py          # Abstract base class (ZedClawAgentBaseEnv)
├── agent_loop.py               # Multi-turn agent engine (ZedClawAgentLoop)
├── tool_context.py             # Per-rollout tool access for reward functions
├── patches.py                  # Async-safety patches for Modal backend
│
├── tool_call_parsers/          # Phase 2 client-side parsers
│   ├── zedclaw_parser.py        # ZedClaw/ChatML <tool_call> format
...
├── zedclaw_swe_env/             # SWE-bench training environment
│

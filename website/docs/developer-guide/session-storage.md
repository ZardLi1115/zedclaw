...
ZedClaw uses a SQLite database (`~/.zedclaw/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `zedclaw_state.py`


## Architecture Overview

```
~/.zedclaw/state.db (SQLite, WAL mode)
...
Multiple zedclaw processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.zedclaw/state.db
...
Default path: `~/.zedclaw/state.db`

This is derived from `zedclaw_constants.get_zedclaw_home()` which resolves to
`~/.zedclaw/` by default, or the value of `ZEDCLAW_HOME` environment variable.

...
This page covers all commands related to [ZedClaw profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `zedclaw profile` without a subcommand shows help.
...
```bash
zedclaw profile use <name>
```

Sets `<name>` as the active profile. All subsequent `zedclaw` commands (without `-p`) will use this profile.
...
zedclaw profile use default
```

## `zedclaw profile create`

```bash
zedclaw profile create <name> [options]
```
...
zedclaw profile create mybot

# Clone config only from current profile
zedclaw profile create work --clone

# Clone everything from current profile
zedclaw profile create backup --clone-all

# Clone config from a specific profile
zedclaw profile create work2 --clone --clone-from work
```

## `zedclaw profile delete`

```bash
zedclaw profile delete <name> [options]
```
...
```bash
zedclaw profile delete mybot
zedclaw profile delete mybot --yes
```
...
## `zedclaw profile show`

```bash
zedclaw profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's ZedClaw home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ zedclaw profile show work
Profile: work
Path:    ~/.zedclaw/profiles/work
...
## `zedclaw profile alias`

```bash
zedclaw profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your ZedClaw installation.
...
```bash
zedclaw profile alias work
# Creates/updates ~/.local/bin/work

zedclaw profile alias work --name mywork
# Creates ~/.local/bin/mywork

zedclaw profile alias work --remove
# Removes the wrapper script
```

## `zedclaw profile rename`

```bash
zedclaw profile rename <old-name> <new-name>
```
...
```bash
zedclaw profile rename mybot assistant
# ~/.zedclaw/profiles/mybot → ~/.zedclaw/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `zedclaw profile export`

```bash
zedclaw profile export <name> [options]
```
...
```bash
zedclaw profile export work
# Creates work.tar.gz in the current directory

zedclaw profile export work -o ./work-2026-03-29.tar.gz
```

## `zedclaw profile import`

```bash
zedclaw profile import <archive> [options]
```
...
```bash
zedclaw profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

zedclaw profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`zedclaw profile export` / `import` are still the right commands for
...
### `zedclaw profile install`

```bash
zedclaw profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `zedclaw -p telemetry`). |
...
zedclaw profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
zedclaw profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
zedclaw profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
zedclaw profile install ./telemetry/
```

### `zedclaw profile update`

```bash
zedclaw profile update <name> [--force-config] [--yes]
```
...
### `zedclaw profile info`

```bash
zedclaw profile info <name>
```

Prints the profile's distribution manifest — name, version, required
ZedClaw version, author, env var requirements, the source URL/path, and
...
`zedclaw profile list` also shows the distribution name and version in a
`Distribution` column, and `zedclaw profile show <name>` / `delete <name>`
...
zedclaw profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
zedclaw profile install https://github.com/your-org/internal-assistant.git
```
...
`zedclaw_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
ZedClaw version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host ZedClaw can clone from.
3. Tell recipients to run `zedclaw profile install <your-repo-url>`.
...
## `zedclaw -p` / `zedclaw --profile`

```bash
zedclaw -p <name> <command> [options]
zedclaw --profile <name> <command> [options]
```

Global flag to run any ZedClaw command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
zedclaw -p work chat -q "Check the server status"
zedclaw --profile dev gateway start
zedclaw -p personal skills list
zedclaw -p work config edit
```

## `zedclaw completion`

```bash
zedclaw completion <shell>
```
...
- `zedclaw profile <TAB>` — subcommands (list, use, create, etc.)
- `zedclaw profile use <TAB>` — profile names
- `zedclaw -p <TAB>` — profile names

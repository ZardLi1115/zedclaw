...
| `zedclaw auth` | Manage credentials — add, list, remove, reset, set strategy. Handles OAuth flows for Codex/Nous/Anthropic. |
| `zedclaw login` / `logout` | **Deprecated** — use `zedclaw auth` instead. |
...
| `zedclaw debug` | Debug tools — upload logs and system info for support. |
...
| `zedclaw curator` | Background skill maintenance — status, run, pause, pin. See [Curator](../user-guide/features/curator.md). |
...
| `zedclaw profile` | Manage profiles — multiple isolated ZedClaw instances. |
...
### `zedclaw -z <prompt>` — scripted one-shot

For programmatic callers (shell scripts, CI, cron, parent processes piping in a prompt), `zedclaw -z` is the purest one-shot entry point: **single prompt in, final response text out, nothing else on stdout or stderr.** No banner, no spinner, no tool previews, no `Session:` line — just the agent's final reply as plain text.
...
```bash
zedclaw -z "…" --provider openrouter --model openai/gpt-5.5
# or:
ZEDCLAW_INFERENCE_MODEL=anthropic/claude-sonnet-4.6 zedclaw -z "…"
```

Same agent, same tools, same skills — just strips every interactive / cosmetic layer. If you need tool output in the transcript too, use `zedclaw chat -q` instead; `-z` is explicitly for "I only want the final answer".

## `zedclaw model`

Interactive provider + model selector. **This is the command for adding new providers, setting up API keys, and running OAuth flows.** Run it from your terminal — not from inside an active ZedClaw chat session.
...
:::warning zedclaw model vs /model — know the difference
...
:::tip WSL users
Use `zedclaw gateway run` instead of `zedclaw gateway start` — WSL's systemd support is unreliable. Wrap it in tmux for persistence: `tmux new -s zedclaw 'zedclaw gateway run'`. See [WSL FAQ](/docs/reference/faq#wsl-gateway-keeps-disconnecting-or-zedclaw-gateway-start-fails) for details.
:::
...
| `--reconfigure` | Backwards-compat alias — bare `zedclaw setup` on an existing install now does this by default. |
...
zedclaw kanban boards create atm10-server --name "ATM10 Server" --icon 🎮
...
Board resolution order (highest precedence first): `--board <slug>` flag → `ZEDCLAW_KANBAN_BOARD` env var → `~/.zedclaw/kanban/current` file → `default`.

All actions are also available as a slash command in the gateway (`/kanban …`), with the same argument surface — including `boards` subcommands and the `--board` flag.

For the full design — comparison with Cline Kanban / Paperclip / NanoClaw / Gemini Enterprise, eight collaboration patterns, four user stories, concurrency correctness proof — see `docs/zedclaw-kanban-v1-spec.pdf` in the repository or the [Kanban user guide](/docs/user-guide/features/kanban).
...
Outputs a compact, plain-text summary of your entire ZedClaw setup. Designed to be copy-pasted into Discord, GitHub issues, or Telegram when asking for support — no ANSI colors, no special formatting, just data.
...
The report includes system info (OS, Python version, ZedClaw version), recent agent and gateway logs (512 KB limit per file), and redacted API key status. Keys are always redacted — no secrets are uploaded.
...
Inspect and manage the shadow git store at `~/.zedclaw/checkpoints/` — the storage layer behind the in-session `/rollback` command. Safe to run any time; does not require the agent to be running.
...
ZedClaw uses Python's `RotatingFileHandler`. Old logs are rotated automatically — look for `agent.log.1`, `agent.log.2`, etc. The `zedclaw logs list` subcommand shows all log files including rotated ones.
...
On a fresh install the first scheduled pass is deferred by one full `interval_hours` (7 days by default) — the gateway will not curate immediately on the first tick after `zedclaw update`. Use `zedclaw curator run --dry-run` to preview before that happens.
...
| Subcommand | Description |
|------------|-------------|
| `serve [-v\|--verbose]` | Run ZedClaw as an MCP server — expose conversations to other agents. |
...
Unified plugin management — general plugins, memory providers, and context engines in one place. Running `zedclaw plugins` with no subcommand opens a composite interactive screen with two sections:
...
Launch the web dashboard — a browser-based UI for managing configuration, API keys, and monitoring sessions. Requires `pip install zedclaw[web]` (FastAPI + Uvicorn). See [Web Dashboard](/docs/user-guide/features/web-dashboard) for full documentation.
...
Manage profiles — multiple isolated ZedClaw instances, each with its own config, sessions, skills, and home directory.
...
| `--backup` | Create a labeled pre-update snapshot of `ZEDCLAW_HOME` (config, auth, sessions, skills, pairing data) before pulling. Default is **off** — the previous always-backup behavior was adding minutes to every update on large homes. Flip it on permanently via `update.backup: true` in `config.yaml`. |

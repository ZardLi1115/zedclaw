...
description: "Run ZedClaw directly on an Android phone with Termux"
---

# ZedClaw on Android with Termux

This is the tested path for running ZedClaw directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop ZedClaw from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

ZedClaw now ships a Termux-aware installer path:
...
- links `zedclaw` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `zedclaw` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/zedclaw" "$PREFIX/bin/zedclaw"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `zedclaw` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.zedclaw/.env`.
...
### `zedclaw doctor` says ripgrep or Node is missing

...
sidebar_position: 2
title: "TUI"
description: "Launch the modern terminal UI for ZedClaw — mouse-friendly, rich overlays, and non-blocking input."
---

# TUI

The TUI is the modern front-end for ZedClaw — a terminal UI backed by the same Python runtime as the [Classic CLI](cli.md). Same agent, same sessions, same slash commands; a cleaner, more responsive surface for interacting with them.

It's the recommended way to run ZedClaw interactively.
...
zedclaw --tui -c
zedclaw --tui --continue

# Resume a specific session by ID or title
zedclaw --tui -r 20260409_000000_aa11bb
zedclaw --tui --resume "my t0p session"
...
```bash
export ZEDCLAW_TUI=1
zedclaw          # now uses the TUI
zedclaw chat     # same
```
...
- **Instant first frame** — the banner paints before the app finishes loading, so the terminal never feels frozen while ZedClaw is starting.
...
- **Node.js** ≥ 20 — the TUI runs as a subprocess launched from the Python CLI. `zedclaw doctor` verifies this.
- **TTY** — like the classic CLI, piping stdin or running in non-interactive environments falls back to single-query mode.

On first launch ZedClaw installs the TUI's Node dependencies into `ui-tui/node_modules` (one-time, a few seconds). Subsequent launches are fast. If you pull a new ZedClaw version, the TUI bundle is rebuilt automatically when sources are newer than the dist.

### External prebuild

Distributions that ship a prebuilt bundle (Nix, system packages) can point ZedClaw at it:

```bash
export ZEDCLAW_TUI_DIR=/path/to/prebuilt/ui-tui
zedclaw --tui
```
...
| `/reload` | Re-reads `~/.zedclaw/.env` into the running TUI process so newly added API keys take effect without a restart |
...
1. `ZEDCLAW_TUI_THEME` env var — highest priority. Values: `light`, `dark`, or a raw 6-char background hex (e.g. `ffffff`, `1a1a2e`).
...
```bash
export ZEDCLAW_TUI_THEME=light
```

## Busy indicator styles

The status-bar busy indicator is pluggable — the default rotates ZedClaw' kawaii face palette every 2.5 seconds during agent work. Pick a different style via config or the `/indicator` slash command:
...
By default, `zedclaw --tui` starts a fresh session each launch. To re-attach to the most recent TUI session automatically (useful when your terminal or SSH connection drops unexpectedly), opt in:

```bash
export ZEDCLAW_TUI_RESUME=1          # most-recent TUI session
# or:
export ZEDCLAW_TUI_RESUME=<session-id>   # specific session
```
...
- **Working directory with git branch** — `~/projects/zedclaw (docs/two-week-gap-sweep)`. The branch suffix updates when you `git checkout` in a side terminal (mtime-cached) so the TUI reflects your actual active branch, not whatever it was at launch.
...
The TUI respects all standard ZedClaw config: `~/.zedclaw/config.yaml`, profiles, personalities, skins, quick commands, credential pools, memory providers, tool/skill enablement. No TUI-specific config file exists.
...
Sessions are shared between the TUI and the classic CLI — both write to the same `~/.zedclaw/state.db`. You can start a session in one, resume in the other. The session picker surfaces sessions from both sources, with a source tag.

See [Sessions](sessions.md) for lifecycle, search, compression, and export.

## Reverting to the classic CLI

Launching `zedclaw` (without `--tui`) stays on the classic CLI. To make a machine prefer the TUI, set `ZEDCLAW_TUI=1` in your shell profile. To go back, unset it.

If the TUI fails to launch (no Node, missing bundle, TTY issue), ZedClaw prints a diagnostic and falls back — rather than leaving you stuck.

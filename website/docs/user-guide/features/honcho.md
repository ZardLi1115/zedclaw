...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of ZedClaw's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple ZedClaw instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
zedclaw memory setup    # select "honcho" from the provider list
```
...
```bash
echo 'HONCHO_API_KEY=***' >> ~/.zedclaw/.env
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$ZEDCLAW_HOME/honcho.json` (profile-local). The setup wizard handles this for you.
...
- `per-session` — each `zedclaw` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — ZedClaw syncs them back at session init.
...
The `zedclaw honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). Run `zedclaw memory setup` and pick Honcho first; the subcommand appears on the next invocation.

```bash
zedclaw honcho status          # Connection status, config, and key settings
zedclaw honcho setup           # Redirects to `zedclaw memory setup`
zedclaw honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
zedclaw honcho peer            # Show or update peer names + dialectic reasoning level
zedclaw honcho mode            # Show or set recall mode (hybrid/context/tools)
zedclaw honcho tokens          # Show or set token budget for context and dialectic
zedclaw honcho identity        # Seed or show the AI peer's Honcho identity
zedclaw honcho sync            # Sync Honcho config to all existing profiles
zedclaw honcho peers           # Show peer identities across all profiles
zedclaw honcho sessions        # List known Honcho session mappings
zedclaw honcho map             # Map current directory to a Honcho session name
zedclaw honcho enable          # Enable Honcho for the active profile
zedclaw honcho disable         # Disable Honcho for the active profile
zedclaw honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `zedclaw honcho`

If you previously used the standalone `zedclaw honcho setup`:
...
No re-login or re-setup needed. Run `zedclaw memory setup` and select "honcho" — the wizard detects your existing config.

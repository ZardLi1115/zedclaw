...
description: "Customize ZedClaw's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

ZedClaw's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `ZEDCLAW_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who ZedClaw is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

ZedClaw now seeds a default `SOUL.md` automatically in:

```text
~/.zedclaw/SOUL.md
```

More precisely, it uses the current instance's `ZEDCLAW_HOME`, so if you run ZedClaw with a custom home directory, it will use:

```text
$ZEDCLAW_HOME/SOUL.md
```

### Important behavior

- **SOUL.md is the agent's primary identity.** It occupies slot #1 in the system prompt, replacing the hardcoded default identity.
- ZedClaw creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- ZedClaw loads `SOUL.md` only from `ZEDCLAW_HOME`
- ZedClaw does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, ZedClaw falls back to a built-in default identity
...
If ZedClaw loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `ZEDCLAW_HOME`, the personality belongs to the ZedClaw instance itself.

That also makes it easier to teach users:
- "Edit `~/.zedclaw/SOUL.md` to change ZedClaw' default personality."
...
- how ZedClaw should handle uncertainty, disagreement, or ambiguity
...
## What ZedClaw injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, ZedClaw falls back to a built-in default identity ("You are ZedClaw, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
ZedClaw ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain ZedClaw, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives ZedClaw its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.zedclaw/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.zedclaw/SOUL.md`
...
- [SOUL.md Guide](/docs/guides/use-soul-with-zedclaw)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how ZedClaw speaks
- `display.skin` and `/skin` affect how ZedClaw looks in the terminal

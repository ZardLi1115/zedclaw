...
description: "Durable SQLite-backed task board for coordinating multiple ZedClaw profiles"
---
...
ZedClaw Kanban is a durable task board, shared across all your ZedClaw profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.zedclaw/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.

### Two surfaces: the model talks through tools, you talk through the CLI

The board has two front doors, both backed by the same `~/.zedclaw/kanban.db`:

- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_create`, `kanban_link`, `kanban_unblock`. The dispatcher spawns each worker with these tools already in its schema; orchestrator profiles can also enable the `kanban` toolset explicitly. The model reads and routes tasks by calling tools directly, *not* by shelling out to `zedclaw kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
- **You (and scripts, and cron) drive the board through `zedclaw kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
For the full design rationale, comparative analysis against Cline Kanban / Paperclip / NanoClaw / Google Gemini Enterprise, and the eight canonical collaboration patterns, see `docs/zedclaw-kanban-v1-spec.pdf` in the repository.
...
- **Workspace** — the directory a worker operates in. Three kinds:
  - `scratch` (default) — fresh tmp dir under `~/.zedclaw/kanban/workspaces/<id>/` (or `~/.zedclaw/kanban/boards/<slug>/workspaces/<id>/` on non-default boards).
...
- **Dispatcher** — a long-lived loop that, every N seconds (default 60): reclaims stale claims, reclaims crashed workers (PID gone but TTL not yet expired), promotes ready tasks, atomically claims, spawns assigned profiles. Runs **inside the gateway** by default (`kanban.dispatch_in_gateway: true`). One dispatcher sweeps all boards per tick; workers are spawned with `ZEDCLAW_KANBAN_BOARD` pinned so they can't see other boards. After `kanban.failure_limit` consecutive spawn failures on the same task (default: 2) the dispatcher auto-blocks it with the last error as the reason — prevents thrashing on tasks whose profile doesn't exist, workspace can't mount, etc.
...
zedclaw kanban boards list

# Create a new board.
zedclaw kanban boards create atm10-server \
    --name "ATM10 Server" \
...
zedclaw kanban --board atm10-server list
zedclaw kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
zedclaw kanban boards switch atm10-server
zedclaw kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
zedclaw kanban boards rename atm10-server "ATM10 (Prod)"
...
2. `ZEDCLAW_KANBAN_BOARD` env var (set by the dispatcher when spawning a
   worker, so workers can't see other boards).
3. `~/.zedclaw/kanban/current` — the slug persisted by `zedclaw kanban
   boards switch`.
4. `default`.
...
`zedclaw dashboard` → Kanban tab shows a board switcher at the top as soon
...
zedclaw gateway start

# 3. Create a task (you — or an orchestrator agent via kanban_create)
zedclaw kanban create "research AI funding landscape" --assignee researcher

# 4. Watch activity live (you)
zedclaw kanban watch

# 5. See the board (you)
zedclaw kanban list
zedclaw kanban stats
```

When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `zedclaw kanban show t_abcd`.
...
Override the config flag at runtime via `ZEDCLAW_KANBAN_DISPATCH_IN_GATEWAY=0`
for debugging. Standard gateway supervision applies: run `zedclaw gateway
...
until one comes up — `zedclaw kanban create` warns about this at creation
time.

Running `zedclaw kanban daemon` as a separate process is **deprecated**;
...
zedclaw kanban create "nightly ops review" \
    --assignee ops \
...
```bash
zedclaw kanban complete t_abc t_def t_hij --result "batch wrap"
zedclaw kanban archive  t_abc t_def t_hij
zedclaw kanban unblock  t_abc t_def
zedclaw kanban block    t_abc "need input" --ids t_def t_hij
```

## How workers interact with the board

**Workers do not shell out to `zedclaw kanban`.** When the dispatcher spawns a worker it sets `ZEDCLAW_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema. The same toolset is also available to orchestrator profiles that enable `kanban` in their toolsets config. These tools read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `zedclaw kanban` CLI.
...
kanban_show()                                     # no args — uses ZEDCLAW_KANBAN_TASK
...
### Why tools instead of shelling to `zedclaw kanban`

Three reasons:

1. **Backend portability.** Workers whose terminal tool points at a remote backend (Docker / Modal / Singularity / SSH) would run `zedclaw kanban complete` *inside* the container, where `zedclaw` isn't installed and `~/.zedclaw/kanban.db` isn't mounted. The kanban tools run in the agent's own Python process and always reach `~/.zedclaw/kanban.db` regardless of terminal backend.
...
**Zero schema footprint on normal sessions.** A regular `zedclaw chat` session has zero `kanban_*` tools in its schema. The `check_fn` on each tool only returns True when `ZEDCLAW_KANBAN_TASK` is set, which only happens when the dispatcher spawned this process. No tool bloat for users who never touch kanban.
...
```json
{
  ...
  "verification": ["pytest tests/zedclaw_cli/test_kanban_db.py -q"],
...
2. `cd $ZEDCLAW_KANBAN_WORKSPACE` (via the terminal tool) and do the work there.
...
```bash
zedclaw -p <your-worker-profile> skills list | grep kanban-worker
```

If the bundled copy is missing, restore it for that profile:

```bash
zedclaw -p <your-worker-profile> skills reset kanban-worker --restore
```
...
```bash
zedclaw kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

zedclaw kanban create "audit auth flow" \
    --assignee reviewer \
...
```bash
zedclaw -p orchestrator skills list | grep kanban-orchestrator
```

If the bundled copy is missing, restore it for that profile:

```bash
zedclaw -p orchestrator skills reset kanban-orchestrator --restore
```
...
- A **Kanban** tab showing one column per status: `triage`, `todo`, `ready`, `running`, `blocked`, `done` (plus `archived` when the toggle is on).
  - `triage` is the parking column for rough ideas a specifier is expected to flesh out. Tasks created with `zedclaw kanban create --triage` (or via the Triage column's inline create) land here and the dispatcher leaves them alone until a human or specifier promotes them to `todo` / `ready`. Run `zedclaw kanban specify <id>` to have the auxiliary LLM expand a triage task into a concrete spec (title + body with goal, approach, acceptance criteria) and promote it to `todo` in one shot; `--all` sweeps every triage task at once. Configure which model runs the specifier under `auxiliary.triage_specifier` in `config.yaml`.
...
└──────────┬─────────────┘                                    │
           ...
           ▼                                                  │
┌────────────────────────┐                                    │
│  ~/.zedclaw/kanban.db   │ ───── append task_events ──────────┘
│  (WAL, shared)         │
└────────────────────────┘
```
...
The plugin uses the standard ZedClaw dashboard plugin contract — see [Extending the Dashboard](./extending-the-dashboard) for the full manifest reference, shell slots, page-scoped slots, and the Plugin SDK. Extra columns, custom card chrome, tenant-filtered layouts, or full `tab.override` replacements are all expressible without forking this plugin.
...
```
zedclaw kanban init                                     # create kanban.db + print daemon hint
zedclaw kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
zedclaw kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived] [--json]
zedclaw kanban show <id> [--json]
zedclaw kanban assign <id> <profile>                    # or 'none' to unassign
zedclaw kanban link <parent_id> <child_id>
zedclaw kanban unlink <parent_id> <child_id>
zedclaw kanban claim <id> [--ttl SECONDS]
zedclaw kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
zedclaw kanban complete <id>... [--result "..."]
zedclaw kanban block <id> "<reason>" [--ids <id>...]
zedclaw kanban unblock <id>...
zedclaw kanban archive <id>...

zedclaw kanban tail <id>                                # follow a single task's event stream
zedclaw kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
zedclaw kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
zedclaw kanban runs <id> [--json]                       # attempt history (one row per run)
zedclaw kanban assignees [--json]                       # profiles on disk + per-assignee task counts
zedclaw kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
zedclaw kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `zedclaw gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
zedclaw kanban stats [--json]                           # per-status + per-assignee counts
zedclaw kanban log <id> [--tail BYTES]                  # worker log from ~/.zedclaw/kanban/logs/
zedclaw kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
zedclaw kanban notify-list [<id>] [--json]
zedclaw kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
zedclaw kanban context <id>                             # what a worker sees
zedclaw kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
zedclaw kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
Every `zedclaw kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `zedclaw chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `zedclaw_cli.kanban.run_slash()` entry point that reuses the `zedclaw kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `zedclaw kanban`. You don't have to leave the chat to drive the board.
...
The gateway normally queues slash commands and user messages while an agent is still thinking — that's what stops you from accidentally starting a second turn while the first is in flight. **`/kanban` is explicitly exempted from this guard.** The board lives in `~/.zedclaw/kanban.db`, not in the running agent's state, so reads (`list`, `show`, `context`, `tail`, `watch`, `stats`, `runs`) and writes (`comment`, `unblock`, `block`, `assign`, `archive`, `create`, `link`, …) all go through immediately, even mid-turn.
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`zedclaw kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `zedclaw kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |

For worked examples of each, see `docs/zedclaw-kanban-v1-spec.pdf`.
...
```bash
zedclaw kanban create "monthly report" \
    --assignee researcher \
    --tenant business-a \
    --workspace dir:~/tenants/business-a/data/
```

Workers receive `$ZEDCLAW_TENANT` and namespace their memory writes by prefix. The board, the dispatcher, and the profile definitions are all shared; only the data is scoped.
...
```bash
zedclaw kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
zedclaw kanban notify-list
zedclaw kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
```bash
zedclaw kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
zedclaw kanban runs t_abcd
...
**Bulk close caveat.** `zedclaw kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`zedclaw kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `zedclaw kanban heartbeat $TASK` to signal liveness during long operations. |
...
`zedclaw kanban tail <id>` shows these for a single task. `zedclaw kanban watch` streams them board-wide.

## Out of scope

Kanban is deliberately single-host. `~/.zedclaw/kanban.db` is a local SQLite file and the dispatcher spawns workers on the same machine. Running a shared board across two hosts is not supported — there's no coordination primitive for "worker X on host A, worker Y on host B," and the crash-detection path assumes PIDs are host-local. If you need multi-host, run an independent board per host and use `delegate_task` / a message queue to bridge them.

## Design spec

The complete design — architecture, concurrency correctness, comparison with other systems, implementation plan, risks, open questions — lives in `docs/zedclaw-kanban-v1-spec.pdf`. Read that before filing any behavior-change PR.

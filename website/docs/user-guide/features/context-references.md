...
Type `@` followed by a reference to inject content directly into your message. ZedClaw expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- ZedClaw env: `$ZEDCLAW_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$ZEDCLAW_HOME/skills/.hub/`

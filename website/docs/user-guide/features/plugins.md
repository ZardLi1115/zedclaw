...
title: "Plugins"
description: "Extend ZedClaw with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

ZedClaw has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](/docs/developer-guide/adding-tools) page is for built-in ZedClaw
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a ZedClaw Plugin](/docs/guides/build-a-zedclaw-plugin)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.zedclaw/plugins/` with a `plugin.yaml` and Python code:

```
~/.zedclaw/plugins/my-plugin/
...
Start ZedClaw — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.zedclaw/plugins/hello-world/plugin.yaml`**
...
**`~/.zedclaw/plugins/hello-world/__init__.py`**

```python
"""Minimal ZedClaw plugin — registers a tool and a hook."""
...
Drop both files into `~/.zedclaw/plugins/hello-world/`, restart ZedClaw, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

Project-local plugins under `./.zedclaw/plugins/` are disabled by default. Enable them only for trusted repositories by setting `ZEDCLAW_ENABLE_PROJECT_PLUGINS=true` before starting ZedClaw.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `zedclaw <plugin> <subcommand>` |
| Inject messages | `ctx.inject_message(content, role="user")` — see [Injecting Messages](#injecting-messages) |
| Ship data files | `Path(__file__).parent / "data" / "file.yaml"` |
| Bundle skills | `ctx.register_skill(name, path)` — namespaced as `plugin:skill`, loaded via `skill_view("plugin:skill")` |
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `zedclaw plugins install` |
| Distribute via pip | `[project.entry-points."zedclaw_agent.plugins"]` |
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with ZedClaw — see [Built-in Plugins](/docs/user-guide/features/built-in-plugins) |
| User | `~/.zedclaw/plugins/` | Personal plugins |
| Project | `.zedclaw/plugins/` | Project-specific plugins (requires `ZEDCLAW_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `zedclaw_agent.plugins` entry_points | Distributed packages |
| Nix | `services.zedclaw.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](/docs/getting-started/nix-setup#plugins) |

Later sources override earlier ones on name collision, so a user plugin with the same name as a bundled plugin replaces it.

### Plugin sub-categories

Within each source, ZedClaw also recognizes sub-category directories that route plugins to specialized discovery systems:
...
User plugins at `~/.zedclaw/plugins/model-providers/<name>/` and `~/.zedclaw/plugins/memory/<name>/` override bundled plugins of the same name — last-writer-wins in `register_provider()` / `register_memory_provider()`. Drop a directory in, and it replaces the built-in without any repo edits.

## Plugins are opt-in (with a few exceptions)

**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `zedclaw plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.zedclaw/config.yaml`. This stops third-party code from running without your explicit consent.
...
```bash
zedclaw plugins                    # interactive toggle (space to check/uncheck)
zedclaw plugins enable <name>      # add to allow-list
zedclaw plugins disable <name>     # remove from allow-list + add to disabled
```

After `zedclaw plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

### What the allow-list does NOT gate

Several categories of plugin bypass `plugins.enabled` — they're part of ZedClaw' built-in surface and would break basic functionality if gated off by default:
...
| **User-installed platforms** (under `~/.zedclaw/plugins/platforms/`) | Opt-in via `plugins.enabled` — third-party gateway adapters need explicit consent. |

In short: **bundled "always-works" infrastructure loads automatically; third-party general plugins are opt-in.** The `plugins.enabled` allow-list is the gate specifically for arbitrary code a user drops into `~/.zedclaw/plugins/`.

### Migration for existing users

When you upgrade to a version of ZedClaw that has opt-in plugins (config schema v21+), any user plugins already installed under `~/.zedclaw/plugins/` that weren't already in `plugins.disabled` are **automatically grandfathered** into `plugins.enabled`. Your existing setup keeps working. Bundled standalone plugins are NOT grandfathered — even existing users have to opt in explicitly. (Bundled platform/backend plugins never needed grandfathering because they were never gated.)
...
ZedClaw has four kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.zedclaw/plugins/` |
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and ZedClaw also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a ZedClaw Plugin](/docs/guides/build-a-zedclaw-plugin) · [Adding Tools](/docs/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/docs/user-guide/features/hooks) · [Build a ZedClaw Plugin](/docs/guides/build-a-zedclaw-plugin) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a ZedClaw Plugin](/docs/guides/build-a-zedclaw-plugin) · [Extending the CLI](/docs/developer-guide/extending-the-cli) |
| A **subcommand** for `zedclaw <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/docs/developer-guide/extending-the-cli) |
...
| An **STT backend** (custom whisper binary, local ASR CLI) | Config-driven — set `ZEDCLAW_LOCAL_STT_COMMAND` env var to a shell template | [Voice Message Transcription (STT)](/docs/user-guide/features/tts#voice-message-transcription-stt) |
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. ZedClaw auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/docs/user-guide/features/mcp) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `zedclaw skills tap add <repo>` | [Skills Hub](/docs/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/docs/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.zedclaw/hooks/<name>/` | [Event Hooks](/docs/user-guide/features/hooks#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `zedclaw plugins install` needed. See the **[Nix Setup guide](/docs/getting-started/nix-setup#plugins)** for full details.
...
```bash
zedclaw plugins                               # unified interactive UI
zedclaw plugins list                          # table: enabled / disabled / not enabled
zedclaw plugins install user/repo             # install from Git, then prompt Enable? [y/N]
zedclaw plugins install user/repo --enable    # install AND enable (no prompt)
zedclaw plugins install user/repo --no-enable # install but leave disabled (no prompt)
zedclaw plugins update my-plugin              # pull latest
zedclaw plugins remove my-plugin              # uninstall
zedclaw plugins enable my-plugin              # add to allow-list
zedclaw plugins disable my-plugin             # remove from allow-list + add to disabled
```

### Interactive UI

Running `zedclaw plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `zedclaw plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
...
See the **[full guide](/docs/guides/build-a-zedclaw-plugin)** for handler contracts, schema format, hook behavior, error handling, and common mistakes.

...
Schedule tasks to run automatically with natural language or cron expressions. ZedClaw exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
All of this is available to ZedClaw itself through the `cronjob` tool, so you can create, pause, edit, and remove jobs by asking in plain language — no CLI required.

:::warning
Cron-run sessions cannot recursively create more cron jobs. ZedClaw disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
zedclaw cron create "every 2h" "Check server status"
zedclaw cron create "every 1h" "Summarize new feed items" --skill blogwatcher
zedclaw cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
Ask ZedClaw normally:

```text
Every morning at 9am, check Hacker News for AI news and send me a summary on Telegram.
```

ZedClaw will use the unified `cronjob` tool internally.
...
zedclaw cron create "every 1d at 09:00" \
...
```bash
zedclaw cron edit <job_id> --schedule "every 4h"
zedclaw cron edit <job_id> --prompt "Use the revised task"
zedclaw cron edit <job_id> --skill blogwatcher --skill maps
zedclaw cron edit <job_id> --add-skill maps
zedclaw cron edit <job_id> --remove-skill blogwatcher
zedclaw cron edit <job_id> --clear-skills
```
...
```bash
zedclaw cron list
zedclaw cron pause <job_id>
zedclaw cron resume <job_id>
zedclaw cron run <job_id>
zedclaw cron remove <job_id>
zedclaw cron status
zedclaw cron tick
```
...
```bash
zedclaw gateway install     # Install as a user service
sudo zedclaw gateway install --system   # Linux: boot-time system service for servers
zedclaw gateway             # Or run in foreground
...
On each tick ZedClaw:

1. loads jobs from `~/.zedclaw/cron/jobs.json`
...
A file lock at `~/.zedclaw/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
| `"local"` | Save to local files only (`~/.zedclaw/cron/output/`) | Default on CLI |
...
If the agent's final response starts with `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.zedclaw/cron/output/`), but no message is sent to the delivery target.
...
Or set the `ZEDCLAW_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 120s default.
...
`.sh` / `.bash` files run under `/bin/bash`; anything else under the current Python interpreter (`sys.executable`). Scripts must live in `~/.zedclaw/scripts/` (same sandboxing rule as the pre-run script gate).

### The agent sets these up for you

The `cronjob` tool's schema exposes `no_agent` to ZedClaw directly, so you can describe a watchdog in chat and let the agent wire it up:

```text
Ping me on Telegram if RAM is over 85%, every 5 minutes.
```

ZedClaw will write the check script to `~/.zedclaw/scripts/` via `write_file`, then call:
...
cronjob(
    action="create",
    prompt="Fetch the top 10 AI/ML stories from Hacker News. Save them to ~/.zedclaw/data/briefs/raw.md in markdown format with title, URL, and score.",
    schedule="0 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.zedclaw/data/briefs/raw.md. Score each story 1–10 for engagement potential and novelty. Output the top 5 to ~/.zedclaw/data/briefs/ranked.md.",
    schedule="30 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.zedclaw/data/briefs/ranked.md. Write 3 tweet drafts (hook + body + hashtags). Deliver to telegram:7976161601.",
    schedule="0 8 * * *",
...
- When Job 2 fires, ZedClaw reads Job 1's most recent output from `~/.zedclaw/cron/output/{job1_id}/*.md`
...
The agent's final response is automatically delivered — you do **not** need to include `send_message` in the cron prompt for that same destination. If a cron run calls `send_message` to the exact target the scheduler will already deliver to, ZedClaw skips that duplicate send and tells the model to put the user-facing content in the final response instead. Use `send_message` only for additional or different targets.
...
Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `zedclaw tools`** — not the CLI default, not everything under the sun.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `zedclaw tools` cron-platform config wins; otherwise ZedClaw falls back to the built-in defaults. This matters for cost control: carrying `moa`, `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.

### Skipping the agent entirely: `wakeAgent`

If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether ZedClaw should even invoke the agent. Emit a final stdout line of the form:
...
Jobs are stored in `~/.zedclaw/cron/jobs.json`. Output from job runs is saved to `~/.zedclaw/cron/output/{job_id}/{timestamp}.md`.

Jobs may store `model` and `provider` as `null`. When those fields are omitted, ZedClaw resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.

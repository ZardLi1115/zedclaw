...
ZedClaw has three layers of resilience that keep your sessions running when providers hit issues:
...
When your main LLM provider encounters errors — rate limits, server overload, auth failures, connection drops — ZedClaw can automatically switch to a backup provider:model pair mid-session without losing your conversation.
...
`zedclaw fallback` reuses the provider picker from `zedclaw model` — same provider list, same credential prompts, same validation. Use the subcommands `add`, `list` (alias `ls`), `remove` (alias `rm`), and `clear` to manage the chain. Changes persist under the top-level `fallback_providers:` list in `config.yaml`.

If you'd rather edit the YAML directly, add a `fallback_model` section to `~/.zedclaw/config.yaml`:
...
`fallback_model` (singular) is the legacy single-fallback key — ZedClaw still honors it for back-compat. `fallback_providers` (plural, list) supports multiple fallbacks tried in order; `zedclaw fallback` writes to this key. When both are set, ZedClaw merges them with `fallback_providers` taking priority.
:::
...
| Nous Portal | `nous` | `zedclaw auth` (OAuth) |
| OpenAI Codex | `openai-codex` | `zedclaw model` (ChatGPT OAuth) |
...
| Google Gemini (OAuth) | `google-gemini-cli` | `zedclaw model` (Google OAuth; optional: `ZEDCLAW_GEMINI_PROJECT_ID`) |
...
| Qwen Portal (OAuth) | `qwen-oauth` | `zedclaw model` (Qwen Portal OAuth; optional: `ZEDCLAW_QWEN_BASE_URL`) |
| MiniMax (OAuth) | `minimax-oauth` | `zedclaw model` (MiniMax portal OAuth) |
...
ZedClaw uses separate lightweight models for side tasks. Each task has its own provider resolution chain that acts as a built-in fallback system.
...
| Triage Specifier | `zedclaw kanban specify` / dashboard ✨ button — fleshes out a one-liner triage task into a real spec | `auxiliary.triage_specifier` |

### Auto-Detection Chain

When a task's provider is set to `"auto"` (the default), ZedClaw tries providers in order until one works:
...
If the resolved provider fails at call time, ZedClaw also has an internal retry: if the provider is not OpenRouter and no explicit `base_url` is set, it tries OpenRouter as a last-resort fallback.
...
For `auxiliary.session_search`, ZedClaw also supports:
...
| `"codex"` | Force Codex OAuth | `zedclaw model` → Codex |
...
`base_url` takes precedence over `provider`. ZedClaw uses the configured `api_key` for authentication, falling back to `OPENAI_API_KEY` if not set. It does **not** reuse `OPENROUTER_API_KEY` for custom endpoints.
...
If no provider is available for compression, ZedClaw drops middle conversation turns without generating a summary rather than failing the session.

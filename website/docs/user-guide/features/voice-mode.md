...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with ZedClaw — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

ZedClaw supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with ZedClaw](/docs/guides/use-voice-mode-with-zedclaw).

## Prerequisites

Before using voice features, make sure you have:

1. **ZedClaw installed** — `pip install zedclaw` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `zedclaw model` or set your preferred provider credentials in `~/.zedclaw/.env`
3. **A working base setup** — run `zedclaw` to verify the agent responds to text before enabling voice

:::tip
The `~/.zedclaw/` directory and default `config.yaml` are created automatically the first time you run `zedclaw`. You only need to create `~/.zedclaw/.env` manually for API keys.
:::
...
pip install "zedclaw[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
pip install "zedclaw[messaging]"

# Premium TTS (ElevenLabs)
pip install "zedclaw[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "zedclaw[all]"
```
...
Voice mode is available in both the **classic CLI** (`zedclaw chat`) and the **TUI** (`zedclaw --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.zedclaw/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
1. Start the CLI with `zedclaw` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.zedclaw/config.yaml` (default: `ctrl+b`).
:::
...
```bash
zedclaw gateway        # Start the gateway (connects to configured platforms)
zedclaw gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@zedclawbyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.zedclaw/.env`:
```bash
...
```bash
zedclaw gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.zedclaw/.env`
...
- Review gateway logs: `tail -f ~/.zedclaw/logs/gateway.log`

...
ZedClaw runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. ZedClaw
...
1. ZedClaw captures a baseline of current diagnostics for the file.
...
…). ZedClaw auto-detects the binary on PATH or in
`<ZEDCLAW_HOME>/lsp/bin/`.
...
`node_modules` tree — ZedClaw installs both packages together when you
run `zedclaw lsp install typescript` or auto-install fires on first
use.

## CLI

```
zedclaw lsp status          # service state + per-server install status
zedclaw lsp list            # registry, optionally --installed-only
zedclaw lsp install <id>    # eagerly install one server
zedclaw lsp install-all     # try every server with a known recipe
zedclaw lsp restart         # tear down running clients
zedclaw lsp which <id>      # print resolved binary path
```

`zedclaw lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <ZEDCLAW_HOME>/lsp/bin
...
When `install_strategy: auto`, ZedClaw installs binaries into
`<ZEDCLAW_HOME>/lsp/bin/`. NPM packages land in
`<ZEDCLAW_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully ZedClaw-owned and is
...
Servers are kept alive for the life of the ZedClaw process. There's
...
**`zedclaw lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<ZEDCLAW_HOME>/lsp/bin/`. Run
`zedclaw lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `zedclaw lsp status`**
...
When `zedclaw lsp status` shows a `Backend warnings` section, install
...
`~/.zedclaw/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.zedclaw/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `zedclaw lsp restart` to clear the set;

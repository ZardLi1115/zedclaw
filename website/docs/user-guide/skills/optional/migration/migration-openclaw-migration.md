---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into ZedClaw"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into ZedClaw"
---
...
Migrate a user's OpenClaw customization footprint into ZedClaw. Imports ZedClaw-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `zedclaw skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | ZedClaw (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `ZedClaw`, `Memory`, `Persona`, `Import` |
| Related skills | [`zedclaw`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-zedclaw) |
...
# OpenClaw -> ZedClaw Migration

Use this skill when a user wants to move their OpenClaw setup into ZedClaw with minimal manual cleanup.
...
```bash
zedclaw claw migrate              # Full interactive migration
zedclaw claw migrate --dry-run    # Preview what would be migrated
zedclaw claw migrate --preset user-data   # Migrate without secrets
zedclaw claw migrate --overwrite  # Overwrite existing conflicts
zedclaw claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `zedclaw setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_zedclaw.py` to:

- import `SOUL.md` into the ZedClaw home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into ZedClaw memory entries
- merge OpenClaw command approval patterns into ZedClaw `command_allowlist`
- migrate ZedClaw-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.zedclaw/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen ZedClaw workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.zedclaw/tts/`
- archive non-secret docs that do not have a direct ZedClaw destination
...
- `scripts/openclaw_to_zedclaw.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.zedclaw/skills/migration/openclaw-migration/scripts/openclaw_to_zedclaw.py`

Do not guess a shorter path like `~/.zedclaw/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.zedclaw/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of ZedClaw-compatible secrets, currently:
...
ZedClaw CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in ZedClaw. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a ZedClaw workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing ZedClaw skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.zedclaw/skills/migration/openclaw-migration/scripts/openclaw_to_zedclaw.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.zedclaw/skills/migration/openclaw-migration/scripts/openclaw_to_zedclaw.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.zedclaw/skills/migration/openclaw-migration/scripts/openclaw_to_zedclaw.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.zedclaw/skills/migration/openclaw-migration/scripts/openclaw_to_zedclaw.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.zedclaw/skills/migration/openclaw-migration/scripts/openclaw_to_zedclaw.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.zedclaw/skills/migration/openclaw-migration/scripts/openclaw_to_zedclaw.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of ZedClaw unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty ZedClaw targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean ZedClaw destination. Unsupported auth blobs must still be reported as skipped.
...
- ZedClaw persona state imported
- ZedClaw memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.zedclaw/skills/openclaw-imports/`

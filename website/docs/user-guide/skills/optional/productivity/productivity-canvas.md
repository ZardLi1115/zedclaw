...
| | |
|---|---|
| Source | Optional — install with `zedclaw skills install official/productivity/canvas` |
...
4. Name the token (e.g., "ZedClaw"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.zedclaw/.env`:
...
```bash
CANVAS="python $ZEDCLAW_HOME/skills/productivity/canvas/scripts/canvas_api.py"

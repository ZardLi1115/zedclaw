...
| | |
|---|---|
| Source | Optional — install with `zedclaw skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${ZEDCLAW_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${ZEDCLAW_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${ZEDCLAW_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client zedclaw
```
...
```bash
PUBLISH="${ZEDCLAW_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client zedclaw
```
...
```bash
DRIVE="${ZEDCLAW_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `zedclaw`)    |

...
| | |
|---|---|
| Source | Optional — install with `zedclaw skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into ZedClaw or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to ZedClaw. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.zedclaw/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.zedclaw/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is ZedClaw integration, either:

- configure the server in `~/.zedclaw/config.yaml` using the `native-mcp` skill, or
...
### ZedClaw cannot see the deployed server

The server-building part may be correct while the ZedClaw config is not. Load the `native-mcp` skill and configure the server in `~/.zedclaw/config.yaml`, then restart ZedClaw.

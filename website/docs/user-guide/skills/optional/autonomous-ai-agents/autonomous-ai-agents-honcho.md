...
description: "Configure and use Honcho memory with ZedClaw -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with ZedClaw -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `zedclaw skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for ZedClaw

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every ZedClaw profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/zedclaw#running-honcho-locally-with-zedclaw

### Verify

```bash
zedclaw honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this ZedClaw profile's AI peer
...
Honcho models conversations as interactions between **peers**. ZedClaw creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this ZedClaw instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each ZedClaw run |
| `global` | Single session across all directories |

Manual override: `zedclaw honcho map my-project-name`
...
Each ZedClaw profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block zedclaw.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `zedclaw.coder` host block in `honcho.json`
...
```bash
zedclaw honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for ZedClaw when Honcho memory is active.
...
Config file: `$ZEDCLAW_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `zedclaw honcho setup`. Ensure `memory.provider: honcho` is in `~/.zedclaw/config.yaml`.

### Memory not persisting across sessions
Check `zedclaw honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `zedclaw profile create <name> --clone`. For existing profiles: `zedclaw honcho sync`.
...
| Command | Description |
|---------|-------------|
| `zedclaw honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `zedclaw honcho status` | Show resolved config, connection test, peer info for active profile |
| `zedclaw honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `zedclaw honcho disable` | Disable Honcho for the active profile |
| `zedclaw honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `zedclaw honcho peers` | Show peer identities across all profiles |
| `zedclaw honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `zedclaw honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `zedclaw honcho sessions` | List known directory-to-session-name mappings |
| `zedclaw honcho map <name>` | Map current working directory to a Honcho session name |
| `zedclaw honcho identity` | Seed AI peer identity or show both peer representations |
| `zedclaw honcho sync` | Create host blocks for all ZedClaw profiles that don't have one yet |
| `zedclaw honcho migrate` | Step-by-step migration guide from OpenClaw native memory to ZedClaw + Honcho |
| `zedclaw memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `zedclaw memory status` | Show active memory provider and config |
| `zedclaw memory off` | Disable external memory provider |

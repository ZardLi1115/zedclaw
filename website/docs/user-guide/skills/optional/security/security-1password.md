...
| | |
|---|---|
| Source | Optional — install with `zedclaw skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by ZedClaw |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during ZedClaw terminal calls (desktop app flow only)
...
### Service Account (recommended for ZedClaw)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.zedclaw/.env` (the skill will prompt for this on first load).
...
## ZedClaw Execution Pattern (desktop app flow)

ZedClaw terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/zedclaw-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/zedclaw-op.sock"

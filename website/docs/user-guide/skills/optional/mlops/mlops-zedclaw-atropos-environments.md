---
title: "ZedClaw Atropos Environments — Build, test, and debug ZedClaw RL environments for Atropos training"
sidebar_label: "ZedClaw Atropos Environments"
description: "Build, test, and debug ZedClaw RL environments for Atropos training"
---
...
Build, test, and debug ZedClaw RL environments for Atropos training. Covers the ZedClawAgentBaseEnv interface, reward functions, agent loop integration, evaluation with tools, wandb logging, and the three CLI modes (serve/process/evaluate). Use when creating, reviewing, or fixing RL environments in the zedclaw repo.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `zedclaw skills install official/mlops/zedclaw-atropos-environments` |
| Path | `optional-skills/mlops/zedclaw-atropos-environments` |
| Version | `1.1.0` |
| Author | ZedClaw |
| License | MIT |
...
Guide for building RL environments in the zedclaw repo that integrate with the Atropos training framework.
...
Atropos BaseEnv (atroposlib/envs/base.py)
    └── ZedClawAgentBaseEnv (environments/zedclaw_base_env.py)
...
ZedClaw environments are special because they run a **multi-turn agent loop with tool calling** — not just single-turn completions. The base env handles the loop; you implement the task and scoring.

## File Locations

| File | Purpose |
|------|---------|
| `environments/zedclaw_base_env.py` | Base class with agent loop + tool resolution |
| `environments/agent_loop.py` | `ZedClawAgentLoop` + `AgentResult` dataclass |
| `environments/tool_context.py` | `ToolContext` for reward verification |
| `environments/tool_call_parsers.py` | Phase 2 tool call parsers (zedclaw, mistral, etc.) |
...
The whole point of zedclaw environments is agentic evaluation:

```python
async def evaluate(self, *args, **kwargs) -> None:
    import time, uuid
    from environments.agent_loop import ZedClawAgentLoop
...
2. **evaluate() must use ZedClawAgentLoop, not chat_completion** — Single-turn chat_completion has no tools. The whole point of zedclaw benchmarks is agentic evaluation with tool use.

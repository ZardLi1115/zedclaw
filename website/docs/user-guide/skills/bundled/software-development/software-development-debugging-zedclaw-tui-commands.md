---
title: "Debugging ZedClaw Tui Commands — Debug ZedClaw TUI slash commands: Python, gateway, Ink UI"
sidebar_label: "Debugging ZedClaw Tui Commands"
description: "Debug ZedClaw TUI slash commands: Python, gateway, Ink UI"
---
...
Debug ZedClaw TUI slash commands: Python, gateway, Ink UI.
...
| Path | `skills/software-development/debugging-zedclaw-tui-commands` |
| Version | `1.0.0` |
| Author | ZedClaw |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `debugging`, `zedclaw`, `tui`, `slash-commands`, `typescript`, `python` |
...
# Debugging ZedClaw TUI Slash Commands
...
Use this skill when you encounter issues with slash commands in the ZedClaw TUI, particularly when commands aren't showing in autocomplete, aren't working properly in the TUI, or need to be added/updated.
...
<!-- ascii-guard-ignore -->
```
Python backend (zedclaw_cli/commands.py)     <- canonical COMMAND_REGISTRY
       │
       ▼
...
3. **Check if the command exists in the Python backend:**
   ```bash
   search_files --pattern "CommandDef" --file_glob "*.py" --path zedclaw_cli/
   search_files --pattern "commandname" --path zedclaw_cli/commands.py --context 3
   ```
...
4. If the command runs server-side, add a handler in `ZedClawCLI.process_command()` in `cli.py`:
   ```python
...
1. **Command shows in TUI but not in autocomplete.** The command is defined in the TUI codebase but missing from `COMMAND_REGISTRY` in `zedclaw_cli/commands.py`. Autocomplete data ships from Python.
...
1. Rebuild the TUI:
   ```bash
   cd /home/bb/zedclaw && npm --prefix ui-tui run build
   ```
...
4. Execute the command and confirm:
   ...
   - Any persisted config updates correctly (`read_file ~/.zedclaw/config.yaml`)

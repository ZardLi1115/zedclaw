...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.zedclaw/.env ] && grep -q "^GITHUB_TOKEN=" ~/.zedclaw/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.zedclaw/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
```bash
source "${ZEDCLAW_HOME:-$HOME/.zedclaw}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by ZedClaw. Code looks clean — good test coverage, no security concerns."

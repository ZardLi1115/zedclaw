...
sidebar_position: 16
title: "Yuanbao"
description: "Connect ZedClaw to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect ZedClaw to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.zedclaw/.env`:
...
All standard ZedClaw commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.zedclaw/logs/gateway.log`
...
4. Consider enabling verbose logging: `ZEDCLAW_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. ZedClaw automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
ZEDCLAW_LOG_LEVEL=debug zedclaw gateway
```
...
```bash
zedclaw chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```

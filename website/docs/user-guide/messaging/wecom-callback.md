...
Connect ZedClaw to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
ZedClaw supports two WeCom integration modes:
...
3. ZedClaw decrypts the message, queues it for the agent
...
(Use `zedclaw gateway start` only after `zedclaw gateway install` has registered the systemd/launchd service.)

...
title: "Git Worktrees"
description: "Run multiple ZedClaws safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

ZedClaw is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with ZedClaw so each session has a clean, isolated working directory.

## Why Use Worktrees with ZedClaw?

ZedClaw treats the **current working directory** as the project root:

- CLI: the directory where you run `zedclaw` or `zedclaw chat`
...
git worktree add ../repo-feature feature/zedclaw-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/zedclaw-experiment` checked out in that directory

Now you can `cd` into the new worktree and run ZedClaw there:

```bash
cd ../repo-feature

# Start ZedClaw in the worktree
zedclaw
```
...
git worktree add ../repo-experiment-a feature/zedclaw-a
git worktree add ../repo-experiment-b feature/zedclaw-b
```
...
Each ZedClaw process:

- Works on its own branch (`feature/zedclaw-a` vs `feature/zedclaw-b`).
...
- ZedClaw checkpoint data under `~/.zedclaw/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per ZedClaw experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/zedclaw-checkpoints-docs`, `feature/zedclaw-refactor-tests`.
- **Commit frequently**
...
- **Avoid running ZedClaw from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `zedclaw -w` (Automatic Worktree Mode)

ZedClaw has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `zedclaw/zedclaw-<hash>`).
...
```bash
zedclaw -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `zedclaw -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each ZedClaw session its own clean checkout.

...
ZedClaw automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.zedclaw/state.db`) — structured session metadata with FTS5 full-text search
2. **JSONL transcripts** (`~/.zedclaw/sessions/`) — raw conversation transcripts including tool calls (gateway)
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`zedclaw` or `zedclaw chat`) |
...
zedclaw -c "my project"

# If there are lineage variants (my project, my project #2, my project #3),
# this automatically resumes the most recent one
zedclaw -c "my project"   # → resumes "my project #3"
```
...
zedclaw --resume 20250305_091523_a1b2c3d4
zedclaw -r 20250305_091523_a1b2c3d4

# Resume by title
zedclaw --resume "refactoring auth"

# Or with the chat subcommand
zedclaw chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `zedclaw sessions list`.

### Conversation Recap on Resume

When you resume a session, ZedClaw displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a ZedClaw session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.zedclaw/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `zedclaw -r "<title>"` from the shell) and pick up where the platform left off.
...
```bash
zedclaw sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), ZedClaw creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`zedclaw -c "my project"`), it automatically picks the most recent session in the lineage.
...
ZedClaw provides a full set of session management commands via `zedclaw sessions`:
...
# Filter by platform
zedclaw sessions list --source telegram

# Show more sessions
zedclaw sessions list --limit 50
```
...
zedclaw sessions export backup.jsonl

# Export sessions from a specific platform
zedclaw sessions export telegram-history.jsonl --source telegram

# Export a single session
zedclaw sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
zedclaw sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
zedclaw sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
zedclaw sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
zedclaw sessions prune

# Custom age threshold
zedclaw sessions prune --older-than 30

# Only prune sessions from a specific platform
zedclaw sessions prune --source telegram --older-than 60

# Skip confirmation
zedclaw sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`zedclaw insights`](/docs/reference/cli-commands#zedclaw-insights).
...
When ZedClaw cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, ZedClaw uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to ZedClaw in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.zedclaw/state.db` | All session metadata + messages with FTS5 |
| Gateway transcripts | `~/.zedclaw/sessions/` | JSONL transcripts per session + sessions.json index |
| Gateway index | `~/.zedclaw/sessions/sessions.json` | Maps session keys to active session IDs |
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every ZedClaw process in the same `ZEDCLAW_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.zedclaw/config.yaml`:
...
zedclaw sessions delete <session_id>

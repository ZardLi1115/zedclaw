...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/zedclaw/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    zedclaw_meta = (fm.get("metadata") or {}).get("zedclaw") or {}
    tags = zedclaw_meta.get("tags") or []
    related = zedclaw_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `zedclaw skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with ZedClaw"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "ZedClaw ships with a large built-in skill library copied into `~/.zedclaw/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "ZedClaw also syncs bundled skills on `zedclaw update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.zedclaw/skills/` tree, it is still shipped with ZedClaw; restore it with `zedclaw skills reset <name> --restore`.",
        "",

# ZedClaw Has Had "Routines" Since March
...
ZedClaw equivalent — works today:
```bash
...
ZedClaw equivalent — works today:
```bash
zedclaw webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working ZedClaw implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | ZedClaw |
|---|---|---|
...
## Things ZedClaw Does That Routines Can't
...
ZedClaw has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
ZedClaw is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
zedclaw gateway setup    # enable webhooks
zedclaw webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation templates gallery: [zedclaw.nousresearch.com/docs/guides/automation-templates](https://zedclaw.nousresearch.com/docs/guides/automation-templates)

Documentation: [zedclaw.nousresearch.com](https://zedclaw.nousresearch.com)

GitHub: [github.com/NousResearch/zedclaw](https://github.com/NousResearch/zedclaw)

---

*ZedClaw is built by [Nous Research](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*

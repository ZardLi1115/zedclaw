...
# Python-driven tool session (e.g. ZedClaw terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/zedclaw.git"
REPO_URL_HTTPS="https://github.com/NousResearch/zedclaw.git"
ZEDCLAW_HOME="${ZEDCLAW_HOME:-$HOME/.zedclaw}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${ZEDCLAW_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$ZEDCLAW_INSTALL_DIR"
...
#   code at /usr/local/lib/zedclaw, command at /usr/local/bin/zedclaw,
#   data still at /root/.zedclaw (ZEDCLAW_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/.zedclaw/zedclaw"
            echo "                   default (root, Linux): /usr/local/lib/zedclaw"
            echo "  --zedclaw-home PATH  Data directory (default: ~/.zedclaw, or \$ZEDCLAW_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, ZedClaw installs the code under"
            echo "  /usr/local/lib/zedclaw and links the command into"
            echo "  /usr/local/bin/zedclaw (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$ZEDCLAW_HOME"
            echo "  (default /root/.zedclaw).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$ZEDCLAW_HOME/zedclaw are preserved in-place."
            exit 0
            ;;
        *)
...
# Decide where the repo checkout + venv live, and where the `zedclaw` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $ZEDCLAW_HOME/zedclaw
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $ZEDCLAW_HOME/zedclaw
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/zedclaw
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $ZEDCLAW_HOME/zedclaw — then preserve it)
#
# Always no-op when the user set --dir or $ZEDCLAW_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in ZEDCLAW_HOME.
    if is_termux; then
        INSTALL_DIR="$ZEDCLAW_HOME/zedclaw"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$ZEDCLAW_HOME/zedclaw/.git" ]; then
            INSTALL_DIR="$ZEDCLAW_HOME/zedclaw"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/zedclaw)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/zedclaw"
        ROOT_FHS_LAYOUT=true
        log_info "Root install on Linux — using FHS layout"
        log_info "  Code:    $INSTALL_DIR"
        log_info "  Command: /usr/local/bin/zedclaw"
        log_info "  Data:    $ZEDCLAW_HOME (unchanged)"
        return 0
    fi
...
get_zedclaw_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/zedclaw" ]; then
        echo "$link_dir/zedclaw"
    else
        echo "zedclaw"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/NousResearch/zedclaw/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/zedclaw-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/zedclaw-uv-installer.$$.sh")"
...
check_node() {
    ...
    if [ -x "$ZEDCLAW_HOME/node/bin/node" ]; then
        export PATH="$ZEDCLAW_HOME/node/bin:$PATH"
        local found_ver=$("$ZEDCLAW_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (ZedClaw-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.zedclaw/node/..."
    ...
    # Place into ~/.zedclaw/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$ZEDCLAW_HOME/node"
    mkdir -p "$ZEDCLAW_HOME"
    mv "$extracted_dir" "$ZEDCLAW_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$ZEDCLAW_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$ZEDCLAW_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$ZEDCLAW_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$ZEDCLAW_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$ZEDCLAW_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.zedclaw/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. ZedClaw install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "ZedClaw itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="zedclaw-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if ZedClaw behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up zedclaw command..."

    if [ "$USE_VENV" = true ]; then
        ZEDCLAW_BIN="$INSTALL_DIR/venv/bin/zedclaw"
    else
        ZEDCLAW_BIN="$(which zedclaw 2>/dev/null || echo "")"
        if [ -z "$ZEDCLAW_BIN" ]; then
            log_warn "zedclaw not found on PATH after install"
            return 0
        fi
    fi

    # Verify the entry point script was actually generated
    if [ ! -x "$ZEDCLAW_BIN" ]; then
        log_warn "zedclaw entry point not found at $ZEDCLAW_BIN"
    ...
    # Create a user-facing shim for the zedclaw command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    cat > "$command_link_dir/zedclaw" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ZEDCLAW_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/zedclaw"
    log_success "Installed zedclaw launcher → $command_link_display_dir/zedclaw"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "zedclaw command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v zedclaw' \
                >/dev/null 2>&1; then
        ...
        log_info "zedclaw not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# ZedClaw — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# ZedClaw — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# ZedClaw — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so zedclaw works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.zedclaw directory structure (config at top level, code in subdir)
    mkdir -p "$ZEDCLAW_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.zedclaw/.env (top level, easy to find)
    if [ ! -f "$ZEDCLAW_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$ZEDCLAW_HOME/.env"
            log_success "Created ~/.zedclaw/.env from template"
        else
            touch "$ZEDCLAW_HOME/.env"
            log_success "Created ~/.zedclaw/.env"
        fi
    else
        log_info "~/.zedclaw/.env already exists, keeping it"
    fi

    # Create config.yaml at ~/.zedclaw/config.yaml (top level, easy to find)
    if [ ! -f "$ZEDCLAW_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$ZEDCLAW_HOME/config.yaml"
            log_success "Created ~/.zedclaw/config.yaml from template"
        fi
    else
        log_info "~/.zedclaw/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$ZEDCLAW_HOME/SOUL.md" ]; then
        cat > "$ZEDCLAW_HOME/SOUL.md" << 'SOUL_EOF'
# ZedClaw Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how ZedClaw communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.zedclaw/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.zedclaw/"

    # Seed bundled skills into ~/.zedclaw/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.zedclaw/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.zedclaw/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$ZEDCLAW_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$ZEDCLAW_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.zedclaw/skills/"
        fi
    fi
}
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        case "$DISTRO" in
            ubuntu|debian|raspbian|pop|linuxmint|elementary|zorin|kali|parrot)
                ...
                log_info "This is standard Playwright setup — ZedClaw itself does not require root access."
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (zedclaw --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'zedclaw setup' after install."
        return 0
    fi
    ...
    # Run zedclaw setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m zedclaw_cli.main setup < /dev/tty
    else
        python -m zedclaw_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$ZEDCLAW_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for ZedClaw to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$ZEDCLAW_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'zedclaw whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                ZEDCLAW_CMD="$(get_zedclaw_command_path)"
                $ZEDCLAW_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'zedclaw whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'zedclaw gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $ZEDCLAW_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $ZEDCLAW_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: zedclaw gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: zedclaw gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $ZEDCLAW_CMD gateway > "$ZEDCLAW_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.zedclaw/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: zedclaw gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: zedclaw gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $ZEDCLAW_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $ZEDCLAW_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $ZEDCLAW_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}zedclaw${NC}              Start chatting"
    echo -e "   ${GREEN}zedclaw setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}zedclaw config${NC}       View/edit configuration"
    echo -e "   ${GREEN}zedclaw config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}zedclaw gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}zedclaw update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'zedclaw' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'zedclaw' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'zedclaw' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"

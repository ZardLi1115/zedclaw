#!/usr/bin/env bash
# Canonical test runner for zedclaw. Run this instead of calling
...
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.zedclaw/zedclaw/venv"; do
...
# Unset ZEDCLAW_* behavioral vars too.
unset ZEDCLAW_YOLO_MODE ZEDCLAW_INTERACTIVE ZEDCLAW_QUIET ZEDCLAW_TOOL_PROGRESS \
      ZEDCLAW_TOOL_PROGRESS_MODE ZEDCLAW_MAX_ITERATIONS ZEDCLAW_SESSION_PLATFORM \
      ZEDCLAW_SESSION_CHAT_ID ZEDCLAW_SESSION_CHAT_NAME ZEDCLAW_SESSION_THREAD_ID \
      ZEDCLAW_SESSION_SOURCE ZEDCLAW_SESSION_KEY ZEDCLAW_GATEWAY_SESSION \
      ZEDCLAW_CRON_SESSION \
      ZEDCLAW_PLATFORM ZEDCLAW_INFERENCE_PROVIDER ZEDCLAW_MANAGED ZEDCLAW_DEV \
      ZEDCLAW_CONTAINER ZEDCLAW_EPHEMERAL_SYSTEM_PROMPT ZEDCLAW_TIMEZONE \
      ZEDCLAW_REDACT_SECRETS ZEDCLAW_BACKGROUND_NOTIFICATIONS ZEDCLAW_EXEC_ASK \
      ZEDCLAW_HOME_MODE 2>/dev/null || true
...
# If a system-wide zedclaw pytest_live_guard plugin is installed at
# $HOME/.zedclaw/pytest_live_guard.py, force-load it here so every test run
...
if [ -f "$HOME/.zedclaw/pytest_live_guard.py" ]; then
  case ":${PYTHONPATH:-}:" in
    *":$HOME/.zedclaw:"*) ;;
    *) export PYTHONPATH="${PYTHONPATH:+$PYTHONPATH:}$HOME/.zedclaw" ;;
  esac
...
WORKERS="${ZEDCLAW_TEST_WORKERS:-4}"

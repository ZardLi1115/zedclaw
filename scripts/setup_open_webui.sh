...
# Bootstrap Open WebUI against ZedClaw's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.zedclaw/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-zedclaw.sh
...
#   OPEN_WEBUI_NAME='Johnny ZedClaw'
...
#   ZEDCLAW_API_PORT=8642
#   ZEDCLAW_API_HOST=127.0.0.1
#   ZEDCLAW_API_MODEL_NAME='ZedClaw'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-ZedClaw WebUI}"
...
ZEDCLAW_ENV_FILE="${ZEDCLAW_ENV_FILE:-$HOME/.zedclaw/.env}"
ZEDCLAW_API_PORT="${ZEDCLAW_API_PORT:-8642}"
ZEDCLAW_API_HOST="${ZEDCLAW_API_HOST:-127.0.0.1}"
ZEDCLAW_API_CONNECT_HOST="${ZEDCLAW_API_CONNECT_HOST:-127.0.0.1}"
ZEDCLAW_API_MODEL_NAME="${ZEDCLAW_API_MODEL_NAME:-ZedClaw}"
ZEDCLAW_API_BASE_URL="http://${ZEDCLAW_API_CONNECT_HOST}:${ZEDCLAW_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-zedclaw.sh"
LOG_DIR="$HOME/.zedclaw/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$ZEDCLAW_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$ZEDCLAW_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.zedclaw.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.zedclaw</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.zedclaw"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.zedclaw"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-zedclaw.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to ZedClaw
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-zedclaw.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.zedclaw/logs/openwebui.log
StandardError=append:%h/.zedclaw/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-zedclaw.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$ZEDCLAW_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring ZedClaw API server is configured...'
  upsert_env API_SERVER_ENABLED true "$ZEDCLAW_ENV_FILE"
  upsert_env API_SERVER_HOST "$ZEDCLAW_API_HOST" "$ZEDCLAW_ENV_FILE"
  upsert_env API_SERVER_PORT "$ZEDCLAW_API_PORT" "$ZEDCLAW_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$ZEDCLAW_API_MODEL_NAME" "$ZEDCLAW_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$ZEDCLAW_ENV_FILE"
  ensure_env_permissions

  log 'Restarting ZedClaw gateway so API server settings take effect...'
  zedclaw gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${ZEDCLAW_API_CONNECT_HOST}:${ZEDCLAW_API_PORT}/health" >/dev/null; then
    log 'ZedClaw API server did not answer on the first check. Trying to start gateway in the background...'
    nohup zedclaw gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${ZEDCLAW_API_CONNECT_HOST}:${ZEDCLAW_API_PORT}/health" >/dev/null
  ...
  log "ZedClaw API endpoint: ${ZEDCLAW_API_BASE_URL}"

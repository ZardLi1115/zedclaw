...
param(
    ...
    [switch]$SkipSetup,
    [string]$Branch = "main",
    [string]$ZedClawHome = "$env:LOCALAPPDATA\zedclaw",
    [string]$InstallDir = "$env:LOCALAPPDATA\zedclaw\zedclaw"
)
...
$RepoUrlSsh = "git@github.com:NousResearch/zedclaw.git"
$RepoUrlHttps = "https://github.com/NousResearch/zedclaw.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "│             ⚕ ZedClaw Installer                    │" -ForegroundColor Magenta
    Write-Host "├─────────────────────────────────────────────────────────┤" -ForegroundColor Magenta
...
function Install-Git {
    ...
    which ZedClaw uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\zedclaw\git`` — never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    ZedClaw needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the ZedClaw copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\zedclaw\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``ZEDCLAW_GIT_BASH_PATH`` (User scope) so ZedClaw can find it in a fresh
    ...
    # Download PortableGit into $ZedClawHome\git.  Always works as long as
    # we can reach github.com — no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found — downloading PortableGit to $ZedClawHome\git\ ..."
    ...
    try {
        ...
        $release = Invoke-RestMethod -Uri $releaseApi -UseBasicParsing -Headers @{ "User-Agent" = "zedclaw-installer" }

        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected — PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent ZedClaw features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  ZedClaw needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``ZEDCLAW_GIT_BASH_PATH`` (User env scope) so ZedClaw can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $ZedClawHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $ZedClawHome\git\usr\bin\bash.exe
    $candidates += "$ZedClawHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$ZedClawHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("ZEDCLAW_GIT_BASH_PATH", $candidate, "User")
            $env:ZEDCLAW_GIT_BASH_PATH = $candidate
            Write-Info "Set ZEDCLAW_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe — ZedClaw may not find Git Bash."
    Write-Info "If needed, set ZEDCLAW_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$ZedClawHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$ZedClawHome\node;$env:Path"
        Write-Success "Node.js $version found (ZedClaw-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # Fallback: download binary zip to ~/.zedclaw/node/
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\zedclaw-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$ZedClawHome\node") { Remove-Item -Recurse -Force "$ZedClawHome\node" }
                Move-Item $extractedDir.FullName "$ZedClawHome\node"
                $env:Path = "$ZedClawHome\node;$env:Path"

                $version = & "$ZedClawHome\node\node.exe" --version
                Write-Success "Node.js $version installed to ~/.zedclaw/node/"
                $script:HasNode = $true
...
function Install-Repository {
    ...
    if (Test-Path $InstallDir) {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                Write-Info "terminals, running zedclaw processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                $zipUrl = "https://github.com/NousResearch/zedclaw/archive/refs/heads/$Branch.zip"
                $zipPath = "$env:TEMP\zedclaw-$Branch.zip"
                $extractPath = "$env:TEMP\zedclaw-extract"
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install zedclaw package even with no extras. Inspect the uv pip install output above."
    }

    # Verify the dashboard deps specifically — they're the most common thing
    # users hit and lazy-import errors from `zedclaw dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable — `zedclaw dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `zedclaw dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up zedclaw command..."
    
    if ($NoVenv) {
        $zedclawBin = "$InstallDir"
    } else {
        $zedclawBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so zedclaw is globally available
    # On Windows, the zedclaw.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$zedclawBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$zedclawBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $zedclawBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set ZEDCLAW_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\zedclaw instead
    # of the Unix default ~/.zedclaw
    $currentZedClawHome = [Environment]::GetEnvironmentVariable("ZEDCLAW_HOME", "User")
    if (-not $currentZedClawHome -or $currentZedClawHome -ne $ZedClawHome) {
        [Environment]::SetEnvironmentVariable("ZEDCLAW_HOME", $ZedClawHome, "User")
        Write-Success "Set ZEDCLAW_HOME=$ZedClawHome"
    }
    $env:ZEDCLAW_HOME = $ZedClawHome
    
    # Update current session
    $env:Path = "$zedclawBin;$env:Path"
    
    Write-Success "zedclaw command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.zedclaw directory structure
    New-Item -ItemType Directory -Force -Path "$ZedClawHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZedClawHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZedClawHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZedClawHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZedClawHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZedClawHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZedClawHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZedClawHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZedClawHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$ZedClawHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.zedclaw/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.zedclaw/.env"
        }
    } else {
        Write-Info "~/.zedclaw/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$ZedClawHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.zedclaw/config.yaml from template"
        }
    } else {
        Write-Info "~/.zedclaw/config.yaml already exists, keeping it"
    }
    ...
    # (the default PS5 behaviour), and ZedClaw's prompt-injection scanner
...
"@
        ...
        Write-Success "Created ~/.zedclaw/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.zedclaw/"
    
    # Seed bundled skills into ~/.zedclaw/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.zedclaw/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.zedclaw/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$ZedClawHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.zedclaw/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'zedclaw setup tools' later."
        return
    }
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\zedclaw-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\zedclaw-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
                try {
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\zedclaw-npm-tui-$(Get-Random).log"
...
function Install-PlatformSdks {
    ...
    # ~/.zedclaw/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m zedclaw_cli.main setup
    } else {
        python -m zedclaw_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $zedclawCmd = "$InstallDir\venv\Scripts\zedclaw.exe"
    if (-not (Test-Path $zedclawCmd)) {
        $zedclawCmd = "zedclaw"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$ZedClawHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'zedclaw whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$ZedClawHome\logs\gateway.log"
            Start-Process -FilePath $zedclawCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$ZedClawHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: zedclaw gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: zedclaw gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$ZedClawHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$ZedClawHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ZedClawHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ZedClawHome\zedclaw\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   zedclaw              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   zedclaw setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   zedclaw config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   zedclaw config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   zedclaw gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   zedclaw update       " -NoNewline -ForegroundColor Green
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/NousResearch/zedclaw/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow

"""
ZedClawAgentBaseEnv -- Abstract Base Environment for ZedClaw-Agent + Atropos

Provides the Atropos integration plumbing that all zedclaw environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via ZedClawAgentLoop
...
# Ensure the zedclaw repo root is on sys.path so that imports like
...
# Load API keys from zedclaw/.env so all environments can access them
...
from environments.agent_loop import AgentResult, ZedClawAgentLoop
...
# Import zedclaw toolset infrastructure
...
class ZedClawAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for zedclaw Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of zedclaw toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: zedclaw, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
    ...
    default_result_size_chars: int = Field(
        ...
        "to sandbox. Results exceeding this are written to /tmp/zedclaw-results/ "
...
class ZedClawAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for zedclaw Atropos integration.
    ...
    name: Optional[str] = "zedclaw"
    env_config_cls = ZedClawAgentEnvConfig

    def __init__(
        self,
        config: ZedClawAgentEnvConfig,
        ...
        # Set terminal environment variables so zedclaw tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY zedclaw tool (terminal, file, web,

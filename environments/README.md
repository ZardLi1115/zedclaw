# ZedClaw-Agent Atropos Environments

This directory contains the integration layer between **zedclaw's** tool-calling capabilities and the **Atropos** RL training framework. It provides everything needed to run agentic LLMs through multi-turn tool-calling loops, score their output with arbitrary reward functions, and feed results into Atropos for training or evaluation.
...
```
                    ...
                    └───────────┬───────────┘
                                │ inherits
                    ┌───────────┴───────────┐
                    │  ZedClawAgentBaseEnv    │  zedclaw_base_env.py
                    │  - Terminal backend    │
                    │  - Tool resolution     │
                    │  - Agent loop          │
                    │  - ToolContext          │
              ...
              ┌─────────────────┼─────────────────┐
              │                 │                  │
     TerminalTestEnv     ZedClawSweEnv    TerminalBench2EvalEnv
...
**ZedClawAgentBaseEnv** (`zedclaw_base_env.py`) extends BaseEnv with zedclaw specifics:
- Sets `os.environ["TERMINAL_ENV"]` to configure the terminal backend (local, docker, ssh, singularity, modal, daytona, vercel_sandbox)
- Resolves zedclaw toolsets via `_resolve_tools_for_group()` (calls `get_tool_definitions()` which queries `tools/registry.py`)
- Implements `collect_trajectory()` which runs the full agent loop and computes rewards
- Supports two-phase operation (Phase 1: OpenAI server, Phase 2: VLLM ManagedServer)
- Applies monkey patches for async-safe tool operation at import time

Concrete environments inherit from `ZedClawAgentBaseEnv` and implement:
...
`ZedClawAgentLoop` is the reusable multi-turn agent engine. It runs the same pattern as zedclaw's `run_agent.py`:
...
`ToolContext` is a per-rollout handle that gives reward/verification functions direct access to **all** zedclaw tools, scoped to the rollout's `task_id`. The same `task_id` means the terminal/browser session is the SAME one the model used during its rollout -- all state (files, processes, browser tabs) is preserved.
...
- **Generic**: `call_tool(name, args)` -- call any zedclaw tool by name
- **Cleanup**: `cleanup()` -- release all resources (called automatically after `compute_reward`)

### Patches (`patches.py`)

**Problem**: Some zedclaw tools use `asyncio.run()` internally (e.g., the Modal backend). This crashes when called from inside Atropos's event loop because `asyncio.run()` cannot be nested.
...
Available parsers:
- `zedclaw` -- ZedClaw/ChatML `<tool_call>` XML format
...
├── zedclaw_base_env.py            # Abstract base (ZedClawAgentBaseEnv)
...
│
├── zedclaw_swe_env/               # SWE-bench style training environment
│   └── zedclaw_swe_env.py
│
...
2. Create your env file inheriting from `ZedClawAgentBaseEnv`
...
2. Inherit from `ZedClawAgentBaseEnv`
...
|-------|-------------|---------|
| `enabled_toolsets` | Which zedclaw toolsets to enable | `None` (all) |
...
| `tool_call_parser` | Parser name for Phase 2 | `zedclaw` |
